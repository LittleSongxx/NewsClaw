...
const isDesktop = typeof window !== 'undefined' && !!window.newsClawAPI
...
async function refresh() {
  try {
    const cfg = await window.newsClawAPI!.getLocalToolsConfig()
...
async function onToggleEnabled(e: Event) {
  ...
  busy.value = true
  try {
    await window.newsClawAPI!.setLocalToolsConfig({ enabled })
    await refresh()
  } catch {
...
async function onAddDir() {
  busy.value = true
  try {
    const result = await window.newsClawAPI!.addLocalToolsDir()
    await refresh()
...
async function onRemoveDir(dir: string) {
  ...
  busy.value = true
  try {
    await window.newsClawAPI!.removeLocalToolsDir(dir)
    await refresh()

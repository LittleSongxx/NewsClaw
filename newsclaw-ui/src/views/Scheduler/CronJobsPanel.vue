...
// The AI-news vertical uses Feishu as its primary IM surface.  Keep every
// configured channel available, but put Feishu first so a new Cron is aligned
// with the seeded default without hiding a user's existing choices.
const deliveryChannels = computed(() =>
  [...channels.value]
    .filter((c) => c.enabled !== false && !NON_PUSHABLE_CHANNEL_TYPES.has(c.channelType))
    .sort((a, b) => {
      const priority = (type?: string) => type === 'feishu' ? 0 : 1

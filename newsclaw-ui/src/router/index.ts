  ...
  routes: [
    {
      ...
      children: [
        // ==================== Core ====================
        {
          ...
          meta: { title: 'AI 动态', requiredCapability: 'view:dashboard' },
        },
        {
          path: 'chat',
          name: 'Chat',
        ...
        // Retired from the AI-news vertical; preserve old bookmarks with a
        // deterministic redirect instead of loading the unrelated demo.
        { path: 'enterprise', redirect: '/ai-news' },
        {
          path: 'memory',
          name: 'Memory',
        ...
        // Retired from the AI-news vertical; extensions remain available via
        // the runtime/tool APIs rather than a separate product surface.
        { path: 'plugins', redirect: '/ai-news' },
        // ==================== Settings (absorbs advanced pages) ====================
        {
          ...
          children: [
            ...
            // Non-goal modalities are retired from this vertical. Keep old
            // links deterministic without exposing their settings surfaces.
            { path: 'tts', redirect: '/ai-news' },
            { path: 'stt', redirect: '/ai-news' },
            { path: 'music', redirect: '/ai-news' },
            { path: 'video', redirect: '/ai-news' },
            { path: 'model3d', redirect: '/ai-news' },
            ...
            { path: 'datasources', redirect: '/ai-news' },
            { path: 'mcp-servers', redirect: '/ai-news' },
            {
              path: 'tools',
              name: 'SettingsTools',
            ...
            // ACP is outside the editorial operating loop.
            { path: 'acp', redirect: '/ai-news' },
            {
              path: 'token-usage',
...
router.beforeEach(async (to) => {
  ...
  if (requireAdmin && !store.isGlobalAdmin) {
    return store.can('view:dashboard') ? { path: '/ai-news' } : (store.can('chat') ? { path: '/chat' } : { path: '/forbidden' })
  }

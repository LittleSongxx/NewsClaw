...
function renderProductCards(rawCode: string): string {
  ...
  const cards = items.map((it) => {
    ...
    const media = img
      ? `<div class="product-card__media"><img src="${escapeHtml(img)}" alt="${name}" referrerpolicy="no-referrer" loading="lazy" decoding="async"></div>`
    ...
    if (href) {
      return `<a class="product-card" href="${escapeHtml(href)}" target="_blank" rel="noopener noreferrer">${media}${body}</a>`
    }
...
const purifyConfig = {
  ...
  // DOMPurify + happy-dom applies this expression to a few non-URI
  // attributes as well.  These additional literals are fixed values emitted
  // by the product-card renderer, not caller-controlled URLs.
  ALLOWED_URI_REGEXP: /^(?:https?:|mailto:|#|\/|\.\/|\.\.\/|_blank$|noopener(?:\s+noreferrer)?$|no-referrer$|lazy$|async$)/i,
}
...
export function useMarkdownRenderer() {
  function renderMarkdown(content: string, opts?: RenderMarkdownOptions): string {
    ...
    // happy-dom (used by Vitest) can treat the first block element of an
    // unsuffixed HTML fragment as the fragment root and drop it while
    // DOMPurify walks the tree.  A leading text node keeps the fragment
    // boundary explicit.  It is inert and does not relax any sanitizer rule;
    // browsers that do not have this parser quirk simply retain no visible
    // whitespace in the returned markup.
    const result = DOMPurify.sanitize(`\n${rawHtml}`, purifyConfig).trimStart()

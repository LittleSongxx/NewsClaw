#!/usr/bin/env python3
"""Stage built web assets under src/newsclaw for release wheels.

The Python package can build without Node outputs. A release job that needs a
self-contained wheel builds the frontend first, then runs this script so Hatch
includes the package-local artifacts declared in pyproject.toml. Docs staging
is opt-in (``--docs``) because this repository does not ship a docs site.
"""
...
WEB_TARGET = ROOT / "src" / "newsclaw" / "web"
DOCS_TARGET = ROOT / "src" / "newsclaw" / "docs_dist"
...
def stage_package_assets(
    *,
    ...
    docs_source: Path | None = DOCS_SOURCE,
    web_target: Path = WEB_TARGET,
    docs_target: Path = DOCS_TARGET,
) -> list[tuple[str, int]]:
    staged = [
...
def main() -> int:
    ...
    parser.add_argument(
        "--docs",
        action="store_true",
        help="also stage the user docs site (requires docs-site/ to be built)",
    )
    ...
    for label, count in stage_package_assets(docs_source=DOCS_SOURCE if args.docs else None):

#!/usr/bin/env bash
#
# NewsClaw One-Click Install Script (PyPI)
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/newsclaw/newsclaw/main/scripts/quickstart.sh | bash
#
# Recommended (download then run with parameters):
#   curl -fsSL -o quickstart.sh https://raw.githubusercontent.com/newsclaw/newsclaw/main/scripts/quickstart.sh
...
NEWSCLAW_ROOT_DEFAULT="${NEWSCLAW_ROOT:-$HOME/.newsclaw}"
NEWSCLAW_APP_DIR_DEFAULT="${NEWSCLAW_APP_DIR:-$NEWSCLAW_ROOT_DEFAULT/app}"
NEWSCLAW_VENV_DIR_DEFAULT="${NEWSCLAW_VENV_DIR:-$NEWSCLAW_ROOT_DEFAULT/venv}"

EXTRAS="${NEWSCLAW_EXTRAS:-}"
INDEX_URL="${NEWSCLAW_INDEX_URL:-}"
TORCH_MODE="${NEWSCLAW_TORCH_MODE:-cpu}" # cpu|skip
INSTALL_PLAYWRIGHT="${NEWSCLAW_INSTALL_PLAYWRIGHT:-1}" # 1|0
RUN_INIT="${NEWSCLAW_RUN_INIT:-1}" # 1|0
INSTALL_WRAPPER="${NEWSCLAW_INSTALL_WRAPPER:-1}" # 1|0
YES="${NEWSCLAW_YES:-0}" # 1|0
FORCE_WRAPPER="${NEWSCLAW_FORCE_WRAPPER:-0}" # 1|0

usage() {
  cat <<'EOF'
NewsClaw one-click install (PyPI).

Options:
  --dir <path>            App working directory (default: ~/.newsclaw/app)
  --venv <path>           Virtualenv directory (default: ~/.newsclaw/venv)
  ...
  --no-init               Skip running `newsclaw init`
  --no-wrapper            Skip creating ~/.local/bin/newsclaw wrapper
  --force-wrapper         Overwrite existing wrapper if present
  -y, --yes               Non-interactive defaults
  -h, --help              Show this help

Environment variables (optional):
  NEWSCLAW_ROOT, NEWSCLAW_APP_DIR, NEWSCLAW_VENV_DIR, NEWSCLAW_EXTRAS,
  NEWSCLAW_INDEX_URL, NEWSCLAW_TORCH_MODE, NEWSCLAW_INSTALL_PLAYWRIGHT,
  NEWSCLAW_RUN_INIT, NEWSCLAW_INSTALL_WRAPPER, NEWSCLAW_YES, NEWSCLAW_FORCE_WRAPPER
EOF
}

APP_DIR="$NEWSCLAW_APP_DIR_DEFAULT"
VENV_DIR="$NEWSCLAW_VENV_DIR_DEFAULT"
...
echo -e "${CYAN}=== NewsClaw One-Click Install ===${NC}"
...
echo -e "${GREEN}✓ NewsClaw installed${NC}"
...
if [[ "$RUN_INIT" == "1" ]]; then
  echo -e "${YELLOW}Running setup wizard (newsclaw init)...${NC}"
  ...
  if [[ -t 0 ]]; then
    ...
    if [[ -e /dev/tty ]]; then
      ...
      echo "  cd \"$APP_DIR\" && source \"$VENV_DIR/bin/activate\" && newsclaw init"
    fi
  fi
  popd >/dev/null
fi
...
if [[ "$INSTALL_WRAPPER" == "1" ]]; then
  ...
  WRAPPER_PATH="$WRAPPER_DIR/newsclaw"
  mkdir -p "$WRAPPER_DIR"

...
EOF
    ...
    print_info "提示: 通过 Setup Center 或 newsclaw llm-config 添加 LLM 端点"
...
ExecStart=$(pwd)/venv/bin/newsclaw serve
Restart=always
RestartSec=10
...
WantedBy=multi-user.target"
    ...
    local service_file="newsclaw.service"
    ...
    echo "  sudo systemctl enable newsclaw"
    echo "  sudo systemctl start newsclaw"
}
...
show_completion() {
    ...
    echo -e "     ${CYAN}newsclaw llm-config${NC}  # 交互式配置向导"
    ...
    echo ""
    echo -e "  4. 启动 Agent:"
    echo -e "     ${CYAN}newsclaw${NC}        # 交互模式"
    echo -e "     ${CYAN}newsclaw serve${NC}  # 服务模式 (Telegram/IM)"
    echo ""
    echo -e "${BLUE}新特性:${NC}"
...
main() {
    echo ""
    echo -e "${MAGENTA}╔════════════════════════════════════════╗"
    echo -e "║   NewsClaw 一键部署脚本 (Linux/macOS)   ║"
    echo -e "╚════════════════════════════════════════╝${NC}"
    echo ""

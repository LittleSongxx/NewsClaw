...
- 一键同步到：
  ...
  - apps/setup-center/src-tauri/Cargo.lock (newsclaw-setup-center package entry)
...
def _update_cargo_lock(version: str) -> bool:
    ...
    for i, raw in enumerate(lines):
        ...
        if in_pkg and line.startswith("name = "):
            ...
            is_target_pkg = name == "newsclaw-setup-center"
            continue
...
def _update_bundled_version(version: str) -> bool:
    ...
    """
    path = ROOT / "src" / "newsclaw" / "_bundled_version.txt"
    old = path.read_text(encoding="utf-8").strip() if path.exists() else ""
    if old == version:
        return False
    path.write_text(version, encoding="utf-8", newline="\n")
    print(f"src/newsclaw/_bundled_version.txt: {old} -> {version}")
    return True
...
def check(expected: str | None) -> int:
    ...
    # 找到 newsclaw-setup-center 这个 package 的 version
    lm = re.search(
        r'(?ms)^\[\[package\]\]\s*\nname\s*=\s*"newsclaw-setup-center"\s*\nversion\s*=\s*"([^"]+)"\s*$',
        cargo_lock,
    )
    ...
    # 独立子包的版本一致性（子包存在时才检查；本仓库不随主仓发布子包）
...
def main() -> int:
    ...
    sub.add_parser("sync", help="从 VERSION 同步到所有文件")

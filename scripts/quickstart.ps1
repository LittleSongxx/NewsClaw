<#
.SYNOPSIS
  NewsClaw 一键安装脚本（PyPI，Windows PowerShell）
.DESCRIPTION
  - 创建独立的 venv（默认在 %USERPROFILE%\.newsclaw\venv）
  - 安装 newsclaw（可选 extras、可选镜像）
  - 可选安装 Playwright 浏览器
  - 可选运行 newsclaw init（在 AppDir 目录生成 .env / data / identity）

默认用法（最简单）：
  irm https://raw.githubusercontent.com/newsclaw/newsclaw/main/scripts/quickstart.ps1 | iex

推荐用法（可传参数）：
  irm https://raw.githubusercontent.com/newsclaw/newsclaw/main/scripts/quickstart.ps1 -OutFile quickstart.ps1
  .\quickstart.ps1 -Extras all -IndexUrl https://pypi.tuna.tsinghua.edu.cn/simple
#>

[CmdletBinding()]
param(
  [string]$AppDir = "$env:USERPROFILE\.newsclaw\app",
  [string]$VenvDir = "$env:USERPROFILE\.newsclaw\venv",
  [string]$Extras = "",
...
Write-Color "=== NewsClaw One-Click Install ===" Cyan
...
# Activate venv (for `newsclaw` entrypoint)
...
Write-Color "OK NewsClaw installed" Green
Write-Host ""
...
if (-not $NoInit) {
  Write-Color "Running setup wizard (newsclaw init)..." Cyan
  Push-Location $AppDir
  try { newsclaw init } finally { Pop-Location }
}

if (-not $NoWrapper) {
  $binDir = "$env:USERPROFILE\.newsclaw\bin"
  New-Item -ItemType Directory -Path $binDir -Force | Out-Null
  $cmdPath = Join-Path $binDir "newsclaw.cmd"

...
#[tauri::command]
pub(crate) fn newsclaw_service_status(workspace_id: String) -> Result<ServiceStatus, String> {
...
#[tauri::command]
pub(crate) fn newsclaw_check_pid_alive(workspace_id: String) -> Result<bool, String> {
...
#[tauri::command]
pub(crate) async fn newsclaw_service_start(
    venv_dir: String,
    workspace_id: String,
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        newsclaw_service_start_impl(venv_dir, workspace_id)
    })
    .await
...
pub(crate) fn newsclaw_service_start_impl(
    venv_dir: String,
    workspace_id: String,
) -> Result<ServiceStatus, String> {
    ...
    cmd.env("NEWSCLAW_DESKTOP_SESSION_TOKEN", desktop_session_token());
    cmd.env(
        "NEWSCLAW_SPAWN_STARTED_AT_MS",
...
#[tauri::command]
pub(crate) fn newsclaw_service_stop(workspace_id: String) -> Result<ServiceStatus, String> {
    let _lifecycle_guard = BACKEND_LIFECYCLE_LOCK.lock().unwrap();
    set_backend_manually_stopped(&workspace_id, true)?;
    let pid_file = service_pid_file(&workspace_id);
    let port = read_workspace_api_port(&workspace_id);
...
#[tauri::command]
pub(crate) fn newsclaw_service_log(
    workspace_id: String,
...
///
/// 老的"重试启动/修复"只是再次调 `newsclaw_service_start`，但 `ensure_venv`
...
pub(crate) fn repair_runtime_env() -> Result<String, String> {
    ...
    if let Some(ws_id) = state.current_workspace_id.clone() {
        match newsclaw_service_stop(ws_id.clone()) {
    ...
    if status == "dead" {
        #[cfg(windows)]
        {
            ...
            cmd.args([
                ...
                "try { \
                    $aumid = 'com.newsclaw.setupcenter'; \
                    $rp = \"HKCU:\\SOFTWARE\\Classes\\AppUserModelId\\$aumid\"; \
                    if (!(Test-Path $rp)) { New-Item $rp -Force | Out-Null; Set-ItemProperty $rp -Name DisplayName -Value 'NewsClaw Desktop' }; \
...
pub(crate) fn scan_newsclaw_orphans_with_timing(
    context: &str,
    total_started: Instant,
) -> Vec<u32> {
    let scan_started = Instant::now();
    let killed = kill_newsclaw_orphans();
    log_to_file(&format!(
...
pub(crate) fn run_tray_quit_cleanup(app: tauri::AppHandle) {
    ...
    scan_newsclaw_orphans_with_timing("tray-cleanup", quit_started);
    ...
    let still_pid = list_service_pids()
        ...
        .collect::<Vec<_>>();
    let still_orphans = scan_newsclaw_orphans_with_timing("tray-verify", quit_started);
...
pub(crate) fn cleanup_backends_on_run_event_exit() {
    ...
    scan_newsclaw_orphans_with_timing("run-event-exit", cleanup_started);
}

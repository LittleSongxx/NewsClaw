...
pub(crate) fn state_file_path() -> PathBuf {
    newsclaw_root_dir().join("state.json")
}

pub(crate) fn workspaces_dir() -> PathBuf {
    newsclaw_root_dir().join("workspaces")
}
...
pub(crate) fn is_pid_file_valid(data: &PidFileData) -> bool {
    ...
    if data.started_at == 0 {
        return is_newsclaw_process(data.pid);
    }
...
#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "camelCase")]
pub(crate) struct NewsClawProcess {
    pid: u32,
    cmd: String,
}

#[tauri::command]
pub(crate) fn newsclaw_list_processes() -> Vec<NewsClawProcess> {
    let mut out = Vec::new();
    #[cfg(windows)]
    {

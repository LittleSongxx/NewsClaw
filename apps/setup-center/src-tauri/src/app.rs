            ...
            if let Some(ref ws_id) = state.current_workspace_id {
                if backend_was_manually_stopped(ws_id) {
                    ...
                    if need_start {
                        ...
                        let venv_dir = newsclaw_root_dir()
                            .join("venv")
                            .to_string_lossy()
                            .to_string();
                        ...
                        std::thread::spawn(move || {
                            ...
                            if backend_was_manually_stopped(&ws_clone) {
                                ...
                                match newsclaw_service_start_impl(venv_dir, ws_clone) {
                                    Ok(status) => {
                                        log_to_file(&format!(
            ...
            // 60+ 分钟才能在 autostart.log 里看到下一次探测。
            {
                ...
                std::thread::spawn(move || {
                    ...
                    loop {
                        ...
                        let venv_dir = newsclaw_root_dir().join("venv");
                        let venv_dir_str = venv_dir.to_string_lossy().to_string();
                        if managed_python_seed_path().is_none()
                            && !legacy_venv_has_newsclaw_backend(&venv_dir_str)
                        {
                            consecutive_failures = 0;
                            continue;
                        }

...
const OPTIONAL_MODULES_MANIFEST: &str =
    include_str!("../../../../src/newsclaw/optional_modules.json");
...
pub(crate) fn get_root_dir_info() -> RootDirInfo {
    RootDirInfo {
        ...
        current_root: newsclaw_root_dir().to_string_lossy().to_string(),
...
pub(crate) fn set_custom_root_dir(
    ...
    let migrate_old_root: Option<PathBuf> = if migrate {
        let old_root = newsclaw_root_dir();
    ...
    if let Some(ref old_root) = migrate_old_root {
        if is_safe_newsclaw_data_root(old_root) {
            let dir_names = [
                "workspaces",
                "venv",
                "runtime",
...
pub(crate) fn check_backend_availability(venv_dir: String) -> BackendAvailability {
    ...
    let venv_ready = legacy_venv_has_newsclaw_backend(&venv_dir);
...
pub(crate) fn legacy_venv_has_newsclaw_backend(venv_dir: &str) -> bool {
    ...
    command.args([
        "-c",
        "import newsclaw; import newsclaw.main; import newsclaw.setup_center.bridge",
    ]);
    command
        .output()
...
pub(crate) fn factory_reset() -> Result<String, String> {
    ...
    let stopped = newsclaw_stop_all_processes();
...
mod tests {
    ...
    fn test_data_root_rejects_drive_or_filesystem_root() {
        ...
        assert!(!is_safe_newsclaw_data_root(&root));
        assert!(ensure_safe_newsclaw_data_root(&root).is_err());
    }

    #[test]
    fn test_data_root_rejects_home_directory() {
        if let Some(home) = home_dir() {
            assert!(!is_safe_newsclaw_data_root(&home));
            assert!(ensure_safe_newsclaw_data_root(&home).is_err());
        }
    }

...
/// `newsclaw_service_start` 的进程级互斥窗口（毫秒）。
...
pub(crate) const NEWSCLAW_ROOT_MARKER: &str = ".newsclaw-root";
pub(crate) const EXTERNAL_BACKEND_DEV_ENV: &str = "NEWSCLAW_EXTERNAL_BACKEND_DEV";
...
pub(crate) fn restart_marker_path() -> PathBuf {
    let base = crate::config::default_root_dir();

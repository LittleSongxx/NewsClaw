...
pub(crate) fn runtime_root_dir() -> PathBuf {
    newsclaw_root_dir().join("runtime")
}
...
pub(crate) const CANONICAL_BACKEND_ENTRYPOINT: &str =
    "from newsclaw.main import app as newsclaw_app; newsclaw_app()";
...
pub(crate) fn resolve_runtime_pip_index() -> RuntimePipIndex {
    if let Ok(url) = std::env::var("NEWSCLAW_PIP_INDEX_URL") {
...
pub(crate) fn apply_runtime_env_builder(
    ...
    cmd.env("NEWSCLAW_RUNTIME_ROOT", runtime_root_dir());
    cmd.env("NEWSCLAW_BOOTSTRAP_DIR", bootstrap_resource_dir());
    cmd.env("NEWSCLAW_ENV_PURPOSE", purpose.as_str());
    cmd.env("NEWSCLAW_ENV_TRUST_SOURCE", "host-runtime");
    ...
    cmd.env("NEWSCLAW_SUBPROCESS_SECRET_SCRUB", "1");
}
...
for mod_name in ("newsclaw", "yaml", "pydantic", "pydantic_core", "certifi"):
    try:
...
pub(crate) fn ensure_app_venv(
    ...
    cmd.arg("certifi");
    cmd.args(["--reinstall-package", "newsclaw"]);
...
/// 返回 path-separated 的 site-packages 目录列表，用于 NEWSCLAW_MODULE_PATHS 环境变量
...
#[tauri::command]
pub(crate) fn newsclaw_runtime_last_error() -> RuntimeLastError {
...
#[tauri::command]
pub(crate) fn newsclaw_open_runtime_root() -> Result<OpenedRuntimePath, String> {
...
mod tests {
    ...
    fn backend_entrypoint_imports_one_canonical_main_module() {
        ...
        assert_eq!(args[0], "-u");
        assert_eq!(args[1], "-c");
        assert!(args[2].contains("from newsclaw.main import app"));

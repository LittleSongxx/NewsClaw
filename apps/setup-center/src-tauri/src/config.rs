...
pub(crate) struct PlatformInfo {
    ...
    arch: String,
    home_dir: String,
    newsclaw_root_dir: String,
}

/// 计算"未配置 custom_root 时的"默认 NewsClaw 数据目录字符串。
///
/// 注意：日常显示请用 [`newsclaw_root_dir`] 取真实 root，否则会和后端
/// 实际写入位置不一致；此函数仅作为兜底/迁移场景的"默认值"语义保留。
#[allow(dead_code)]
pub(crate) fn default_newsclaw_root() -> String {
    default_root_dir().to_string_lossy().to_string()
}
...
pub(crate) fn get_platform_info() -> PlatformInfo {
    ...
    PlatformInfo {
        ...
        newsclaw_root_dir: newsclaw_root_dir().to_string_lossy().to_string(),
    }
}
...
pub(crate) fn is_safe_newsclaw_data_root(path: &Path) -> bool {
...
pub(crate) fn ensure_safe_newsclaw_data_root(path: &Path) -> Result<(), String> {
...
pub(crate) fn write_root_marker(root: &Path) -> Result<(), String> {
    ...
    fs::write(
        root.join(NEWSCLAW_ROOT_MARKER),
        b"NewsClaw data root\nDo not delete this file unless you no longer use this directory for NewsClaw.\n",
    )
...
pub(crate) fn newsclaw_root_dir() -> PathBuf {
    ...
    if let Some(ref custom) = config.custom_root {
        if !custom.is_empty() {
            ...
            if !is_safe_newsclaw_data_root(&p) {
                eprintln!(
...
pub(crate) fn crashdumps_dir() -> PathBuf {
    newsclaw_root_dir().join("crashdumps")
}
...
#[tauri::command]
pub(crate) fn newsclaw_desktop_session_token() -> String {
...
pub(crate) fn save_log_export(filename: String, content: String) -> Result<String, String> {
    let downloads = dirs_next::download_dir()
        ...
        .unwrap_or_else(|| newsclaw_root_dir().join("logs"));
...
pub(crate) fn modules_dir() -> PathBuf {
    newsclaw_root_dir().join("modules")
}

...
//   2. 通过 window.__NEWSCLAW_BACKEND_READY 暴露状态（组件可以读它做 useQuery
//      的 enabled 依赖），并 dispatch `newsclaw_backend_ready` 事件，方便外部
...
declare global {
  interface Window {
    __NEWSCLAW_BACKEND_READY?: boolean;
  }
}

window.__NEWSCLAW_BACKEND_READY = false;

waitForBackend().then((ok) => {
  window.__NEWSCLAW_BACKEND_READY = ok;
  window.dispatchEvent(
    new CustomEvent("newsclaw_backend_ready", { detail: { ok } }),
  );
  if (!ok) {

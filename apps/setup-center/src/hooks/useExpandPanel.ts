// useExpandPanel — listen for `newsclaw:expand-panel` CustomEvents and bring
...
//   2. ConfigHintCard dispatches `newsclaw:navigate-view` to switch to the
//      settings view, then `newsclaw:expand-panel` after a short delay so the
 ...
 * ``newsclaw:expand-panel`` event fires with ``detail.anchor === targetAnchor``,
...
export function useExpandPanel(targetAnchor: string) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("newsclaw:expand-panel", handler as EventListener);
    return () => {
      window.removeEventListener("newsclaw:expand-panel", handler as EventListener);
    };
  }, [targetAnchor]);
...
export function dispatchExpandPanel(anchor: string, scrollTo?: string): void {
  ...
  window.dispatchEvent(
    new CustomEvent<ExpandPanelDetail>("newsclaw:expand-panel", { detail }),
  );
}

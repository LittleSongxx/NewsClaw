...
const NewsroomView = lazy(() => import("./views/NewsroomView").then(m => ({ default: m.NewsroomView })));
const WikiView = lazy(() => import("./views/WikiView").then(m => ({ default: m.WikiView })));
...
function MainApp() {
    ...
    invoke<any>("take_startup_recovery_notice")
      .then((payload) => {
        ...
        const message = reason === "native_frontend_crash"
          ? "OpenNewsClaw 刚刚从前端异常退出中恢复。后台服务和数据仍然保留，可以继续使用。"
          : "OpenNewsClaw 刚刚从桌面端崩溃中自动恢复。后台服务和数据仍然保留，可以继续使用。";
  ...
  const [storeVisible, setStoreVisible] = useState(() => localStorage.getItem("newsclaw_storeVisible") === "true");
  ...
  const [apiBaseUrl, setApiBaseUrl] = useState(() =>
    ...
    : IS_WEB ? ""
    : (localStorage.getItem("newsclaw_apiBaseUrl") || DEFAULT_LOCAL_API_BASE),
  );
  ...
  async function refreshAll() {
    if (IS_TAURI) {
      ...
      try {
        ...
        setInfo({ os: "web", arch: "", homeDir: "", newsclawRootDir: infoPath });
      } catch {
        // Backend not reachable yet (auth pending, first load) — use safe defaults
        setInfo({ os: "web", arch: "", homeDir: "", newsclawRootDir: "" });
  ...
  const handleAppResumed = useCallback(() => {
    ...
    lastResumeRef.current = now;
        ...
        setTimeout(() => { visibilityGraceRef.current = false; }, 10000);
        reconnectWsNow();
        window.dispatchEvent(new Event("newsclaw_app_resumed"));
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      ...
      try {
        ...
        if (IS_TAURI && dataMode !== "remote") {
          try {
            const alive = await invoke<boolean>("newsclaw_check_pid_alive", { workspaceId: currentWorkspaceId });
            if (alive) {
  ...
  const venvDir = useMemo(() => {
    if (!info) return "";
    return joinPath(info.newsclawRootDir, "venv");
  }, [info]);
        ...
        try {
          ...
          try {
            const ss = await invoke<ServiceStatus>(
              "newsclaw_service_start", { venvDir, workspaceId: opts.targetId },
            );
            setServiceStatus(ss);
          } catch (e) {
            setRestartOverlay({ phase: "fail" });
  ...
  async function doLoadProviders() {
    ...
    try {
      ...
      if (shouldUseHttpApi()) {
        ...
        try {
          const raw = await invoke<string>("newsclaw_list_providers", { venvDir });
  ...
  async function restartService(): Promise<void> {
    ...
    try {
      ...
      if (IS_TAURI && wsId && dataMode === "local") {
        try {
          const ss = await invoke<ServiceStatus>("newsclaw_service_status", { workspaceId: wsId });
      ...
      if (IS_TAURI && wsId && venvDir && dataMode === "local" && tauriManagedBackend) {
        ...
        try {
          await invoke("newsclaw_service_stop", { workspaceId: wsId });
        } catch { /* PID 文件可能不存在 */ }
        ...
        try {
          const ss = await invoke<ServiceStatus>(
            "newsclaw_service_start", { venvDir, workspaceId: wsId },
          );
          setServiceStatus(ss);
        } catch (e) {
          setRestartOverlay({ phase: "fail" });
  ...
  async function refreshStatus(overrideDataMode?: "local" | "remote", overrideApiBaseUrl?: string, forceAliveCheck?: boolean) {
    ...
    try {
      ...
      if (useHttpApi) {
        ...
        if (effectiveDataMode !== "remote" && currentWorkspaceId) {
          try {
            const ss = await invoke<ServiceStatus>("newsclaw_service_status", { workspaceId: currentWorkspaceId });
            setServiceStatus((prev) => ({
      ...
      if (effectiveDataMode !== "remote") {
        try {
          const ss = await invoke<ServiceStatus>("newsclaw_service_status", {
            workspaceId: currentWorkspaceId,
          });
          setServiceStatus(ss);
      ...
      if (effectiveDataMode !== "remote") {
        try {
          const procs = await invoke<Array<{ pid: number; cmd: string }>>("newsclaw_list_processes");
  ...
  async function doStartLocalService(effectiveWsId: string) {
    ...
    try {
      ...
      const ss = await invoke<ServiceStatus>("newsclaw_service_start", {
        venvDir,
        workspaceId: effectiveWsId,
      });
      setServiceStatus(ss);
      const ready = await waitForServiceReady("http://127.0.0.1:18900", LOCAL_SERVICE_READY_TIMEOUT_MS);
      const real = await invoke<ServiceStatus>("newsclaw_service_status", {
        workspaceId: effectiveWsId,
      });
  ...
  async function doStopService(wsId?: string | null) {
    ...
    try {
      const ss = await invoke<ServiceStatus>("newsclaw_service_stop", { workspaceId: id });
      setServiceStatus(ss);
    ...
    // Final status
    try {
      const final_ss = await invoke<ServiceStatus>("newsclaw_service_status", { workspaceId: id });
  ...
  async function refreshServiceLog(workspaceId: string) {
    try {
      ...
      if (shouldUseHttpApi()) {
        ...
        chunk = await invoke<{ path: string; content: string; truncated: boolean }>("newsclaw_service_log", {
          workspaceId,
          tailBytes: 60000,
        });
      }
    ...
    try {
      ...
      if (!shouldUseHttpApi() && skillsList.length === 0 && currentWorkspaceId) {
        try {
          ...
          logger.warn("App", "newsclaw_list_skills via Tauri failed", { error: String(e) });
        }
      }
  ...
  async function obRunSetup() {
    ...
    try {
      ...
      try {
        const effectiveVenv = venvDir || (info ? joinPath(info.newsclawRootDir, "venv") : "");
        ...
        if (!backendInfo.bundled && !backendInfo.venvReady) {
          ...
          if (pythonCandidate) {
            ...
            updateTask("backend-check", { detail: "安装 newsclaw..." });
            logTask("检查后端环境", "running", "安装 newsclaw...");
            setInstallProgress({ stage: "安装 newsclaw...", percent: 0 });
            try {
              await invoke<string>("pip_install", {
                venvDir: effectiveVenv,
                packageSpec: "newsclaw",
                installId,
              });
            } finally {
      ...
      try {
        ...
        if (earlyProbe) {
          ...
          if (backendStartInFlight) {
            ...
            const ss = await invoke<ServiceStatus>("newsclaw_service_start", { venvDir: effectiveVenv, workspaceId: activeWsId });
            setServiceStatus(ss);
        ...
        return (
          <div className="obPage">
            <div className="flex flex-col items-center text-center max-w-[520px] gap-5">
              <img src={logoUrl} alt="OpenNewsClaw" className="w-20 h-20 rounded-2xl shadow-lg mb-1" />
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              // joinPath(info.newsclawRootDir, "venv") / "runtime"
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              // PlatformInfo，否则 info.newsclawRootDir 仍是旧 root。
                              try {
                                const plat = await invoke<PlatformInfo>("get_platform_info");
                                setInfo(plat);
                              } catch { /* ignore */ }
                  ...
                  if (IS_TAURI && earlyStartWsId) {
                    ...
                    (async () => {
                      try {
                        ...
                        setObBackendStartupPhase("starting", t("onboarding.backendStartup.starting"));
                        const ss = await invoke<ServiceStatus>("newsclaw_service_start", { venvDir: effectiveVenv, workspaceId: wsId });
                        setServiceStatus(ss);
                        setObBackendStartupPhase("waiting", t("onboarding.backendStartup.waiting"));
  ...
  function renderStepContent() {
    ...
    if (view === "newsroom") {
      return <NewsroomView serviceRunning={serviceStatus?.running ?? false} apiBaseUrl={apiBaseUrl} />;
    }
    if (view === "wiki") {
      return <WikiView serviceRunning={serviceStatus?.running ?? false} apiBaseUrl={apiBaseUrl} />;
    }
    if (view === "memory") {
                ...
                <button className="btnPrimary" disabled={!!busy} onClick={async () => {
                  ...
                  try {
                    ...
                    if (data.status === "ok") {
                      ...
                      if (!authOk) {
                        setApiBaseUrl(url);
                        localStorage.setItem("newsclaw_apiBaseUrl", url);

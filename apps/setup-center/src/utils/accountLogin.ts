...
export function connectNewsClawAccount(
  apiBaseUrl: string,
  options: AccountLoginOptions = {},
): Promise<AccountStatusSummary> {
  if (activeLogin) {
    if (activeLoginBase === apiBaseUrl) return activeLogin;
    return activeLogin.catch(() => undefined).then(() => connectNewsClawAccount(apiBaseUrl, options));
  }
  if (IS_CAPACITOR && nativeRecovery) {
    ...
    return nativeRecovery.then(snapshot => snapshot && recoveryBase === apiBaseUrl
      ? snapshot : connectNewsClawAccount(apiBaseUrl, options));
  }
...
export async function refreshNewsClawAccountEntitlements(
  apiBaseUrl: string,
): Promise<AccountStatusSummary> {
  const generation = accountGeneration;
...
export async function disconnectNewsClawAccount(
  apiBaseUrl: string,
): Promise<AccountStatusSummary> {
  const generation = ++accountGeneration;
  activeLogin = null;

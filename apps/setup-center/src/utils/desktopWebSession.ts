import { DESKTOP_TOKEN_HEADER } from '../constants';
...
export async function openDesktopWebSession(webUrl: string) {
  ...
  const token = await invoke<string>('newsclaw_account_session_token');
  const response = await safeFetchResponse(base + '/api/auth/desktop-web-session', {
    method: 'POST', headers: { [DESKTOP_TOKEN_HEADER]: token },
  });
  ...
  if (destination.origin !== base || destination.pathname !== '/web/' ||
    !destination.hash.startsWith('#newsclaw-web-session=')) throw new Error('desktop_web_session_failed');
  await openExternalUrl(destination.href);
}

export async function consumeDesktopWebSession() {
  if (!location.hash.startsWith('#newsclaw-web-session=')) return;
  const params = new URLSearchParams(location.hash.slice(1));
  const ticket = params.get('newsclaw-web-session');
  history.replaceState(history.state, '', location.pathname + location.search);
  if (params.getAll('newsclaw-web-session').length !== 1 || !ticket) throw new Error('desktop_web_session_failed');

...
import {
  connectNewsClawAccount,
  disconnectNewsClawAccount,
  loadAccountCapability,
  refreshNewsClawAccountEntitlements,
...
describe("account login flow", () => {
  ...
  it('uses native authentication on Capacitor without the device-code dialog', async () => {
    ...
    const snapshot = await connectNewsClawAccount('https://original.example',{onDevicePreparing,onNativePreparing});
  ...
  it('does not publish a native login result after the current server changes', async () => {
    ...
    try {
      await expect(connectNewsClawAccount('https://original.example',{signal:abort.signal})).rejects.toThrow('account_login_cancelled');
  ...
  it("opens OAuth, waits for completion, and publishes the signed-in snapshot", async () => {
    ...
    window.addEventListener(ACCOUNT_STATUS_CHANGED_EVENT, listener);
    try {
      const snapshot = await connectNewsClawAccount("http://localhost:18900", {
        pollIntervalMs: 0,
      });
  ...
  it("surfaces a failed OAuth attempt", async () => {
    ...
    await expect(connectNewsClawAccount("http://localhost:18900", {
      pollIntervalMs: 0,
  ...
  it("reuses an in-progress login instead of opening OAuth twice", async () => {
    ...
    const first = connectNewsClawAccount("http://localhost:18900", { pollIntervalMs: 0 });
    const second = connectNewsClawAccount("http://localhost:18900", { pollIntervalMs: 0 });
  ...
  it("refreshes entitlements and publishes the latest status", async () => {
    ...
    window.addEventListener(ACCOUNT_STATUS_CHANGED_EVENT, listener);
    try {
      const snapshot = await refreshNewsClawAccountEntitlements("http://localhost:18900");
  ...
  it("signs out locally without opening a browser", async () => {
    ...
    const snapshot = await disconnectNewsClawAccount("http://localhost:18900");
  ...
  it("prepares the code without opening a tab and only opens on explicit continue", async () => {
    ...
    const onDevicePreparing = vi.fn();
    const operation = connectNewsClawAccount("https://akita.example", { pollIntervalMs: 30_000, onDevicePreparing, onDeviceAuthorization:p=>{prompt=p;} });
  ...
  it("lets the user retry a blocked popup without creating a second grant", async () => {
    ...
    vi.mocked(safeFetch)
      ...
      .mockResolvedValueOnce(response({status:"active"}));
    let prompt!: DeviceAuthorizationPrompt;
    const operation = connectNewsClawAccount("https://akita.example",{pollIntervalMs:30_000,onDeviceAuthorization:p=>{prompt=p;}});
    await vi.advanceTimersByTimeAsync(0);
  ...
  it("does not open a blank tab when the account service does not support device authorization", async () => {
    ...
    await expect(connectNewsClawAccount("https://akita.example")).rejects.toThrow("account_device_not_supported");
  ...
  it("refuses to navigate an expired preview and cancels without opening a tab", async () => {
    ...
    let prompt!: DeviceAuthorizationPrompt;
    const operation = connectNewsClawAccount("https://akita.example",{signal:abort.signal,pollIntervalMs:30_000,onDeviceAuthorization:p=>{prompt=p;}});
  ...
  it("bounds polling even when the backend never reports expiry", async () => {
    vi.mocked(safeFetch)
      ...
      .mockResolvedValueOnce(response({ status: "cancelled" }));
    await expect(connectNewsClawAccount("http://localhost:18900")).rejects.toThrow("account_login_expired");
    expect(safeFetch).toHaveBeenCalledTimes(2);
  });
  ...
  it("publishes a login that finished before cancellation reached the backend", async () => {
    ...
    vi.mocked(safeFetch)
      ...
      .mockResolvedValueOnce(response({ status: "complete" }))
      .mockResolvedValueOnce(response({ status: "active" }));
    const result = await connectNewsClawAccount("http://localhost:18900", {
  ...
  it.each(["visibilitychange", "focus", "pageshow"])("checks immediately on %s and removes wake listeners after completion", async event => {
    ...
    vi.mocked(safeFetch)
      ...
      .mockResolvedValueOnce(response({status:"complete"}))
      .mockResolvedValueOnce(response({status:"active"}));
    const operation = connectNewsClawAccount("https://akita.example", {pollIntervalMs:30_000});
    await vi.advanceTimersByTimeAsync(0);
    expect(safeFetch).toHaveBeenCalledTimes(1);
  ...
  it("wakes a pending wait immediately on cancellation", async () => {
    ...
    vi.mocked(safeFetch)
      ...
      .mockResolvedValueOnce(response({status:"cancelled"}));
    const operation = connectNewsClawAccount("https://akita.example", {pollIntervalMs:30_000, signal:abort.signal});
  ...
  it.each(["login", "refresh"])("discards a delayed %s snapshot after logout", async (kind) => {
    ...
    try {
      const old = kind === "login"
        ? connectNewsClawAccount("http://localhost:18900", { pollIntervalMs: 0 })
        : refreshNewsClawAccountEntitlements("http://localhost:18900");
      ...
      await disconnectNewsClawAccount("http://localhost:18900");

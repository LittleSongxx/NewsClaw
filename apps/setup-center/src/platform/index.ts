...
export async function onDeepLinkOpen(handler: (urls: string[]) => void): Promise<() => void> {
  ...
  const [{ onOpenUrl }, unlistenForwarded] = await Promise.all([
    ...
    listen<string>("newsclaw-deep-link", (event) => handler([event.payload])),
  ]);
...
export async function checkForUpdate(options?: {
  ...
  if (options?.channel) headers.set("X-NewsClaw-Channel", options.channel);
...
async function buildUpdaterHeaders(apiBaseUrl?: string): Promise<Headers> {
  ...
  try {
    ...
    if (typeof data.channel === "string" && data.channel) {
      headers.set("X-NewsClaw-Channel", data.channel);
    }
  } catch {

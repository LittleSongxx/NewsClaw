...
export function initTheme() {
  const pref = (localStorage.getItem("newsclaw-theme-pref") as Theme) || "system";
  applyTheme(pref);

  const mediaQuery = window.matchMedia("(prefers-color-scheme: dark)");
  const listener = (e: MediaQueryListEvent) => {
    const currentPref = localStorage.getItem("newsclaw-theme-pref") as Theme | null;
...
export function setThemePref(theme: Theme) {
  _previewBackup = null;
  localStorage.setItem("newsclaw-theme-pref", theme);
  applyTheme(theme);
  window.dispatchEvent(new CustomEvent(THEME_CHANGE_EVENT, { detail: theme }));
}

export function getThemePref(): Theme {
  return (localStorage.getItem("newsclaw-theme-pref") as Theme) || "system";
}

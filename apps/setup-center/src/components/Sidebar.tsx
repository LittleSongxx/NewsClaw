...
import {
  ...
  IconZap, IconCalendar, IconNewspaper,
  IconBug, IconBrain, IconUsers, IconBot,
  IconGear, IconBook, IconPuzzle, IconLayoutGrid,
  IconRadar, IconRefresh, IconHelp,
  IconAlertCircle,
} from "../icons";
...
const CAPABILITY_VIEWS: ViewId[] = ["skills", "wiki", "memory", "scheduler", "newsroom"];
const MONITOR_VIEWS: ViewId[] = ["token_stats"];
...
export function Sidebar({
  ...
  const accountUsesNewsClawBrand = accountCapability?.provider === "newsclaw";
  ...
  const startAccountLogin = useCallback(async () => {
    ...
    try {
      const snapshot = await connectNewsClawAccount(httpApiBase, {
  ...
  const refreshAccountEntitlements = useCallback(async () => {
    ...
    try {
      const snapshot = await refreshNewsClawAccountEntitlements(httpApiBase);
      setAccountSnapshot(snapshot);
  ...
  const logoutAccount = useCallback(async () => {
    ...
    try {
      const snapshot = await disconnectNewsClawAccount(httpApiBase);
      setAccountSnapshot(snapshot);
      setAccountLoginError(null);
    ...
    <aside className={`sidebar ${collapsed ? "sidebarCollapsed" : ""}${configMode ? " sidebarConfigMode" : ""}${mobileOpen ? " sidebarOpen" : ""}`}>
      ...
      {configMode ? (
        ...
        {(collapsed || capExpanded) && (
          <div className="navGroupItems">
            ...
            <div className={`navItem ${view === "newsroom" ? "navItemActive" : ""}`} onClick={() => onViewChange("newsroom")} role="button" tabIndex={0} title={t("sidebar.newsroom")}>
              <IconNewspaper size={16} /> {!collapsed && <span>{t("sidebar.newsroom")}</span>}
            </div>
            <div className={`navItem ${view === "wiki" ? "navItemActive" : ""}`} onClick={() => onViewChange("wiki")} role="button" tabIndex={0} title={t("sidebar.wiki")}>
              <IconBook size={16} /> {!collapsed && <span>{t("sidebar.wiki")}</span>}
            </div>
          </div>
        )}
      ...
      <div className="sidebarAccountArea" ref={accountAreaRef}>
        ...
        >
          ...
          ) : accountSignedIn && accountUsesNewsClawBrand ? (

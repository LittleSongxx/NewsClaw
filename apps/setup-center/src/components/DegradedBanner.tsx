...
async function getDesktopToken(): Promise<string | null> {
  ...
  try {
    return await invoke<string>("newsclaw_desktop_session_token");
  } catch {
    return null;
  }
}
...
const DegradedRepairDialog: React.FC<DialogProps> = ({ apiBase, canAutoRestart, onClose }) => {
  ...
  const quarantine = async (subsystem: string) => {
    ...
    try {
      ...
      // session token whenever ``NEWSCLAW_DESKTOP_SESSION_TOKEN`` is set in
      ...
      >
        ...
        <p style={{ fontSize: 13, color: "#374151", margin: "8px 0 16px" }}>
          {t("degraded.dialogDescription", {
            defaultValue:
              "OpenNewsClaw detected one or more subsystems unable to open their database. You can quarantine each one (rename the file, recreate empty on next boot) — no other data is touched.",
          })}
        </p>
            ...
            <div style={{ marginTop: 6 }}>
              {canAutoRestart
                ? t("degraded.restartTauri", {
                    defaultValue:
                      "OpenNewsClaw will stop the backend; the desktop app will respawn it automatically.",
                  })

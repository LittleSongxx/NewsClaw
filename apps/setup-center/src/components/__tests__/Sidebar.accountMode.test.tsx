...
vi.mock("../../utils/accountLogin", () => ({
  ...
  connectNewsClawAccount: vi.fn(),
  disconnectNewsClawAccount: vi.fn(),
  loadAccountCapability: vi.fn(),
  refreshNewsClawAccountEntitlements: vi.fn(),
...
describe("Sidebar account distribution mode", () => {
  ...
  it("closes mobile navigation during device preparation and keeps login alive until cancelled", async () => {
    vi.mocked(loadAccountCapability).mockResolvedValue({
      enabled: true, mode: "newsclaw", provider: "newsclaw",
      display_name: "NewsClaw", supports_entitlements: true,
    });
    let loginOptions: Parameters<typeof connectNewsClawAccount>[1];
    vi.mocked(connectNewsClawAccount).mockImplementation((_base, options = {}) => {

...
export function StatusView(props: StatusViewProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        logger.warn("newsclaw_runtime_last_error failed", String(e));
      }
    })();
    return () => {
      cancelled = true;
  ...
  // NewsClaw fork：状态页仅展示保留通道（飞书 / QQ / 微信）。
  // 其余通道的后端适配器仍在，只是不再出现在状态面板。
  const im = [
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-5 px-6 py-5">
      ...
      {/* Banner: RUNTIME_PERMISSION_DENIED — 企业 AD / 杀软"勒索软件防护"拦截
          ...
          newsclaw_open_runtime_root 命令——它在目录尚未创建时会自动回退到
        ...
        {IS_TAURI && detectedProcesses.length > 1 && (
          <div className="statusPanelAlert">
            ...
            <Button size="sm" variant="destructive" style={{ marginLeft: "auto" }} onClick={async () => {
              ...
              try {
                const stopped = await invoke<number[]>("newsclaw_stop_all_processes");
...
function MemoryRepairDialog({
  ...
  const executeAction = async () => {
    ...
    try {
      ...
      if (IS_TAURI) {
        headers[DESKTOP_TOKEN_HEADER] =
          await invoke<string>("newsclaw_desktop_session_token");
      }
      const body =
        action === "restore"

...
import {
  ...
  IconHourglass, IconTarget, IconCheckCircle, IconPlug, IconClock, IconGlobe,
  IconNewspaper, IconBook,
...
export function ChatView({
  ...
  const STORAGE_KEY_DATA_EPOCH = `newsclaw_data_epoch_${wsTag}`;
  ...
  useEffect(() => {
    ...
    window.addEventListener("newsclaw_app_resumed", handler);
    return () => window.removeEventListener("newsclaw_app_resumed", handler);
  }, []);
  ...
  useEffect(() => {
    ...
    void restoreSessionsUntilReady<Record<string, unknown>>({
      ...
      reconcile: async (data) => {
        ...
        try { localStorage.removeItem("newsclaw_data_epoch"); } catch { /* ignore */ }
        ...
        <div ref={scrollContainerRef} role="log" aria-live="polite" aria-label={t("chat.messageList", "消息列表")} className="flex min-h-0 flex-1 flex-col overflow-hidden px-5 py-4">
          ...
          <MessageList
            ...
            onCompletionAction={(msg, action) => {
              ...
              onOpenFeedback?.({
                mode: "bug",
                title: t("chat.diagnosticFeedbackPrefillTitle", "OpenNewsClaw 运行日志诊断反馈"),
                description: t(

...
export function SchedulerView({ serviceRunning, apiBaseUrl = "" }: { serviceRunning: boolean; apiBaseUrl?: string }) {
  ...
  // 任务 prompt 预览的展开态（截断预览 + 按需展开，见下方渲染）
            ...
            {filteredTasks.map(task => (
              ...
              <Card className={cn(
                ...
                <CardContent className="px-4 py-3.5">
                  ...
                  {/* Content preview */}
                  {(() => {
                    const previewText = task.reminder_message || task.prompt;
                    if (!previewText) return null;
                    const expanded = !!expandedPrompts[task.id];
                    // 长文本折叠：默认 3 行 line-clamp（不再用固定 max-h 硬裁——
                    // 硬裁会在行中间截断，看起来像组件错位）。短文本不显示开关。
                    const isLong = previewText.length > 120 || previewText.split("\n").length > 3;
                    return (
                      <div className="mt-2.5 rounded-md bg-muted/30 border border-border/50 overflow-hidden">
                        <div
                          className={`px-3 py-1.5 text-xs leading-5 text-foreground whitespace-pre-wrap break-words ${
                            expanded ? "max-h-64 overflow-y-auto custom-scrollbar" : "line-clamp-3"
                          }`}
                        >
                          {previewText}
                        </div>
                        {isLong && (
                          <button
                            type="button"
                            onClick={() => setExpandedPrompts((prev) => ({ ...prev, [task.id]: !expanded }))}
                            className="w-full border-t border-border/50 py-1 text-[11px] text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors"
                          >
                            {expanded ? t("common.collapse", "收起") : t("common.expand", "展开全部")}
                          </button>
                        )}
                      </div>
                    );

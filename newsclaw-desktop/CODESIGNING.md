...
本文档详细说明如何创建、导出和配置 macOS **Developer ID Application** 证书，用于 NewsClaw Desktop 的签名与公证。
...
```bash
# 验证代码签名
codesign --verify --deep --strict release/mac-arm64/NewsClaw.app

# 验证 Gatekeeper 公证状态
spctl --assess --type execute --verbose release/mac-arm64/NewsClaw.app
# 期望输出: accepted, source=Developer ID

# 验证 DMG
spctl --assess --type open --context context:primary-signature release/NewsClaw-*.dmg
```
...
# 2. 手动公证
xcrun notarytool submit release/NewsClaw_1.0.0_arm64.zip \
  --apple-id "your@apple.id" \
  --password "app专用密码" \
  --team-id "XXXXXXXXXX" \
  --wait

xcrun notarytool submit release/NewsClaw_1.0.0_x64.zip \
  --apple-id "your@apple.id" \
  --password "app专用密码" \
  --team-id "XXXXXXXXXX" \
  --wait
...
# 3. 装订公证票据到 DMG
xcrun stapler staple release/NewsClaw_1.0.0_arm64.dmg
xcrun stapler staple release/NewsClaw_1.0.0_x64.dmg
```

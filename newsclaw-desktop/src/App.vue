...
function handleDownloadUpdate() {
  window.newsClawAPI?.downloadUpdate()
}

function handleInstallUpdate() {
  window.newsClawAPI?.installUpdate()
}
...
function navigateToApp() {
  if (window.newsClawAPI?.navigateToApp) {
    window.newsClawAPI.navigateToApp()
  }
}
...
async function handleBackendReady() {
  if (connectionMode.value === 'remote') {
    ...
    if (window.newsClawAPI) {
      BACKEND_URL = await window.newsClawAPI.getBackendUrl()
    }
    status.value = 'ready'
...
async function chooseLocal() {
  ...
  await window.newsClawAPI?.useLocalConnection()
  if (window.newsClawAPI) {
    BACKEND_URL = await window.newsClawAPI.getBackendUrl()
  }
}
...
async function testRemote() {
  if (!remoteUrlInput.value.trim() || !window.newsClawAPI) return
  testing.value = true
  testResult.value = null
  try {
    const r = await window.newsClawAPI.testConnection(remoteUrlInput.value)
...
async function connectRemote(url?: string) {
  ...
  if (!target || !window.newsClawAPI) return
  remoteUrlInput.value = target
  const r = await window.newsClawAPI.useRemoteConnection(target)
  if (!r.ok) {
...
onMounted(async () => {
  ...
  if (window.newsClawAPI) {
    ...
    unsubStatus = window.newsClawAPI.onBackendStatus((s: string) => {
      if (s === 'choose') {
    ...
    unsubCrashed = window.newsClawAPI.onBackendCrashed((msg: string) => {
      status.value = 'crashed'
    ...
    try {
      const cfg = await window.newsClawAPI.getConnectionConfig()
      ...
      if (buildMode.value === 'remote') {
        if (cfg.forceChoose || !cfg.mode || cfg.mode === 'local') {
          ...
          if (await window.newsClawAPI.isBackendReady()) {
            handleBackendReady()
          }
        }
    ...
    unsubUpdater = window.newsClawAPI.onUpdaterState((state: UpdaterState) => {
      updater.value = state
    })
    window.newsClawAPI.getUpdaterState().then((state) => {
...
function handleRestart() {
  ...
  errorMessage.value = ''
  window.newsClawAPI?.restartBackend()
}
    ...
    <div class="splash">
      ...
      <div class="logo-section fade-enter">
        <div class="logo-wrap">
          ...
          <img class="logo-img" src="/logo/newsclaw_logo_s.png" alt="NewsClaw" />
        </div>
        <div class="brand-name">
          <span class="mate">News</span><span class="claw">Claw</span>
        </div>
        <div class="brand-tagline">AI News Operations Agent</div>
      </div>

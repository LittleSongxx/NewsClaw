...
NewsClaw 的桌面客户端，基于 Electron 构建，自动集成 JRE 21 和后端服务，实现双击即用。
...
├── newsclaw-server.jar       ← Spring Boot 后端 + Vue 前端
...
应用内置 `electron-updater` 自动升级，更新产物托管在 [GitHub Releases](https://github.com/LittleSongxx/NewsClaw/releases)。
...
完成后前往 https://github.com/LittleSongxx/NewsClaw/releases ，找到 Draft Release：
...
ls -la release/
# 产物包括：
#   NewsClaw_1.1.0_arm64.dmg           macOS ARM64 安装包
#   NewsClaw_1.1.0_x64.dmg             macOS x64 安装包
#   NewsClaw_1.1.0_arm64.zip           macOS ARM64 更新包（升级用）
#   NewsClaw_1.1.0_x64.zip             macOS x64 更新包（升级用）
#   NewsClaw_1.1.0_x64_Setup.exe       Windows x64 安装包
#   NewsClaw_1.1.0_arm64_Setup.exe     Windows ARM64 安装包
#   NewsClaw_1.1.0_*.blockmap          差分下载支持文件
...
# 2. 手动公证（支持断点续传）
xcrun notarytool submit release/NewsClaw_*.zip \
...
# 3. 装订公证票据
xcrun stapler staple release/NewsClaw_*.dmg
```
...
```bash
# 1. 在 newsclaw-desktop/ 根目录创建 dev-app-update.yml
...
# 4. 另一个终端启动开发模式
cd newsclaw-desktop && npm run dev

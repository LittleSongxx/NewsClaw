...
async function startJavaBackend(): Promise<void> {
  ...
  if (BUILD_MODE === 'remote') {
    console.log('[NewsClaw] Remote build — local backend unavailable, showing connection chooser')
  ...
  console.log(`[NewsClaw] Using dynamic port: ${BACKEND_PORT}`)

  const javaExec = getJavaExecutable()
  const jarPath = getJarPath()
  const workingDir = getUserDataPath()

  console.log(`[NewsClaw] Java executable: ${javaExec}`)
  console.log(`[NewsClaw] JAR path: ${jarPath}`)
  console.log(`[NewsClaw] Working directory: ${workingDir}`)

  if (!existsSync(javaExec)) {
    console.error(`[NewsClaw] Java executable not found: ${javaExec}`)
    dialog.showErrorBox(
      'NewsClaw 启动失败',
      `找不到 Java 运行时环境。\n路径: ${javaExec}\n\n请重新安装 NewsClaw。`
    )
  ...
  if (!existsSync(jarPath)) {
    console.error(`[NewsClaw] JAR not found: ${jarPath}`)
    dialog.showErrorBox(
      'NewsClaw 启动失败',
      `找不到应用程序包。\n路径: ${jarPath}\n\n请重新安装 NewsClaw。`
    )
  ...
  const env = {
    ...
    SPRING_DATASOURCE_URL: `jdbc:h2:file:${join(workingDir, 'data', 'newsclaw')};MODE=MySQL;DATABASE_TO_LOWER=TRUE;CASE_INSENSITIVE_IDENTIFIERS=TRUE`,
  }
  ...
  javaProcess = spawn(javaExec, [
    ...
    '--newsclaw.setup.await-language-selection=true',
  ], {
    cwd: workingDir,
    env,
  ...
  javaProcess.on('error', (err: Error) => {
    console.error('[NewsClaw] Failed to start Java process:', err)
    sendToWindow('backend:crashed', `Java 进程启动失败: ${err.message}`)
  })

  javaProcess.on('exit', (code: number | null, signal: string | null) => {
    console.log(`[NewsClaw] Java process exited: code=${code}, signal=${signal}`)
    javaProcess = null
...
function pollBackendReady(): void {
  ...
  const check = () => {
    ...
    if (elapsed > timeout) {
      console.error('[NewsClaw] Backend health check timed out')
    ...
    const onResponse = (res: IncomingMessage) => {
      ...
      backendReady = true
      console.log(`[NewsClaw] Backend ready (${elapsed}ms, status: ${res.statusCode})`)
...
async function stopJavaBackend(): Promise<void> {
  ...
  console.log('[NewsClaw] Stopping Java backend...')

  return new Promise<void>((resolve) => {
    const timeout = setTimeout(() => {
      console.log('[NewsClaw] Force killing Java process')
    ...
    javaProcess!.on('exit', () => {
      ...
      console.log('[NewsClaw] Java process stopped')
      resolve()
    })
...
function startRemoteConnection(url: string): void {
  ...
  console.log(`[NewsClaw] Remote mode → ${BACKEND_URL}`)
  pollBackendReady()
}
...
function setupAutoUpdater(): void {
  if (!app.isPackaged) {
    ...
    if (!existsSync(devUpdateConfig)) {
      console.log('[NewsClaw] Skipping auto-updater in dev mode (no dev-app-update.yml)')
      return
    }
    console.log('[NewsClaw] Dev mode: using dev-app-update.yml for updater')
  ...
  autoUpdater.on('checking-for-update', () => {
    ...
    console.log('[NewsClaw] Checking for update...')
  })
  ...
  autoUpdater.on('update-available', (info: UpdateInfo) => {
    ...
    sendToWindow('updater:state', updaterState)
    console.log(`[NewsClaw] Update available: ${info.version}`)
  })

  autoUpdater.on('update-not-available', (info: UpdateInfo) => {
    updaterState = { status: 'not-available', version: info.version }
    sendToWindow('updater:state', updaterState)
    console.log('[NewsClaw] No update available')
  })
  ...
  autoUpdater.on('update-downloaded', (info: UpdateInfo) => {
    ...
    sendToWindow('updater:state', updaterState)
    console.log(`[NewsClaw] Update downloaded: ${info.version}`)
  })

  autoUpdater.on('error', (err: Error) => {
    updaterState = { status: 'error', error: err.message }
    sendToWindow('updater:state', updaterState)
    console.error('[NewsClaw] Auto-updater error:', err.message)
  ...
  setTimeout(() => {
    autoUpdater.checkForUpdates().catch((err) => {
      console.error('[NewsClaw] Update check failed:', err.message)
    })
  }, 3000)
}
...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle('app:navigate-to-app', () => {
    if (mainWindow && !mainWindow.isDestroyed()) {
      console.log('[NewsClaw] Navigating to main application')
  ...
  ipcMain.handle('updater:check', async () => {
    ...
    try {
      ...
      console.error('[NewsClaw] Manual update check failed:', err.message)
    }
    return updaterState
  })
  ...
  ipcMain.handle('updater:download', async () => {
    ...
    try {
      ...
      console.error('[NewsClaw] Download failed:', err.message)
    }
  })
  ...
  ipcMain.handle('updater:install', async () => {
    ...
    console.log('[NewsClaw] Installing update, stopping backend first...')
    isUpdating = true

    try {
      await stopJavaBackend()
    } catch (err) {
      console.error('[NewsClaw] Error stopping backend before update:', err)
    }
...
function showAboutDialog(): void {
  ...
  dialog.showMessageBox({
    type: 'info',
    title: 'About NewsClaw',
    message: 'NewsClaw',
    detail: [
      `Version: ${app.getVersion()}`,
      '',
      'AI news operations agent powered by Spring AI Alibaba.',
      '',
      `Copyright © 2026 LittleSongxx`,
    ].join('\n'),
    buttons: ['OK'],
    icon,
  })
  ...
  try {
    ...
    if (!result || !result.updateInfo || result.updateInfo.version === app.getVersion()) {
      dialog.showMessageBox({
        ...
        detail: `NewsClaw ${app.getVersion()} is the latest version.`,
      })
    }
  ...
  template.push({
    ...
    submenu: [
      ...
      ] : []),
      {
        ...
        click: () => shell.openExternal('https://github.com/LittleSongxx/NewsClaw'),
      },
      {
        label: 'Report Issue',
        click: () => shell.openExternal('https://github.com/LittleSongxx/NewsClaw/issues'),
      },
      ...(!isMac ? [
        { type: 'separator' as const },
        { label: `About ${app.name}`, click: showAboutDialog },
...
app.on('before-quit', async (event) => {
  ...
  try {
    ...
    console.error('[NewsClaw] Error stopping backend:', err)
  } finally {
    app.exit(0)
  }
})

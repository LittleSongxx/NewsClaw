 ...
 * and replaces hardcoded "NewsClaw" strings in all built files — source code
 * stays untouched.
 *
...
function loadBrandConfig(rootDir) {
  ...
  return {
    name:           env.BRAND_NAME        || config.name        || 'NewsClaw',
    tagline:        env.BRAND_TAGLINE     || config.tagline     || 'AI News Operations Agent',
    team:           env.BRAND_TEAM        || config.team        || 'LittleSongxx',
    copyright:      env.BRAND_COPYRIGHT   || config.copyright   || 'Copyright © 2026 LittleSongxx',
    appId:          env.BRAND_APP_ID      || config.appId       || 'io.github.littlesongxx.newsclaw',
    githubUrl:      env.BRAND_GITHUB_URL  || config.githubUrl   || 'https://github.com/LittleSongxx/NewsClaw',
    logoFile:       env.BRAND_LOGO_FILE   || config.logoFile    || 'newsclaw_logo_s.png',
  }
}
 ...
 * partial matches (e.g. the copyright line before the team name).
 */
function buildReplacements(brand) {
  ...
  // 2. GitHub URLs (before the team-name replacement, because the owner
  //    name is part of the URL).
  replacements.push([
    'https://github.com/LittleSongxx/NewsClaw/issues',
    brand.githubUrl + '/issues',
  ])
  replacements.push([
    'https://github.com/LittleSongxx/NewsClaw',
    brand.githubUrl,
  ])

  // 3. Team name
  replacements.push(['LittleSongxx', brand.team])
  ...
  // 5. Tagline
  replacements.push([
    'AI News Operations Agent',
    brand.tagline,
  ])

  // 6. Split-span brand name in App.vue template:
  //    <span class="mate">News</span><span class="claw">Claw</span>
  ...
  replacements.push([
    '>News</span><span class="claw">Claw<',
    '>' + firstPart + '</span><span class="claw">' + secondPart + '<',
  ])

  // 7. Brand name (catch-all, must come last)
  //    Only replace the exact display name. Internal server property
  //    namespaces are intentionally not part of the desktop branding layer.
  replacements.push(['NewsClaw', brand.name])
...
function brandingPlugin(options) {
  ...
  var isDefault =
    brand.name === 'NewsClaw' &&
    brand.tagline === 'AI News Operations Agent' &&
    brand.team === 'LittleSongxx'

#!/usr/bin/env bash
#
# scripts/build.sh — Build the NewsClaw Spring Boot backend JAR and place it
...
SERVER_DIR="$(cd "$PROJECT_ROOT/../newsclaw-server" && pwd)"
RESOURCES_DIR="$PROJECT_ROOT/resources"

echo "==> Building newsclaw-server JAR from $SERVER_DIR"
...
JAR_FILE=$(ls "$SERVER_DIR"/target/newsclaw-server-*.jar 2>/dev/null | head -1)
if [ -z "$JAR_FILE" ]; then

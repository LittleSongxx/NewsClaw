...
class RiskIntentClassifier:
    ...
    def _target_kind(lowered: str) -> TargetKind:
        ...
        # NewsClaw 主线豁免：data/newsroom/ 是「AI 早报」的产物目录（markdown /
        # YAML / 反馈库），内容全部由管线自己生成。若落到下面的通用 data/ 规则
        # 会被判为受保护文件，无人值守的每日管线每写一个产物都卡在确认门。
        if "data/newsroom/" in lowered or "data\\newsroom\\" in lowered:

...
# ==================== 内部 trace marker 常量 ====================
#
# NewsClaw 通过 `build_tool_trace_summary()` 注入到历史回放上下文里的内部
...
_ARTIFACT_INTENT_KEYWORDS: tuple[str, ...] = (
    ...
    "做一份",
    "写一份",
    # 强信号：带量词的"整理/准备/拟一份"——用户场景"帮我整理一份NewsClaw
...
class ResponseHandler:
    ...
    async def verify_task_completion(
        ...
        plan_fail_reason = ""
        try:
            from newsclaw.agent.validators import (
                ValidationContext,
                ValidationResult,
...
工具在 **NewsClaw 宿主**执行，与用户发消息的设备/IM 客户端**默认不同域**。宿主上命令成功 ≠ 用户本机已出现窗口或已安装软件。
...
NEXT: 建议的下一步"""
        ...
        try:
            response = await self._brain.think_lightweight(
                ...
                system=(
                    "你是任务完成度判断助手。NewsClaw 工具在宿主环境执行，与用户聊天设备通常不是同一台机器；"

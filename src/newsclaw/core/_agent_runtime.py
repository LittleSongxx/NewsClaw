...
# (smoke-F0/F6) -- importing ``newsclaw.agent.confirmation`` at module-top
# triggers ``newsclaw.agent.__init__`` which immediately re-enters this very
# module via ``agent.core -> newsclaw.core._agent_runtime``.

# Import Identity from the canonical home (not the ``core.identity`` re-export
# shim) so a cold ``import newsclaw.agent.identity`` entry point cannot deadlock
...
# cycle whenever this module is loaded BEFORE ``newsclaw.agent``.
...
- 不要给建议
- 不要输出最终答案
- 不要编造能力：不得虚构「工具在用户本机执行」等事实；但若任务涉及**用户本机才可观测的效果**（本机 GUI、本机安装、游戏内 overlay 等），必须在 `constraints` 或 `risks_or_ambiguities` 中**如实**写出「默认仅在 NewsClaw 宿主执行、与用户聊天设备可能不同域」等部署边界——这与「假设能力限制」不同，是事实约束
- 只输出 YAML 格式的结构化任务定义
- 保持简洁，每项不超过一句话
...
class Agent:
    ...
    # 现在改为使用 `newsclaw.core.im_context` 中的 contextvars（协程隔离）。
    ...
    def __init__(
        ...
        # ==================== 人格系统 + 活人感 + 表情包 ====================
        from newsclaw.agent.persona import PersonaManager
        from newsclaw.agent.trait_miner import TraitMiner
    ...
    def _init_handlers(self) -> None:
        ...
        # 飞书云文档归档（NewsClaw 自研：产物落云文档 / Wiki）
        from ..tools.handlers.feishu_doc import (
            create_handler as create_feishu_doc_handler,
        )
        ...
        self.handler_registry.register("wiki", create_wiki_handler(self))
    ...
    def notify_pools_skills_changed() -> None:
        ...
        """
        try:
            from newsclaw.main import _desktop_pool, _orchestrator
    ...
    def _prepare_prompt_identity_dir(self) -> Path:
        ...
        runtime_root = Path(
            getattr(self, "_prompt_identity_runtime_root", None) or settings.newsclaw_home
        )
            ...
            try:
                ...
                session_context = {
                    ...
                    "working_directory": str(
                        __import__(
                            "newsclaw.core.working_directory",
...
- **重要**：这两个 marker 是系统注入的、由真实工具凭证支撑的回放摘要；不要在你自己的回复中模仿这种格式编造执行结果"""
    ...
    def _resolve_memory_workspace_id(session: Any | None) -> str:
        ...
        ``NEWSCLAW_DESKTOP_PROJECT_WORKSPACE=1`` 或 session.metadata
    ...
    async def _mine_traits_in_background(self, message: str, session_id: str) -> None:
        ...
        try:
            ...
            from newsclaw.agent.persona import persist_trait_to_memory
    ...
    async def _prepare_session_context(
        ...
        *,
        ...
        if session and hasattr(session, "context"):
            try:
                from newsclaw.agent.working_facts import extract_working_facts, merge_working_facts
        ...
        if session is not None and not topic_changed and hasattr(session, "context"):
            try:
                from newsclaw.runtime.context.continuity import content_digest
            ...
            for fdata in pending_files:
                if has_pdf_cap and fdata.get("type") == "document":
                    ...
                    if local_path and Path(local_path).exists():
                        try:
                            from newsclaw.channels.media.handler import MediaHandler
        ...
        *,
        ...
        try:
            ...
            if (
                ...
                from newsclaw.agent.confirmation import (
    ...
    async def _preempt_or_queue_prev_task(
        ...
        *,
        ...
        策略来自 :func:`newsclaw.api.routes.double_texting.resolve_policy`（按 channel；
    ...
    async def execute_task(self, task: Task) -> TaskResult:
        ...
        # 双桶割裂（面板按 desktop_user 查询，无头路径此前落到 'default'）。
        try:
            from ..memory.session_identity import DESKTOP_USER_ID, memory_session_user

            owner_id = memory_session_user(getattr(self, "_current_session", None))
            if owner_id in ("default", "anonymous", ""):
                owner_id = DESKTOP_USER_ID
            if getattr(self.memory_manager, "_current_user_id", None) != owner_id:
                self.memory_manager.start_session(conversation_id, user_id=owner_id)
        except Exception as exc:  # 记忆初始化失败不应阻断任务本体
            logger.debug("[Task] memory owner alignment skipped: %s", exc)
        self._current_conversation_id = conversation_id
        self._current_session_type = "cli"
            ...
            if success:
                ...
                if settings.desktop_notify_enabled and not getattr(
                    ...
                    if channel in ("cli", "desktop"):
                        from newsclaw.agent.desktop_notify import notify_task_completed_async

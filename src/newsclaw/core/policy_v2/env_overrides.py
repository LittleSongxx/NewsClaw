...
1. ``NEWSCLAW_POLICY_FILE`` — alternate POLICIES.yaml path (helm
   ConfigMap mount + sidecar render workflows). Resolved BEFORE loading,
   not via this module.
2. ``NEWSCLAW_POLICY_HOT_RELOAD`` — force enable/disable hot-reload
   regardless of POLICIES.yaml (rolling deploy: enable hot-reload
   without rewriting the config file).
3. ``NEWSCLAW_AUTO_CONFIRM`` — CI/automation: bypass non-destructive
...
def _coerce_bool(raw: str) -> bool:
    ...
    falsy — operators making typos like ``NEWSCLAW_HOT_RELOAD=yes-please``

...
def _resolve_yaml_path() -> Path | None:
    ...
    1. ``NEWSCLAW_POLICY_FILE`` 环境变量（操作员用 helm / docker run -e
    ...
    env_path = _os.environ.get("NEWSCLAW_POLICY_FILE", "").strip()
    if env_path:
...
def _audit_env_overrides(report, cfg: PolicyConfigV2 | None = None) -> None:
    ...
    try:
        if cfg is not None:
            from newsclaw.agent.audit import DEFAULT_AUDIT_PATH, AuditLogger
...
def rebuild_engine_v2(
    *,
    ...
    with _lock:
        ...
        # ENV ``NEWSCLAW_AUDIT_LOG_PATH`` would change ``_config`` but

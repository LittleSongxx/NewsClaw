"""NewsClaw built-in safety_immune paths (9 semantic categories).
...
- We want NewsClaw's own identity files / audit trails / scheduler state
...
6. **User credentials / keys** — SSH/GPG/AWS/cloud creds + LLM endpoint
   keys + NewsClaw's own user manager profiles.
...
_CATEGORY_5_SCHEDULER: tuple[str, ...] = (
    "${CWD}/data/scheduler/**",
    "${CWD}/.newsclaw/system_tasks.lock",
)

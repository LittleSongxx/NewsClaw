...
``newsclaw.core.risk_gate_workflow.RiskGateWorkflow``. This module stores the
...
class RiskGateConfirmationRecord:
    ...
    def is_pending(self, now: float | None = None) -> bool:
        return self.state == RiskGateConfirmationState.PENDING and not self.pending_deadline_passed(
            now
        )
...
class PendingRiskConfirmationStore:
    ...
    def sweep_expired(self, *, now: float | None = None) -> int:
        ...
        removed = 0
        for confirmation_id, record in list(self._records.items()):
            if record.state == RiskGateConfirmationState.PENDING and record.pending_deadline_passed(
                now
            ):
                self.transition_record(
                    record,

...
from newsclaw.scheduler.delivery import is_im_delivery_channel
...
from newsclaw.scheduler._naming import (
    FORBIDDEN_TOKENS as _TASK_NAME_FORBIDDEN,  # noqa: F401  (保持向后兼容导入)
)
from newsclaw.scheduler._naming import validate_task_name as _validate_task_name  # noqa: E402
...
async def create_task(request: Request, body: TaskCreateRequest):
    ...
    if body.working_directory:
        ...
        task.working_directory = str(authorize_working_directory(request, body.working_directory))
    else:
...
async def update_task(request: Request, task_id: str, body: TaskUpdateRequest):
    ...
    if body.task_type is not None:
        from newsclaw.scheduler.task import TaskType

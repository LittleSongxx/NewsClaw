...
def _backfill_install_source_from_symlink(pm, plugins_dir: Path) -> None:
    ...
    Older NewsClaw versions did not record where a plugin was installed
...
def _install_plugin_source(
    ...
    if (local / "plugin.json").is_file():
        return installer.install_from_path(local, plugins_dir, dev_mode=dev_mode, progress=progress)
...
def _plugin_config_store(plugin_id: str):
    from newsclaw.plugins.config_store import PluginConfigStore
...
async def get_plugin_llm_models(plugin_id: str, request: Request) -> dict[str, Any]:
    ...
    from newsclaw.plugins.llm_support import llm_catalog_payload
...
async def update_plugin_config(
    ...
    if "llm_endpoint" in body:
        ...
        from newsclaw.plugins.llm_support import validate_llm_endpoint

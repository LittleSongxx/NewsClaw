...
from newsclaw.utils.errors import format_user_friendly_error
...
def _get_storage():
    try:
        from newsclaw.config import settings
        from newsclaw.memory.storage import get_shared_storage

        return get_shared_storage(settings.project_root / "data" / "memory" / "newsclaw.db")
    except Exception:
        return None
...
async def delete_im_session(request: Request, session_id: str):
    ...
    try:
        ...
        db_path = settings.project_root / "data" / "memory" / "newsclaw.db"
...
async def set_bot_config(request: Request, body: BotConfigRequest):
    ...
    from newsclaw.channels.bot_config import BotConfigRule
...
async def set_group_policy(request: Request, body: GroupPolicyRequest):
    ...
    from newsclaw.channels.group_response import GroupResponseMode

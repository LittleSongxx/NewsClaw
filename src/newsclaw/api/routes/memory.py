...
def _current_owner(request: Request) -> tuple[str, str]:
    ...
    user_hdr = (request.headers.get("X-NewsClaw-User") or "").strip()
    ws_hdr = (request.headers.get("X-NewsClaw-Workspace") or "").strip()
    if user_hdr:
...
async def create_memory(request: Request, body: MemoryCreateRequest):
    ...
    try:
        ...
        if len(content) > _MAX_MEMORY_CONTENT_LEN:
            raise HTTPException(
                413,
                f"Memory content too long: {len(content)} chars (max {_MAX_MEMORY_CONTENT_LEN})",
            )
        try:
...
async def migrate_workspace(request: Request, body: MigrateWorkspaceRequest):
    ...
    典型场景：用户启用了项目专属工作区（``NEWSCLAW_DESKTOP_PROJECT_WORKSPACE=1``

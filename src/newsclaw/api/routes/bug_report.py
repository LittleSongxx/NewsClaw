...
def _collect_system_info() -> dict:
    ...
    try:
        ...
        info["newsclaw_version"] = get_version_string()
    except Exception:
        info["newsclaw_version"] = "unknown"
    ...
    try:
        from newsclaw.llm.config import get_default_config_path, load_endpoints_config
...
def _resolve_global_logs_dir() -> Path:
    """Return the global logs directory (Tauri-managed, under newsclaw_home).

    Respects a custom root via settings.newsclaw_home (NEWSCLAW_ROOT / NEWSCLAW_ROOT)."""
    try:
        from newsclaw.config import settings

        return settings.newsclaw_home / "logs"
    except Exception:
        from newsclaw.data_root import resolve_data_root

        return resolve_data_root() / "logs"


def _resolve_newsclaw_home_dir() -> Path:
    """Return the NewsClaw home directory shared with the desktop shell."""
    try:
        from newsclaw.config import settings

        return settings.newsclaw_home
    except Exception:
        from newsclaw.data_root import resolve_data_root
...
def _add_windows_crash_artifacts(zf: zipfile.ZipFile) -> None:
    ...
    try:
        _add_dir_recent(
            zf,
            _resolve_newsclaw_home_dir() / "crashdumps",
            "crashdumps",
            days=30,
...
def _add_wer_reports(zf: zipfile.ZipFile) -> None:
    """Add recent WER reports (and any captured minidumps) for NewsClaw.
        ...
        for report_dir in candidates:
            ...
            try:
                ...
                if not report_wer.is_file():
                    continue
                matched = "newsclaw" in report_dir.name.lower()
                if not matched:
                    try:
                        sample = report_wer.read_bytes()[: 64 * 1024]
                        matched = "newsclaw" in sample.decode("utf-8", "ignore").lower()
                    except Exception:
                        matched = False
                if not matched:
                    continue
...
def _add_webview2_crashpad(zf: zipfile.ZipFile) -> None:
    ...
    if not local_appdata:
        return
    reports = (
        Path(local_appdata) / "com.newsclaw.setupcenter" / "EBWebView" / "Crashpad" / "reports"
    )
    _add_dir_recent(
        zf,
        reports,
...
def _collect_windows_event_log() -> bytes:
    """Collect recent Application Error/WER events mentioning NewsClaw."""
    import subprocess
    ...
    ps_cmd = (
        ...
        "Where-Object { $_.Message -match 'newsclaw' } | Select-Object -First 30; "
...
def _collect_endpoint_summary() -> dict:
    ...
    try:
        from newsclaw.llm.config import get_default_config_path, read_workspace_env_values
...
def _add_diagnostic_files(zf: zipfile.ZipFile, *, upload_logs: bool, upload_debug: bool) -> None:
    ...
    if upload_logs:
        try:
            ...
            main_log = logs_dir / "newsclaw.log"
        ...
        runtime_dir = _resolve_newsclaw_home_dir() / "runtime"
    ...
    async def event_stream():
        ...
        try:
            ...
            sys_info_brief = (
                ...
                f"NewsClaw: "
                f"{sys_info.get('newsclaw_version', '?')}"
            )

...
``newsclaw.orgs`` (canonical v2 runtime, not the legacy v1 layout).
"""
...
def query_memory(request: Request, org_id: str) -> list[dict[str, Any]]:
    from newsclaw.orgs import MemoryScope, MemoryType
...
def query_activity(request: Request, org_id: str) -> dict[str, Any]:
    ...
    events = es.query(since=qp.get("since"), command_id=qp.get("command_id"), limit=limit) or []

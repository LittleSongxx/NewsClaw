"""Machine-authenticated endpoints used by NewsClaw Account."""
...
async def update_newsclaw_user_status(request: Request) -> JSONResponse:
    ...
    try:
        result = await store.process(
            secret=os.environ.get("NEWSCLAW_STATUS_PROPAGATION_SECRET", ""),
            window_seconds=int(
                os.environ.get("NEWSCLAW_STATUS_PROPAGATION_WINDOW_SECONDS", "300")
            ),
            timestamp=request.headers.get("X-NewsClaw-Timestamp", ""),
            event_id=request.headers.get("X-NewsClaw-Event-ID", ""),
            idempotency_key=request.headers.get("Idempotency-Key", ""),
            signature=request.headers.get("X-NewsClaw-Signature", ""),
            body=body,
        )

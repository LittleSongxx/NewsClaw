...
from newsclaw.agents.identity_files import PROFILE_IDENTITY_FILENAMES
from newsclaw.api.agent_events import (
...
from newsclaw.memory.json_utils import coerce_text
...
def _invalidate_bot_agent_sessions(bot_cfg: dict) -> None:
    ...
    """
    try:
        from newsclaw.main import _bot_channel_name, get_message_gateway
...
async def _apply_persisted_bot_change(
    ...
    *,
    ...
    from newsclaw.agents.profile import get_profile_store
    from newsclaw.config import runtime_state, settings
    from newsclaw.main import get_im_bot_runtime_error
...
def _mask_bot_credentials(bot: dict) -> dict:
    from newsclaw.utils.redaction import redact_value
...
def _unmask_credentials(submitted: dict, stored: dict) -> dict:
    ...
    from newsclaw.utils.redaction import REDACTION
...
def _validate_bot_credentials(bot_type: str, credentials: dict) -> None:
    from newsclaw.channels.status import missing_bot_credentials
...
async def list_bots():
    ...
    from newsclaw.config import settings
    from newsclaw.main import get_message_gateway
...
async def create_bot(body: BotCreateRequest, request: Request):
    ...
    from newsclaw.agents.profile import get_profile_store
    from newsclaw.runtime_config_coordinator import profile_reference_change_lock
...
async def create_agent_profile(body: ProfileCreateRequest):
    ...
    from newsclaw.agents.profile import AgentProfile, AgentType, SkillsMode, get_profile_store
...
async def reset_agent_profile(profile_id: str, request: Request):
    ...
    from newsclaw.agents.presets import get_preset_by_id
    from newsclaw.agents.profile import get_profile_store
    ...
    if existing is None:
        ...
        from newsclaw.agents.profile import AgentProfile
...
async def init_profile_identity(profile_id: str, request: Request):
    ...
    from newsclaw.agents.identity_resolver import ProfileIdentityResolver
    from newsclaw.config import settings
...
async def get_profile_memory_stats(profile_id: str):
    ...
    db_path = memory_dir / "newsclaw.db"

    if not db_path.exists():
        return {"exists": False, "semantic_count": 0, "db_size_bytes": 0}

    from newsclaw.storage.safe_sqlite import SQLiteUnavailable, safe_open_async_ctx
    ...
    try:
        ...
        try:
            from newsclaw.storage.degraded import registry as _degraded

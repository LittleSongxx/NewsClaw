...
from newsclaw.account.desktop import require_desktop_account
...
def _is_local_from_real_ip(request: Request) -> bool:
    ...
    Thin wrapper around :func:`newsclaw.api.auth.is_trusted_local` kept so
...
async def create_desktop_web_session(request: Request, response: Response):
    ...
    return {"url": f"{origin}/web/#newsclaw-web-session={ticket}"}

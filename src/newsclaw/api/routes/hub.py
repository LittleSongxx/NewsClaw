...
def _invalidate_imported_profile_runtime(request: Request, profile_id: str) -> None:
    try:
        from newsclaw.prompt.builder import clear_prompt_section_cache
...
async def _propagate_store_skill_install(request: Request, skill_dir: Path) -> None:
    ...
    from newsclaw.skills.events import SkillEvent
...
async def export_agent(req: ExportRequest):
    ...
    from newsclaw.agents.packager import AgentPackager, PackageError
...
async def import_agent(
    ...
    from newsclaw.agents.packager import AgentInstaller, PackageError
...
# ---------------------------------------------------------------------------
# Hub proxy routes — forward requests to the NewsClaw Platform
# ---------------------------------------------------------------------------


def _get_hub_client():
    from newsclaw.hub import AgentHubClient

    return AgentHubClient()


def _get_skill_client():
    from newsclaw.hub import SkillStoreClient

...
def _subsystem_unavailable(subsystem: str, klass: str) -> HTTPException:
    ...
    return HTTPException(
        ...
        detail={
            ...
            "subsystem": subsystem,
            "message": (f"{klass} is registered but not yet connected. See PR-9.7 wiring."),
...
def _runtime_method_not_wired(method_name: str) -> HTTPException:
    ...
    return HTTPException(
        ...
        detail={
            ...
            "message": (f"OrgRuntime.{method_name} is not yet connected. See PR-9.7gamma wiring."),
            "next_milestone": _NEXT_MILESTONE,
        },
    )
...
def _scope_to_org(subsystem: Any, request: Request) -> Any:
    ...
    """
    from newsclaw.orgs.scoped_subsystems import OrgScopedRegistry

...
def validate_identity_file(name: str, content: str) -> dict[str, list[str]]:
    ...
    if name == "POLICIES.yaml":
        ...
        from newsclaw.memory.types import MEMORY_MD_MAX_CHARS
...
async def compile_identity(request: Request, mode: str = "rules"):
    ...
    if mode == "llm":
        ...
        if brain:
            from newsclaw.prompt.compiler import PromptCompiler
    ...
    from newsclaw.prompt.compiler import get_compiled_content
...
async def compile_status():
    ...
    from newsclaw.prompt.compiler import check_compiled_outdated, get_compiled_content

...
from newsclaw.agent.security_actions import execute_controlled_action
from newsclaw.agent.trusted_paths import grant_session_trust
from newsclaw.core.ask_user_context import AskUserReplyContext
from newsclaw.core.confirmation_state import ConfirmationDecision, get_confirmation_store
from newsclaw.core.context_stats import (
    context_snapshot_from_dict,
    get_context_snapshot,
    merge_context_snapshot_into_usage,
)
from newsclaw.core.engine_bridge import engine_stream, is_dual_loop, to_engine
...
def _chat_endpoint_names() -> set[str]:
    ...
    """
    try:
        from newsclaw.api.routes.config import _get_endpoint_manager
            ...
            if session_manager and conversation_id:
                try:
                    ...
                    if session:
                        ...
                        try:
                            from newsclaw.core.feature_flags import (
...
def _cleanup_chat_runtime_state(request: Request, conversation_id: str) -> None:
    ...
    try:
        ...
        try:
            from newsclaw.main import _orchestrator as global_orchestrator
...
def _extract_source_used(event: dict) -> dict | None:
    """Extract NewsClaw source provenance from tool results.

    Tool handlers prefix result text with ``[NEWSCLAW_SOURCE] {json}`` so the
...
def _extract_mcp_call(event: dict) -> dict | None:
    ...
    ``[NEWSCLAW_MCP] {json}`` marker so the chat route can emit a stable
...
def _resolve_profile(agent_profile_id: str | None):
    ...
    from newsclaw.agents.profile import AgentProfile, get_profile_store
...
async def _stream_chat(
    ...
    from newsclaw.agent.sse_replay import format_sse_frame
    from newsclaw.agent.sse_replay import (
    ...
    try:
        ...
        from newsclaw.core.policy_v2 import DeferredApprovalRequired
        ...
        if _agent_task and not _agent_task.done() and not _bg_save_scheduled:
            ...
            try:
                ...
                pass
            try:
                import newsclaw.main as _main_mod
...
async def chat(request: Request, body: ChatRequest):
    ...
    (canonical definitions in newsclaw.events.StreamEventType):
    ...
    # before falling back to 409.  Header X-NewsClaw-DoubleTexting can
...
async def chat_resume(
    ...
    * Does NOT modify the SSE session except by reading its current_seq.
    ...
    """
    from newsclaw.agent.sse_replay import format_sse_frame
    from newsclaw.agent.sse_replay import get_registry as _get_sse_registry

...
:class:`~newsclaw.runtime.stream.StreamBus` (built lazily by
:mod:`newsclaw.runtime.stream_registry`). Each event is one
...
:data:`~newsclaw.runtime.stream.STANDARD_CHANNELS` except the
...
from newsclaw.orgs import OrgNotFound
from newsclaw.runtime.stream import STANDARD_CHANNELS, StreamEvent
...
def _build_streaming_response(request: Request, org_id: str) -> StreamingResponse:
    ...
    :class:`~newsclaw.orgs.manager.OrgManager` (``data/orgs/<id>/

...
(:mod:`newsclaw.core.conversation_metrics`) and the in-flight turn registry
(:mod:`newsclaw.api.routes.turn_registry`).  Admin uses these to verify
...
from newsclaw.core.conversation_metrics import snapshot as conversation_metrics_snapshot
...
async def get_conversation_metrics() -> dict:
    ...
    Counter names (see ``newsclaw.core.conversation_metrics``):

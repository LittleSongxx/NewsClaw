...
from newsclaw.account.desktop import desktop_session_token
from newsclaw.config import settings
from newsclaw.memory.storage import MemoryStorage
from newsclaw.memory.telemetry import emit_memory_health_event
...
def _db_path() -> Path:
    return _memory_dir() / "newsclaw.db"
...
def _desktop_token_expected() -> str:
    return os.environ.get("NEWSCLAW_DESKTOP_SESSION_TOKEN", "")
...
def _recover_method() -> str:
    ...
    if os.environ.get("NEWSCLAW_SQLITE3_EXE"):
        candidates.append(Path(os.environ["NEWSCLAW_SQLITE3_EXE"]))
    if os.environ.get("LOCALAPPDATA"):
        candidates.append(
            Path(os.environ["LOCALAPPDATA"]) / "Programs" / "NewsClaw" / "sqlite3.exe"
        )
    if os.environ.get("PROGRAMFILES"):
        candidates.append(Path(os.environ["PROGRAMFILES"]) / "NewsClaw" / "sqlite3.exe")
...
def _sqlite3_executable() -> str | None:
    if (
        os.environ.get("NEWSCLAW_SQLITE3_EXE")
        and Path(os.environ["NEWSCLAW_SQLITE3_EXE"]).exists()
    ):
        return os.environ["NEWSCLAW_SQLITE3_EXE"]
    for env_name in ("LOCALAPPDATA", "PROGRAMFILES"):
        root = os.environ.get(env_name)
        if not root:
            continue
        candidate = Path(root) / "NewsClaw" / "sqlite3.exe"
        if candidate.exists():
...
async def repair_status(request: Request):
    ...
    return {
        ...
        "backups": _list_candidates("newsclaw.db.bak.*"),
        "snapshots": _list_candidates("newsclaw.db.snapshot.*"),
...
async def run_recover(payload: RepairTokenRequest, request: Request):
    def work():
        db = _db_path()
        stamp = time.strftime("%Y%m%d_%H%M%S")
        recovered = _memory_dir() / f"newsclaw.recovered.{stamp}.db"
        sqlite3_exe = _sqlite3_executable()
        if sqlite3_exe:
            sql_path = _memory_dir() / f"newsclaw.recovered.{stamp}.sql"
...
# convention: both ``X-NewsClaw-Desktop-Token`` (Tauri-injected) AND a
...
async def _quiesce_subsystem(request: Request, subsystem: str) -> None:
    ...
    try:
        if subsystem == "token_tracking":
            from newsclaw.core.token_tracking import shutdown_token_tracking

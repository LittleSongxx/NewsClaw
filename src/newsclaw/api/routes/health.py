...
def mark_memory_repair_completed_restart_required() -> None:
    ...
    :class:`newsclaw.storage.degraded.DegradedRegistry` so the unified
    ...
    try:
        from newsclaw.storage.degraded import registry as _registry
...
def _read_last_shutdown_marker() -> dict:
    try:
        ...
        current_spawn_raw = os.environ.get("NEWSCLAW_SPAWN_STARTED_AT_MS")
...
async def health(request: Request):
    ...
    from newsclaw import __git_hash__, get_version_string
    ...
    from newsclaw.storage.degraded import registry as _degraded_registry
...
# is alive. That's useful for "is NewsClaw on at all?" but it tells external
...
async def _check_policy_engine() -> dict[str, Any] | None:
    ...
    try:
        from newsclaw.core.policy_v2.global_engine import get_engine_v2
...
async def logs_health_summary():
    ...
    from newsclaw.core.log_health import get_log_health_registry
...
async def clear_session_caches_endpoint(request: Request, conversation_id: str | None = None):
    ...
    summaries.
    """
    from newsclaw.core.session_caches import clear_session_caches
...
async def domain_rules(conversation_id: str = ""):
    ...
    from newsclaw.agent.domain_allowlist import get_domain_allowlist
...
def _probe_command_version(command: str, *args: str) -> dict[str, str | bool]:
    ...
    from newsclaw.runtime_manager import resolve_toolchain_command
...
async def diagnostics():
    ...
    from newsclaw import __version__ as backend_version
    from newsclaw.runtime_manager import (
    ...
    try:
        from newsclaw.setup_center import bridge  # noqa: F401
        ...
        checks.append(
            {
                ...
                "status": "pass",
                "code": "CORE_OK",
                "evidence": [f"newsclaw {backend_version}"],
                "autoFix": False,
                "fixHint": None,
            }
        )
    ...
    for mod_name in ("newsclaw", "pydantic", "pydantic_core", "certifi"):
        try:
    ...
    return {
        ...
        "environment": {
            ...
            "newsclawVersion": backend_version,
            "pid": os.getpid(),
            ...
            "envTrustSource": os.environ.get("NEWSCLAW_ENV_TRUST_SOURCE", ""),
            "subprocessSecretScrub": os.environ.get("NEWSCLAW_SUBPROCESS_SECRET_SCRUB") == "1",
        },
    }

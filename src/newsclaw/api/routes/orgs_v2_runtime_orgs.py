...
:mod:`newsclaw.api.routes.orgs_v2_runtime` -- thin wiring only
(D-4 LOCKED). Behavioural oracle: ``src/newsclaw/api/routes/orgs.py``
...
from newsclaw.api.schemas.orgs_v2 import OrgCreate
from newsclaw.api.schemas.orgs_v2.orgs import OrgPatch
...
def create_org(request: Request, body: OrgCreate) -> dict[str, Any]:
    from newsclaw.orgs import OrgNameConflictError
...
def get_avatar_presets() -> list[dict[str, Any]]:
    from newsclaw.orgs._runtime_templates import list_avatar_presets
...
def list_plugin_workbench_templates(request: Request) -> list[dict[str, Any]]:
    from newsclaw.orgs._runtime_templates import build_workbench_templates

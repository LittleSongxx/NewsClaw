"""
FastAPI HTTP API server for NewsClaw.

集成在 `newsclaw serve` 中，提供：
- Chat (SSE streaming)
- Models list
- Health check
- Skills management
...
import newsclaw._ensure_utf8  # noqa: F401  # Windows UTF-8 编码保护
from newsclaw.account.config import AccountFeatureConfig
from newsclaw.account.oidc import (
    AccountOIDCManager,
    KeyringTokenStore,
...
# :func:`newsclaw.api.host_resolution.resolve_api_host` at startup and passed
...
def _do_force_exit(grace_s: int) -> None:
    ...
    ``newsclaw.api.server.os._exit`` and intercept the call here.
    """
    logger.error(
...
def create_app(
    ...
    from newsclaw.runtime_config_coordinator import RuntimeConfigCoordinator
    ...
    app.include_router(newsroom.router, tags=["AI 早报"])
    app.include_router(feishu_oauth.router, tags=["飞书授权"])
    app.include_router(wiki.router, tags=["本地 Wiki"])
    ...
    if account_config.enabled:
        app.include_router(newsclaw_internal.router)
    ...
    from newsclaw import get_version_string as _get_ver
    ...
    async def _import_pending_feedback():
        ...
        try:
            ...
            home = settings.newsclaw_home
        except Exception:
            from newsclaw.data_root import resolve_data_root

            home = resolve_data_root()
    ...
    async def _cleanup_expired_resume_state():
        ...
        try:
            from newsclaw.core.cancel_cleanup import cleanup_expired_working_messages
    ...
    async def _wire_pending_approvals_sse():
        ...
        """
        try:
            from newsclaw.agent.pending_approval_notifications import (
                build_pending_approval_event_hook,
                notify_pending_approval_im,
            )
            from newsclaw.agent.pending_approvals import get_pending_approvals_store
            from newsclaw.api.routes.websocket import fire_event
        ...
        # 影响 server 启动。
        try:
            from newsclaw.core.policy_v2.hot_reload import start_hot_reloader
    ...
    async def shutdown(request: Request):
        """Gracefully shut down the NewsClaw service process.
    ...
    async def _start_async_audit_writer():
        ...
        """
        try:
            from newsclaw.agent.audit import DEFAULT_AUDIT_PATH
            from newsclaw.core.policy_v2.audit_writer import (
    ...
    async def _check_frontend_bundle_freshness() -> None:
        try:
            ...
            from newsclaw.api.routes.build_info import (
    ...
    async def _shutdown_plugin_aiosqlite_workers() -> None:
        ...
        try:
            from newsclaw.api.routes import token_stats as _token_stats
    ...
    async def _arm_shutdown_diagnostics() -> None:
        ...
        try:
            from newsclaw.api._shutdown_diagnostics import arm_shutdown_diagnostics

            try:
                from newsclaw.config import settings as _settings2
...
async def start_api_server(
    ...
    try:
        from newsclaw.config import settings as _serve_settings
    ...
    from newsclaw.core.engine_bridge import set_api_loop, set_engine_loop
    ...
    proxy_task._newsclaw_api_app = app
    return proxy_task
...
def update_runtime_refs(
    ...
    *,
    ...
    """
    app = getattr(api_task, "_newsclaw_api_app", None) if api_task is not None else None
    if app is None:
        return False
    if agent is not None:
        _attach_agent_to_app(app, agent)

...
def configured_working_roots() -> tuple[Path, ...]:
    ...
    try:
        ...
        roots.extend(
            Path(p).expanduser().resolve(strict=False) for p in get_config_v2().workspace.paths
        )
    except Exception:
        pass
    deduped: list[Path] = []
...
def resolve_session_file(session: Any, relative_path: str, *, must_exist: bool = True) -> Path:
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail=f"invalid working-directory file: {exc}"
        ) from exc
...
def resolve_chat_attachments(attachments: list[Any] | None, session: Any) -> list[Any] | None:
    ...
    for attachment in attachments:
        ...
        if supplied_local:
            ...
            try:
                if Path(supplied_local).resolve(strict=True) != resolved_upload.resolve(
                    strict=True
                ):
                    raise HTTPException(status_code=403, detail="attachment local_path mismatch")
            except OSError as exc:
                raise HTTPException(
                    status_code=422, detail="attachment local_path is invalid"
                ) from exc

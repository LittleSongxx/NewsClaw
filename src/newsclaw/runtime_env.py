...
NEWSCLAW_SECRET_ENV_PREFIXES = ("NEWSCLAW_FORCE_",)
...
def _get_newsclaw_root() -> Path:
    """获取 NewsClaw 数据根目录。

    与 config.settings.newsclaw_home 共用 :func:`newsclaw.data_root.resolve_data_root`，
    这里 import 它而不是 config，避免循环导入。
    """
...
def _get_bundled_internal_python() -> str | None:
    ...
    构建时 newsclaw.spec 会将 sys.executable 和 pip 一起复制到 _internal/，
...
def get_runtime_root() -> Path:
    """Return the dual-venv runtime root under <data root>/runtime."""
    return _get_newsclaw_root() / "runtime"
...
def get_toolchain_root() -> Path:
    return _get_newsclaw_root() / "toolchains"
...
def get_readonly_seed_roots() -> list[Path]:
    ...
    Seed roots are never written by NewsClaw. They are only used as verified
    fallback locations for enterprise/offline pre-provisioned resources.
    """
    roots: list[Path] = []
    raw_env = os.environ.get("NEWSCLAW_READONLY_SEED_DIRS", "").strip()
    if raw_env:
...
def get_bootstrap_manifest_path() -> Path | None:
    candidates: list[Path] = []
    env_bootstrap = os.environ.get("NEWSCLAW_BOOTSTRAP_DIR", "").strip()
    if env_bootstrap:
...
def resolve_pip_index() -> dict[str, str]:
    ...
    runtime manifest/settings -> NEWSCLAW_PIP_INDEX_URL -> PIP_INDEX_URL -> Aliyun.
    """
...
def sanitize_runtime_environment(
    ...
    *,
    ...
    """Return an NewsClaw-managed subprocess environment.
    ...
    if scrub_secrets:
        for key in list(merged):
            if key in NEWSCLAW_SECRET_ENV_BLOCKLIST:
                merged.pop(key, None)
    merged["PYTHONNOUSERSITE"] = "1"
    merged["NEWSCLAW_ENV_TRUST_SOURCE"] = merged.get("NEWSCLAW_ENV_TRUST_SOURCE", "runtime-api")
    return merged
...
def apply_runtime_pip_environment(
    ...
    *,
    ...
    merged["NEWSCLAW_SUBPROCESS_SECRET_SCRUB"] = "1"
...
def apply_subprocess_secret_scrub(env: dict[str, str]) -> dict[str, str]:
    """Remove NewsClaw/provider secrets unless explicitly force-injected."""
    merged: dict[str, str] = {}
    for key, value in env.items():
        force_prefix = next((p for p in NEWSCLAW_SECRET_ENV_PREFIXES if key.startswith(p)), "")
        if force_prefix:
...
def build_user_subprocess_environment(
    ...
    *,
    ...
    This keeps normal shell/project context while applying NewsClaw's minimal
    ...
    locale, proxy, Git/SSH config), while NewsClaw still scrubs runtime
    ...
    merged["NEWSCLAW_ENV_TRUST_SOURCE"] = "user-subprocess"
    if scrub_secrets:
...
def apply_managed_node_environment(env: dict[str, str]) -> dict[str, str]:
    """Prefer NewsClaw-managed Node/npm/corepack for agent/tool subprocesses."""
    merged = dict(env)
    node = get_managed_node_seed()
    node_bin = get_managed_node_bin_dir()
    ...
    for path in (npm_prefix, npm_cache, corepack_home):
        path.mkdir(parents=True, exist_ok=True)
    if node:
        merged["NEWSCLAW_NODE"] = node
    if node_bin:
        merged["NEWSCLAW_NODE_BIN"] = node_bin
    ...
    merged["NEWSCLAW_WORKSPACE_DEPS_CACHE"] = str(cache_root)
    return merged


def get_app_python_executable() -> str | None:
    env_py = os.environ.get("NEWSCLAW_APP_PYTHON", "").strip()
...
def get_agent_python_executable() -> str | None:
    env_py = os.environ.get("NEWSCLAW_AGENT_PYTHON", "").strip()
    if env_py and verify_python_executable(env_py):
        return env_py
...
def apply_agent_python_environment(env: dict[str, str]) -> dict[str, str]:
    ...
    if agent_py:
        merged["NEWSCLAW_AGENT_PYTHON"] = agent_py
    if agent_bin:
        merged["NEWSCLAW_AGENT_BIN"] = agent_bin
        merged["PATH"] = agent_bin + os.pathsep + merged.get("PATH", "")

    app_py = get_app_python_executable()
    if app_py:
        merged["NEWSCLAW_APP_PYTHON"] = app_py
...
def _probe_uv_version() -> str:
    ...
    candidates: list[str] = []
    env_uv = os.environ.get("NEWSCLAW_UV", "").strip()
    if env_uv:
...
def get_channel_deps_dir() -> Path:
    ...
    路径: <数据根目录>/modules/channel-deps/site-packages
    该目录会被 inject_module_paths() 自动扫描并注入到 sys.path。
    """
    return _get_newsclaw_root() / "modules" / "channel-deps" / "site-packages"
...
def _sanitize_sys_path() -> None:
    ...
    newsclaw_root = str(_get_newsclaw_root())
    ...
    for p in list(sys.path):
        ...
        if meipass and p.startswith(meipass):
            continue
        # 允许: 项目数据目录 (<数据根目录>/，默认 ~/.newsclaw)
        if p.startswith(newsclaw_root):
            continue
        # 允许: 当前工作目录 ('' 或 '.')
        if p in ("", "."):
            continue
...
def inject_module_paths() -> None:
    ...
    路径来源（按优先级）：
    1. NEWSCLAW_MODULE_PATHS 环境变量 — Tauri 端通过此变量传递已安装模块路径
    2. 扫描 <数据根目录>/modules/*/site-packages — 兜底机制
    ...
    # 来源 1：从 NEWSCLAW_MODULE_PATHS 环境变量读取（Tauri 端设置）
    env_paths = os.environ.get("NEWSCLAW_MODULE_PATHS", "")
    if env_paths:
    ...
    # 来源 3：扫描 <数据根目录>/modules/*/site-packages（兜底）
    # 跳过已内置到 core 包的模块，避免外部旧版本与内置版本冲突
    _BUILTIN_MODULE_IDS = {"browser"}
    modules_base = _get_newsclaw_root() / "modules"
...
def inject_module_paths_runtime() -> int:
    ...
    # 扫描 <数据根目录>/modules/*/site-packages
    modules_base = _get_newsclaw_root() / "modules"
    if modules_base.exists():
        for module_dir in modules_base.iterdir():
            if not module_dir.is_dir():

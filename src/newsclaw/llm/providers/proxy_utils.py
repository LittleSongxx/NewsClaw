...
def is_proxy_disabled() -> bool:
    ...
    支持的开关（任一为真即禁用）：
    - LLM_DISABLE_PROXY=1
    - NEWSCLAW_DISABLE_PROXY=1
    - DISABLE_PROXY=1
    """
    return (
        _is_truthy_env("LLM_DISABLE_PROXY")
        or _is_truthy_env("NEWSCLAW_DISABLE_PROXY")
...
def _get_no_proxy_value() -> str:
    ...
    parts: list[str] = []
    for env_var in ("NEWSCLAW_NO_PROXY", "NO_PROXY", "no_proxy"):
...
def should_bypass_proxy(target_url: str) -> bool:
    ...
    依据三层规则（任一命中即跳过）：
    1. 目标地址是私有 / 内网地址（自动）
    2. 匹配 NO_PROXY / NEWSCLAW_NO_PROXY 环境变量
    3. 匹配配置文件 no_proxy

...
from newsclaw.agents.task_queue import TaskQueue

if TYPE_CHECKING:
    from newsclaw.channels import MessageGateway
...
_BUDGET_GUIDE = (
    "\n\n提示：可以在 NewsClaw「配置 → 高级配置 → 长任务与上下文保护 → 任务预算」"
    ...
    def _ensure_deps(self) -> None:
        ...
        try:
            ...
            if self._log_dir is None:
                from newsclaw.config import settings as _s
    ...
    async def _run_with_progress_timeout(
        ...
        *,
        ...
        if isolated_browser and hasattr(agent, "browser_manager"):
            from newsclaw.tools.browser import BrowserUseRunner, PlaywrightTools
        ...
        *,
        ...
        async with agent._execution_lock:
            ...
            try:
                ...
                if is_sub_agent:
                    try:
                        from newsclaw.agent.output_guard import (
...
def _cleanup_sub_agent_resources(agent: Any, session: Any) -> None:
    ...
    try:
        from newsclaw.tools.handlers.todo_state import cleanup_session
...
def _build_work_summary(record: dict) -> str:
    ...
    """
    from newsclaw.agent.tools import smart_truncate

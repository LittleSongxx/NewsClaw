...
class AgentInstaller:
    ...
    _ORIGIN_FILE = ".newsclaw-origin.json"
    _LEGACY_ORIGIN_FILE = ".openakita-origin.json"  # rename 前的 provenance sidecar

    def _read_installed_version(self, skill_dir: Path) -> str | None:
        """Read the version of an already-installed skill.

        Checks (in order): .newsclaw-origin.json → SKILL.md frontmatter version.
        """
        origin = skill_dir / self._ORIGIN_FILE
        if not origin.is_file():
            legacy_origin = skill_dir / self._LEGACY_ORIGIN_FILE
            origin = legacy_origin if legacy_origin.is_file() else origin
        if origin.exists():
            try:
    ...
    def _write_origin(
        ...
        *,
        ...
        """Write .newsclaw-origin.json sidecar to track provenance."""
        data = {
            "source": source,
            "version": version or "",
            "type": origin_type,

...
class ProfileStore:
    ...
    def prune_system_presets(self, keep_ids: set[str]) -> list[str]:
        """移除不在 keep_ids 中的系统预设定义（NewsClaw fork 的预设裁剪同步）。

        只删定义（profiles/<id>.json + 缓存），保留该 profile 的数据目录
        （memory/identity），以便日后需要时找回；user_customized=True 的不动。
        """
        removed: list[str] = []
        with self._lock:
            for pid in list(self._cache):
                profile = self._cache[pid]
                if not profile.is_system or pid in keep_ids or profile.user_customized:
                    continue
                del self._cache[pid]
                fp = self._profiles_dir / f"{pid}.json"
                if fp.exists():
                    fp.unlink()
                removed.append(pid)
        if removed:
            logger.info(f"ProfileStore pruned retired system presets: {', '.join(removed)}")
        return removed

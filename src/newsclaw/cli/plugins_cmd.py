"""``newsclaw plugins ...`` Typer sub-app.
...
copy ``data/plugins/`` that :class:`newsclaw.plugins.manager.PluginManager`
...
def reseed(
    ...
    for entry, err in result.errors:
        console.print(f"  [red]X[/red] {entry.plugin_id}/{entry.rel_path}: {err}")
    if result.errors:
        raise typer.Exit(1)

...
import newsclaw._ensure_utf8  # noqa: F401  # isort: skip
...
# ``python -m newsclaw.main`` executes this file as ``__main__``. Register that
...
if __name__ == "__main__" and getattr(_executed_main, "__dict__", None) is globals():
    sys.modules["newsclaw.main"] = _executed_main
...
# Sub-app: ``newsclaw plugins ...`` (see ``cli/plugins_cmd.py``).
...
async def _init_orchestrator():
    ...
    if _orchestrator is not None:
        return
    from newsclaw.agents.orchestrator import AgentOrchestrator
...
def _ensure_channel_deps(progress_fn=None) -> dict:
    ...
    """
    from newsclaw.runtime_channel_deps import ensure_channel_dependencies
...
def _set_im_bot_runtime_state(
    ...
    from newsclaw.channels.runtime_status import set_bot_runtime_state

    set_bot_runtime_state(channel_name, status, error, progress)

    try:
        from newsclaw.api.routes.websocket import broadcast_event
...
def get_im_bot_runtime_state(channel_name: str) -> dict:
    ...
    from newsclaw.channels.runtime_status import resolve_bot_runtime_state
...
async def remove_im_bot(bot_cfg: dict) -> bool:
    ...
    if _message_gateway is None:
        return False
    channel_name = _bot_channel_name(bot_cfg)
    from newsclaw.channels.runtime_status import clear_bot_runtime_state
...
async def init_core_services(agent_or_master):
    ...
    if _desktop_pool is None:
        from newsclaw.agents.factory import AgentFactory, AgentInstancePool
...
def _apply_auto_confirm_flag(*, enabled: bool) -> None:
    ...
    ``NEWSCLAW_AUTO_CONFIRM`` override registered in
    ...
    _os.environ["NEWSCLAW_AUTO_CONFIRM"] = "1"
...
def main(
    ...
    auto_confirm: bool = typer.Option(
        ...
        help=(
            ...
            "Equivalent to NEWSCLAW_AUTO_CONFIRM=1. Destructive "
    ...
    cwd: str | None = typer.Option(
        None,
        "--cwd",
        help="会话工作目录（默认使用启动 NewsClaw 时的当前目录）",
    ),
):
    """
    NewsClaw - 全能自进化AI助手
    ...
    if version:
        ...
        console.print(f"NewsClaw v{__version__}")
        raise typer.Exit(0)
    ...
    if ctx.invoked_subcommand is None:
        ...
        if not has_endpoint:
            ...
            console.print(
                "请设置 ANTHROPIC_API_KEY，或运行 'newsclaw init' 配置 data/llm_endpoints.json"
            )
            raise typer.Exit(1)

        # C14 / R4-8: 交互模式需要 TTY 来驱动 prompt_toolkit + Rich 的
        # security_confirm 提示。stdin 为管道时（``cat ... | newsclaw`` /
        ...
        if cli_class.is_unattended:
            ...
            console.print(
                ...
                '  • [bold]newsclaw run "<task>"[/bold] - 单次任务执行（unattended）\n'
                "  • [bold]newsclaw serve[/bold] - 启动 API 服务并通过 /api/chat 调用"
            )
            raise typer.Exit(1)
...
def run(
    ...
    async def _run():
        ...
        # 桌面通知
        from newsclaw.agent.desktop_notify import notify_task_completed
...
def stop(
    ...
    Sprint 14 / v31 Phase A 配套 CLI：用 ``newsclaw stop`` 取代手敲
...
def serve(
    ...
    # `--workspace <ws_dir>` 传入，或环境变量 NEWSCLAW_USER_WORKSPACE）。
    # 用 Path.cwd() 作 fallback 仅在 CLI 用户从其它目录跑 `newsclaw serve`
    ...
    def _write_heartbeat():
        ...
        try:
            ...
            from newsclaw import __git_hash__, __version__
    ...
    async def _serve():
        ...
        from newsclaw import get_version_string

        _version_str = get_version_string()
        logger.info(f"NewsClaw {_version_str} starting...")
        try:
        ...
        try:
            ...
            from newsclaw.api.host_resolution import resolve_api_host
            from newsclaw.api.server import API_PORT, start_api_server
            ...
            if _api_host in ("0.0.0.0", "::"):
                if (
                    ...
                    and not (os.environ.get("NEWSCLAW_WEB_PASSWORD") or "").strip()
                ):
                    console.print(
            ...
            try:
                from newsclaw.api.server import update_runtime_refs
        ...
        if not _api_fatal:
            ...
            from newsclaw.startup import run_optional_startup
        ...
        if dev:
            ...
            async def _file_watcher():
                try:
                    ...
                    src_dir = Path(__file__).resolve().parent  # src/newsclaw/
        ...
        try:
            ...
            if not shutdown_cleanup_started:
                ...
                from newsclaw.startup import cancel_startup_tasks
    ...
    while first_run or cfg._restart_requested:
        ...
        if cfg._restart_requested:
            ...
            try:
                from newsclaw.runtime_env import inject_module_paths_runtime
            ...
            try:
                ...
                from newsclaw.api.host_resolution import resolve_api_host
                from newsclaw.api.server import API_PORT, wait_for_port_free
...
def plugin_scaffold(
    ...
    manifest: dict = {
        ...
        "type": "python",
        "entry": "plugin.py",
        "description": f"NewsClaw plugin: {name}",
...
from newsclaw.plugins.api import PluginAPI, PluginBase
...
"""
    ...
    console.print(f"  Next: copy to data/plugins/{name}/ and restart NewsClaw")
...
def run_mcp_module(
    ...
    """
    if not module_path.startswith("newsclaw."):
        print(f"Error: only newsclaw.* modules allowed, got: {module_path}", file=sys.stderr)
        raise typer.Exit(1)
...
def reset_password(
    ...
    from newsclaw.api.auth import WebAccessConfig
    ...
    if backend_running:
        console.print(
            "[yellow]⚠ 检测到 newsclaw backend 仍在运行。请手动重启 backend "
            "（关闭 Setup Center / kill 进程后重新启动），新的 setup 流程才会生效。[/yellow]"
        )
    else:
        console.print("[dim]下次启动 newsclaw 时会进入 setup 流程。[/dim]")

...
- ``requires.newsclaw``   — minimum system version  (``>=1.27.0``)
- ``requires.openakita``  — same check under the pre-rename key; plugins
  published before the rebrand only declare this one, and silently skipping
  the check would drop the minimum-version gate for the whole existing
  plugin ecosystem.
...
def _check_openakita(plugin_id: str, spec: str, result: CompatResult) -> None:
    ...
    if not spec.startswith(">="):
        result.warnings.append(f"Unrecognised newsclaw spec '{spec}' (expected >=X.Y.Z)")
        return

    min_ver = _parse_version(spec[2:])
    if min_ver is None:
        result.warnings.append(f"Cannot parse newsclaw version in '{spec}'")
        return

    current = _get_system_version()
    if current < min_ver:
        msg = (
            f"Plugin '{plugin_id}' requires newsclaw {spec}, "
...
def _check_sdk(plugin_id: str, spec: str, result: CompatResult) -> None:
    ...
    if sdk is None:
        result.warnings.append(
            ...
            f'installed. Fix it with `pip install "newsclaw[plugins]"` '

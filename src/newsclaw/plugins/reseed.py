...
the path actually loaded by :class:`newsclaw.plugins.manager.PluginManager`).
This module provides:

* :func:`compute_drift` -- pure-function, per-file diff used by both the
  startup warning and the ``newsclaw plugins reseed`` CLI.
    ...
    *,
    ...
    for idx, entry in enumerate(source_newer):
        ...
        logger.warning(
            ...
            "stale code. Run `newsclaw plugins reseed --apply` to sync.",
            runtime_path,
            source_path,
            delta,
        )
    if len(source_newer) > _MAX_WARN_DETAILS:
        logger.warning(
            ...
            "run `newsclaw plugins reseed` for the full diff.",
    ...
    if source_only:
        ...
        for pid, n in sorted(per_plugin.items()):
            logger.info(
                ...
                "seeded to %s/. Run `newsclaw plugins reseed --apply` to seed.",
                pid,
                n,
                source_root.name,
                runtime_root.name,
...
def apply_reseed(
    ...
    *,
    ...
    for entry in to_copy:
        ...
        bucket = result.forced if entry.status == STATUS_RUNTIME_NEWER else result.copied
        if dry_run:
            bucket.append(entry)
            continue
        try:

...
class PluginManager:
    ...
    def _check_newsclaw_version(manifest: PluginManifest) -> bool:
        ...
        for e in result.errors:
            if record_health_event(
                ...
                e,
                severity="error",
                suggestion="插件与当前 NewsClaw/API 版本不兼容，请升级插件或禁用它。",
            ):
                logger.error(e)
        if not result.ok:
            if record_health_event(
            ...
            try:
                ...
                if (
                    ...
                    msg = (
                        ...
                        'Run `pip install "newsclaw[plugins]"` (production) or '
    ...
    def _format_missing_module_error(
        ...
        module_dir = resolve_data_root() / "modules" / manifest.id / "site-packages"
        pip_specs: list[str] = []
        try:
    ...
    def _load_python_plugin(
        ...
        module_name = f"newsclaw_plugin_{manifest.id.replace('-', '_')}"
        ...
        # which is not a pattern NewsClaw plugins use.
    ...
    def _maybe_warn_on_source_drift(self) -> None:
        ...
        :func:`newsclaw.plugins.reseed.warn_on_drift` for the details.
        """
        try:

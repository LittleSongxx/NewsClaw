...
def _pip_subprocess_env(python_executable: str) -> dict[str, str]:
    ...
    """
    from newsclaw.runtime_manager import apply_runtime_pip_environment
...
def safe_install_error(value: object) -> str:
    ...
    from newsclaw.utils.redaction import redact_text
...
def _validate_tool_classes_completeness(manifest: Any, *, mode: str = "off") -> list[str]:
    ...
    Args:
        manifest: A :class:`~newsclaw.plugins.manifest.PluginManifest`
...
def _download_to_file(url: str, dest: Path) -> None:
    req = Request(url, headers={"User-Agent": "NewsClaw-PluginInstaller/1.0"})
    try:
...
def install_pip_deps(
    ...
    *,
    ...
    if proc.returncode != 0:
        ...
        return failed(
            excerpt, "dependency_network" if _pip_network_failure(excerpt) else "dependency_install"
        )
    return True
...
def _finalize_install(
    ...
    if not installed:
        ...
        raise PluginInstallError(
            ...
            "NewsClaw does not migrate plugin dependency directories in place; "
            "the isolated plugin directory was removed and reinstall/rebuild is required. "
            "See newsclaw.plugins.installer logs for the pip error excerpt."
        )
    return manifest.id

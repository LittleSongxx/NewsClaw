...
This module bridges the unified ``newsclaw.llm.types`` block representation
...
``newsclaw.agent.brain`` calls this module directly.
"""
...
def map_stop_reason(stop_reason: StopReason) -> str:
    """Project an :class:`newsclaw.llm.types.StopReason` to the Anthropic string.

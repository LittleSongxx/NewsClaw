...
--------------------
The dual-ledger :class:`~newsclaw.runtime.supervisor.Supervisor` skeleton
(outer/inner loop, :class:`~newsclaw.runtime.stall_detector.StallDetector`,
...
``newsclaw.agent.supervisor_brain`` (``Degenerate`` / ``PassThrough``),
...
* ``newsclaw.runtime.supervisor.SupervisorBrain`` -- the *orchestration*
  protocol the :class:`Supervisor` actually depends on:
  ``extract_facts`` / ``draft_plan`` / ``emit_progress_ledger``.
* ``newsclaw.agent.brain`` -- a *gateway* protocol (``think_lightweight`` /
...
from newsclaw.runtime.supervisor import DelegationResult, SupervisorBrain
...
# The PROGRESS_LEDGER schema is reordered to match NewsClaw's strict
...
class CallableSupervisorLLMClient:
    ...
    async def complete(
        ...
        *,
        ...
        return await self.fn(role=role, system=system, user=user, cancel_event=cancel_event)
...
class LLMSupervisorBrain(SupervisorBrain):
    ...
    def _render_outputs(self, recent_outputs: Sequence[DelegationResult] | None) -> str:
    ...
    async def draft_plan(
        ...
        *,
        ...
        user = ORCHESTRATOR_TASK_LEDGER_PLAN_PROMPT.format(team=self._team_block(), facts=facts)
        return await self.client.complete(
            role="plan",
            system="You are the orchestrator of an AI agent organization.",
            user=user,
    ...
    def _normalise_next_speaker(self, raw: str) -> str:
        ...
        resolved = self.resolve_next_speaker(original, self.node_directory, self.root_node_id)
        if resolved == original:
            return raw
        entry["answer"] = resolved
        prior_reason = str(entry.get("reason", ""))
        entry["reason"] = (f"{prior_reason} [resolved {original!r} -> {resolved!r}]").strip()

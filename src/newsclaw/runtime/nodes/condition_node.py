        ...
        *,
        ...
        for label, target in branches.items():
            if not isinstance(label, str) or not label:
                raise ValueError(f"branch label must be a non-empty string, got {label!r}")
            if not isinstance(target, str) or not target:
                raise ValueError(f"branch target for {label!r} must be a non-empty string")
    ...
    def _build_inputs(msg: NodeMessage) -> BranchInputs:
        ...
        if text.startswith("{") and text.endswith("}"):
            try:
                ...
                logger.debug("ConditionNode: instruction looked like JSON but failed to parse")
        inputs["text"] = text
        if msg.metadata:
            inputs.update(msg.metadata)
        return inputs

    def _fail(self, msg: NodeMessage, reason: str, extra: Mapping[str, Any]) -> DelegationResult:
        return DelegationResult(
            success=False,
            speaker=self.node_id,
            message=reason,

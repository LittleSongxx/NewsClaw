...
* It exposes a small, declarative contract for *what* the brain
  ...
  pattern but trimmed to what NewsClaw actually needs in v2.
...
class BrainResponse:
    ...
    def __post_init__(self) -> None:
        if (self.answer is None) == (self.tool_call is None):
            raise ValueError("BrainResponse must set exactly one of answer or tool_call")
...
class LLMNode(BaseNode):
    ...
    async def _run_tool(
        ...
        if self._tool_runner is None:
            return TranscriptTurn(
                speaker="tool",
                content=(f"tool call rejected: node {self.node_id} has no tool runner"),
                metadata={"tool_name": request.tool_name, "rejected": True},
            )
        if self._allowed_tools is not None and request.tool_name not in self._allowed_tools:
            return TranscriptTurn(
                speaker="tool",
                content=(
    ...
    def _tool_budget_exhausted(self, msg: NodeMessage, used: int) -> DelegationResult:
        return DelegationResult(
            success=False,
            speaker=self.node_id,
            message=(
                f"tool call budget exhausted ({used} >= max_tool_calls={self._max_tool_calls})"
            ),
            metadata={
                "correlation_id": msg.correlation_id,
                "tool_calls": used,
    ...
    async def _emit_assistant(self, ctx: NodeContext, answer: str) -> None:
        await ctx.stream.emit(
            "messages",
            "assistant_answer",
            {

...
ConditionalRouter = Callable[[DelegationResult], "str | Awaitable[str]"]
...
class StateGraph:
    ...
    def add_node(self, node_id: str) -> None:
        ...
        if not isinstance(node_id, str) or not node_id:
            raise StateGraphError(f"node id must be a non-empty string, got {node_id!r}")
        if node_id in (START, END):
            raise StateGraphError(f"{node_id!r} is reserved; cannot register as a node")
    ...
    def add_edge(self, src: str, dst: str) -> None:
        ...
        if existing is not None and existing != dst:
            raise StateGraphError(
                f"{src!r} already has a static edge to {existing!r}; cannot also point at {dst!r}"
            )
    ...
    def add_conditional_edges(
        ...
        if not mapping:
            raise StateGraphError(f"conditional edges from {src!r} need at least one mapping")
        for label, target in mapping.items():
            if not isinstance(label, str) or not label:
                raise StateGraphError(f"branch label must be a non-empty string, got {label!r}")
        ...
        self.conditional_edges[src] = _ConditionalEdge(router=router, mapping=dict(mapping))
    ...
    async def route(self, src: str, result: DelegationResult) -> str | None:
        ...
        if cond is not None:
            ...
            if not isinstance(label, str):
                raise StateGraphError(
                    f"conditional router for {src!r} must return str, got {type(label).__name__}"
                )
    ...
    def validate(self) -> None:
        ...
        if self.entry_point is None:
            raise StateGraphError(
                "graph has no entry point; call add_edge(START, <node>) or set_entry_point(<node>)"
            )
        if self.entry_point not in self.nodes:
            raise StateGraphError(f"entry point {self.entry_point!r} is not a registered node")
        for src, dst in self.static_edges.items():
            if src not in self.nodes:
                raise StateGraphError(f"static edge src {src!r} is not a registered node")
            if dst != END and dst not in self.nodes:
                raise StateGraphError(f"static edge dst {dst!r} from {src!r} is not registered")
        for src, edge in self.conditional_edges.items():
            if src not in self.nodes:
                raise StateGraphError(f"conditional edge src {src!r} is not a registered node")
            for label, dst in edge.mapping.items():
                if dst != END and dst not in self.nodes:
                    raise StateGraphError(
                        f"conditional edge mapping {src!r}/{label!r} -> {dst!r} is not registered"
                    )
    ...
    def _validate_source(self, src: str) -> None:
        if src in (START, END):
            raise StateGraphError(f"{src!r} cannot be the source of an edge")
        if src not in self.nodes:
            raise StateGraphError(f"{src!r} is not a registered node; call add_node first")

    def _validate_target(self, dst: str, *, allow_end: bool = True) -> None:
        if not isinstance(dst, str) or not dst:
            raise StateGraphError(f"edge target must be a non-empty string, got {dst!r}")
        if dst == END:
            if not allow_end:
        ...
        if dst not in self.nodes:
            raise StateGraphError(f"target {dst!r} is not a registered node")
...
def compile_from_org(
    ...
    *,
    entry_point: str | None = None,
    conditional_routers: Mapping[str, tuple[ConditionalRouter, Mapping[str, str]]] | None = None,
) -> StateGraph:

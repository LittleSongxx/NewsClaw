...
---------------------------------------------------------------
The orchestration brain (:class:`~newsclaw.runtime.llm_supervisor_brain.LLMSupervisorBrain`)
depends only on the narrow :class:`~newsclaw.runtime.llm_supervisor_brain.SupervisorLLMClient`
...
from newsclaw.llm.types import ConfigurationError, Message
...
class GatewaySupervisorLLMClient:
    """Adapt the gateway :class:`~newsclaw.llm.client.LLMClient` to the narrow
    ``SupervisorLLMClient`` ``complete`` seam.

    Args:
        client: the gateway LLM client (typically
            :func:`newsclaw.llm.client.get_default_client`).

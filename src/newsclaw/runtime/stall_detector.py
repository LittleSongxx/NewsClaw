    ...
    def evaluate(self, progress: ProgressLedger) -> StallDecision:
        ...
        if self.n_turns >= self.max_turns:
            decision = StallDecision(
                ...
                max_stalls=self.max_stalls,
                max_turns=self.max_turns,
                reason=(f"reached max_turns={self.max_turns} without request_satisfied"),
            )
            self.history.append(decision)
            return decision

        # 3) Stall accounting.
        stalled_this_turn = (not progress.progress_being_made) or progress.in_loop
        if stalled_this_turn:
            self.n_stalls += 1
            stall_reason_parts: list[str] = []
            if not progress.progress_being_made:
                stall_reason_parts.append(f"no progress: {progress.is_progress_being_made.reason}")
            if progress.in_loop:
                stall_reason_parts.append(f"in loop: {progress.is_in_loop.reason}")
        ...
        if self.n_stalls >= self.max_stalls:
            decision = StallDecision(
                ...
                max_turns=self.max_turns,
                reason=(
                    f"n_stalls={self.n_stalls} >= max_stalls={self.max_stalls}: {stall_reason}"
                ),
            )
            self.history.append(decision)
            return decision
        ...
        if self.n_stalls > 0:
            ...
            reason = f"n_stalls={self.n_stalls} (max={self.max_stalls}); " + stall_reason
        else:

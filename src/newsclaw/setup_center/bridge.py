...
- `python -m newsclaw.setup_center.bridge list-providers`
- `python -m newsclaw.setup_center.bridge list-models --api-type ... --base-url ... [--provider-slug ...]`
- `python -m newsclaw.setup_center.bridge list-skills --workspace-dir ...`
...
def list_providers() -> None:
    from newsclaw.llm.registries import list_providers as _list_providers
...
async def _list_models_openai(api_key: str, base_url: str, provider_slug: str | None) -> list[dict]:
    ...
    from newsclaw.llm.capabilities import infer_capabilities
    ...
    from newsclaw.llm.types import normalize_base_url
    ...
    from newsclaw.llm.providers.proxy_utils import get_httpx_client_kwargs
...
async def feishu_onboard_start(domain: str) -> None:
    ...
    from newsclaw.setup.feishu_onboard import FeishuOnboard
...
async def feishu_validate(app_id: str, app_secret: str, domain: str) -> None:
    """验证飞书凭证有效性"""
    from newsclaw.setup.feishu_onboard import validate_credentials
...
async def wecom_onboard_start() -> None:
    ...
    from newsclaw.setup.wecom_onboard import WecomOnboard
...
async def qqbot_onboard_start() -> None:
    ...
    from newsclaw.setup.qqbot_onboard import QQBotOnboard
...
async def qqbot_validate(app_id: str, app_secret: str) -> None:
    """验证 QQ 机器人凭证有效性"""
    from newsclaw.setup.qqbot_onboard import validate_credentials
...
async def wechat_onboard_start() -> None:
    ...
    from newsclaw.setup.wechat_onboard import WeChatOnboard
...
def list_skills(workspace_dir: str) -> None:
    from newsclaw.skills.loader import SkillLoader
    ...
    for s in skills:
        ...
        if skill_path and not source_url:
            try:
                ...
                origin_file = source_base / ".newsclaw-source"
...
def _resolve_skills_dir(workspace_dir: str) -> Path:
    ...
    若参数为空则使用数据根目录（NEWSCLAW_ROOT / NEWSCLAW_ROOT / 默认路径）。
    """
    if workspace_dir and workspace_dir.strip():
        return Path(workspace_dir).expanduser().resolve() / "skills"

    from newsclaw.data_root import resolve_data_root

    return resolve_data_root() / "workspaces" / "default" / "skills"
...
def _try_platform_skill_download(skill_id: str, dest_dir: Path) -> bool:
    """Try downloading a cached skill ZIP from the NewsClaw platform.
    ...
    if api_key:
        headers["X-NewsClaw-Key"] = api_key
...
def _download_github_zip(repo_owner: str, repo_name: str, dest_dir: Path) -> None:
    ...
    for branch in ("main", "master"):
        ...
        for tpl in _GITHUB_ZIP_MIRRORS:
            ...
            try:
                req = urllib.request.Request(url, headers={"User-Agent": "NewsClaw"})
                with urllib.request.urlopen(req, timeout=30) as resp:
                    data = resp.read()
                break
            except Exception as e:
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        ...
        tmp_extract = Path(tempfile.mkdtemp(prefix="newsclaw_zip_"))
        try:
...
def _parse_github_url(url: str) -> tuple[str, str, str | None] | None:
    ...
    """
    from newsclaw.skills.source_url import parse_github_source
...
def _download_gitee_zip(repo_owner: str, repo_name: str, dest_dir: Path) -> None:
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        _validate_zip_members(zf)
        tmp_extract = Path(tempfile.mkdtemp(prefix="newsclaw_gitee_"))
        try:
            zf.extractall(tmp_extract)
            children = list(tmp_extract.iterdir())
            src = children[0] if len(children) == 1 and children[0].is_dir() else tmp_extract
...
def _read_skill_source(d: Path) -> str:
    """读取技能目录的安装来源标记。"""
    try:
        return (d / ".newsclaw-source").read_text(encoding="utf-8").strip()
    except Exception:
        return ""
...
def _install_repo_tree_to_target(
    *,
    ...
    tmp_parent = Path(tempfile.mkdtemp(prefix="newsclaw_skill_"))
...
def _extract_skill_signal(raw: str) -> str:
    ...
    支持三种粘贴方式：
      ...
      2. 单行 CLI：``newsclaw install-skill owner/repo`` —— 先用
...
def install_skill(
    ...
    *,
    ...
    if category:
        try:
            from newsclaw.skills.categories import is_valid_category_name
    ...
    try:
        ...
        origin_file.write_text(url, encoding="utf-8")
        if origin_metadata:
            (target / ".newsclaw-origin.json").write_text(
...
def main(argv: list[str] | None = None) -> None:
    ...
    p = argparse.ArgumentParser(prog="newsclaw.setup_center.bridge")
...
if __name__ == "__main__":
    from newsclaw.runtime_env import IS_FROZEN, ensure_ssl_certs, inject_module_paths

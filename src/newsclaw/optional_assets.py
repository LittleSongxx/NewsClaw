"""Resolve accelerated download sources for optional NewsClaw features.

The public manifest is deliberately declarative: it may select a mirror for a
locally-known installer strategy, but it cannot supply commands for NewsClaw
...
def _fetch_manifest(url: str) -> dict:
    request = urllib.request.Request(
        url,
        headers={"Accept": "application/json", "User-Agent": "NewsClaw optional-assets"},
    )
...
def resolve_optional_asset_mirror(
    ...
    *,
    ...
    ``NEWSCLAW_OPTIONAL_ASSET_MIRROR`` overrides the public manifest and is
    interpreted as the root that contains ``mirror_path``.  The manifest URL
    itself can be overridden with ``NEWSCLAW_OPTIONAL_ASSET_MANIFEST``.
    Resolution failures are intentionally non-fatal so callers can use their
    upstream source.
    """

    override_root = _valid_base_url(os.environ.get("NEWSCLAW_OPTIONAL_ASSET_MIRROR"))
    if override_root:
        base_url = urljoin(f"{override_root}/", mirror_path.strip("/"))
        return OptionalAssetMirror(feature_id, strategy, base_url.rstrip("/"))

    manifest_url = os.environ.get(
        "NEWSCLAW_OPTIONAL_ASSET_MANIFEST", DEFAULT_OPTIONAL_ASSET_MANIFEST
    ).strip()

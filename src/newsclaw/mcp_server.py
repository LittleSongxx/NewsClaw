...
将 NewsClaw 的核心能力暴露为 MCP 服务器，
允许其他 AI Agent（如 Claude Desktop、Cursor 等）通过 MCP 协议调用。

暴露的工具:
- newsclaw_chat: 与 NewsClaw 对话
- newsclaw_memory_search: 搜索记忆
- newsclaw_schedule_task: 创建定时任务
- newsclaw_list_skills: 列出可用技能
- newsclaw_execute_skill: 执行技能

启动方式:
    python -m newsclaw.mcp_server [--port 8765]
"""
...
MCP_SERVER_NAME = "newsclaw"
MCP_SERVER_VERSION = "1.0.0"

EXPOSED_TOOLS = [
    {
        "name": "newsclaw_chat",
        "description": "Send a message to NewsClaw and get a response",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "newsclaw_memory_search",
        "description": "Search NewsClaw's memory for relevant information",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "newsclaw_list_skills",
        "description": "List available NewsClaw skills",
        "inputSchema": {
            "type": "object",
            "properties": {},
        },
...
class MCPServer:
    """Lightweight MCP server that exposes NewsClaw capabilities via stdio."""
    ...
    async def _execute_tool(self, tool_name: str, arguments: dict) -> str:
        ...
        if tool_name == "newsclaw_chat":
            ...
            # process over stdio). Each ``newsclaw_chat`` invocation is
            ...
            try:
                ...
                mcp_ctx = build_policy_context(
                    session_id="mcp_tool_newsclaw_chat",
                    channel="mcp",
        ...
        elif tool_name == "newsclaw_memory_search":
        ...
        elif tool_name == "newsclaw_list_skills":
    ...
    async def run_stdio(self):
        ...
        logger.info("NewsClaw MCP Server starting on stdio...")

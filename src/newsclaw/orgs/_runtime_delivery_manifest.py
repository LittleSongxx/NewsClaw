...
class DeliveryManifestLedger:
    ...
    def __init__(self, *, max_commands: int = 128) -> None:
        ...
        self._records: OrderedDict[tuple[str, str], dict[tuple[str, str], DeliveryManifest]] = (
            OrderedDict()
        )
        ...
        *,
        ...
        with self._lock:
            ...
            if assignment_id is not None:
                ...
                candidates = [
                    item
                    for (candidate_node, _assignment), item in records.items()
    ...
    *,
    ...
    if (workspace_dir or "").strip():
        command_roots.append(
            Path(str(workspace_dir)).expanduser().resolve() / manifest.command_id / "artifacts"
        )
    try:
    ...
    for artifact in manifest.artifacts:
        ...
        if not matches:
            ...
            continue
        if artifact.kind == "video" and not any(
            _record_has_valid_file(record) for record in matches
        ):
            failures.append(
                {
                    "code": "media_delivery_validation_missing",

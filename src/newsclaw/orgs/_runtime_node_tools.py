...
:data:`newsclaw.tools.handlers.default_handler_registry` the v1 chat path
...
# ``src/newsclaw/orgs/tool_handler.py`` therefore landed INSIDE the source
...
# resolves into the NewsClaw source tree (or the VCS dir), anchored on the
...
def _guarded_source_roots() -> tuple[Path, ...]:
    ...
    Anchored on ``newsclaw.__file__`` so the source tree is protected even
    when the process CWD differs from the repo root.
    """
    try:
        import newsclaw

        pkg_dir = Path(newsclaw.__file__).resolve().parent  # .../src/newsclaw
        ...
        for root in roots:
            try:
                if resolved == root or resolved.is_relative_to(root):
                    return (
                        f"[拒绝写入：{tool_name} 的目标路径 '{raw}' 落在 NewsClaw 工程源码"
...
MAX_TOOL_ROUNDS = _env_int("NEWSCLAW_ORG_MAX_TOOL_ROUNDS", 6, lo=1, hi=12)
...
MAX_TOOL_CALLS = _env_int("NEWSCLAW_ORG_MAX_TOOL_CALLS", 16, lo=1, hi=40)
...
MAX_LLM_CALL_S = _env_int("NEWSCLAW_ORG_NODE_LLM_CALL_TIMEOUT_S", 240, lo=0, hi=1800)
...
MAX_SEARCH_CALLS = _env_int("NEWSCLAW_ORG_MAX_SEARCH_CALLS", 6, lo=1, hi=40)
...
def _flatten_external_tools(entries: Iterable[str] | None) -> set[str]:
    ...
    Mirrors :func:`newsclaw.orgs._runtime_tool_categories.expand_tool_categories`
...
def resolve_node_tools(
    *,
    ...
    from newsclaw.tools.definitions import get_tool_definition
...
async def execute_node_tool(
    ...
    * ``is_error=True`` -- the handler raised or no handler was mapped.
    ...
    from newsclaw.llm.converters.tools import PARSE_ERROR_KEY
    ...
    # NewsClaw source tree (the "stray tool_handler.py" pollution incident).
    ...
    from newsclaw.tools.tool_hints import ToolConfigError

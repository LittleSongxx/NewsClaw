...
subsystems. The v1 ``src/newsclaw/orgs/runtime.py`` is 6 355
...
def _pick_event_field(ev: dict[str, Any], nested: dict[str, Any], keys: tuple[str, ...]) -> Any:
...
def _describe_recent_event(etype: str, ev: dict[str, Any], last_assigned: dict[str, str]) -> str:
    ...
    **Composes** (DI via ``__init__``) the 6 reused Protocols
    ...
    def __init__(
        ...
        *,
        ...
        # managers are reachable via ``newsclaw.orgs``
            ...
            if store is not None and hasattr(store, "query"):
                for ev in store.query(limit=300) or []:
                    ...
                    if isinstance(out_len, int) and out_len > 0:
                        data["result_preview"] = (
                            data.get("result_preview", "") or f"（输出 {out_len} 字）"
                        )
    ...
    async def set_node_status(
        ...
        try:
            ...
            prior = None
        try:
            await self._node_lifecycle.set_node_status(org_id, node_id, new_status, reason=reason)
        except TypeError:
    ...
    async def freeze_node(
        self, org_id: str, node_id: str, *, reason: str | None = None
    ) -> str | None:
    ...
    def get_stats(self, org_id: str) -> dict[str, Any] | None:
        ...
        for n in nodes:
            ...
            try:
                ...
                status = (
                    str(live).lower() if live else str(_attr(n, "status", "idle") or "idle").lower()
                )
            except Exception:  # noqa: BLE001
            ...
            if status not in buckets:
                status = "idle"
            buckets[status] += 1
            per_node.append({"id": nid, "role_title": role, "department": dept, "status": status})
    ...
    async def _stream_event_tap(self, event_name: str, payload: dict[str, Any]) -> None:
        ...
        lazily because ``newsclaw.runtime`` pulls a chunk of the
    ...
    async def _ws_event_tap(self, event_name: str, payload: dict[str, Any]) -> None:
        ...
        try:
            if event_name == "agent_run_started" and node_id:
                ...
                event_name in ("agent_run_finished", "agent_run_failed", "agent_run_cancelled")
                and node_id
            ):
                ...
                if incomplete:
                    ...
                    if bb_registry is not None:
                        try:
                            ...
                            _LOGGER.debug("contract: incomplete note publish failed", exc_info=True)
                    return
                    ...
                    try:
                        ...
                        if not open_tasks:
                            open_tasks = [
                                ...
                                and (not command_id or str(t.get("chain_id") or "") != command_id)
                            ]
                        if open_tasks:
                            t = open_tasks[-1]
    ...
    async def _maybe_render_root_pdf(
        ...
        *,
        ...
        rendered = await render_markdown_to_pdf(
            ...
            out_path=pdf_path,
            title="任务交付报告",
            meta=f"由根节点 {node_id} 汇总交付 · NewsClaw 组织编排",
        )
        if not rendered:
            return
        done.add(command_id)

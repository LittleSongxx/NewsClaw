...
contract; concrete factories live in ``newsclaw.agents``
and get wired by the runtime composition root (P9.6i).

ADR-0012 (no-shim): zero ``newsclaw.orgs`` imports; v2
...
dispatch_depth_var: ContextVar[int] = ContextVar("newsclaw_orgs_v2_dispatch_depth", default=0)
...
current_command_id_var: ContextVar[str] = ContextVar("newsclaw_orgs_v2_command_id", default="")
...
current_chain_id_var: ContextVar[str] = ContextVar("newsclaw_orgs_v2_chain_id", default="")
...
class AgentSpec:
    ...
    ``newsclaw.agents`` at the type level. The builder
...
class AgentBuilderProtocol(Protocol):
    ...
    Implementations in ``newsclaw.agents.factory`` /
    ``newsclaw.agent`` wire concrete Agent / Brain

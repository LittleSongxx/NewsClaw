...
Replaces v1 ``newsclaw.orgs.blackboard.OrgBlackboard`` (344
...
``src/newsclaw/orgs/`` once P9.9 runs).
"""
    ...
    * ``<org_dir>/memory/nodes/<node_id>.jsonl`` -- NODE.
    ...
    def _evict_if_needed(self, path: Path, max_entries: int) -> None:
        if not path.is_file():
            return
        with self._lock:
            records = [r for r in read_jsonl_objects(path, log=logger) if isinstance(r, dict)]
    ...
    def read(
        ...
        *,
        ...
        with self._lock:
            if not path.is_file():
                return []
            records = read_jsonl_objects(path, log=logger, decoder=OrgMemoryEntry.from_dict)
...
class OrgBlackboard:
    ...
    def read_department(
        ...
        return self._backend.read(MemoryScope.DEPARTMENT, dept_name, limit=limit, tag=tag)
    ...
    def write_org(
        ...
        if self._backend.is_duplicate(MemoryScope.ORG, self._org_id, content):
            ...
            return None
        self._backend.append(MemoryScope.ORG, self._org_id, entry, max_entries=MAX_ORG_MEMORIES)
        return entry
    ...
    def write_node(
        ...
        self._backend.append(MemoryScope.NODE, node_id, entry, max_entries=MAX_NODE_MEMORIES)
        return entry
    ...
    def query(
        ...
        if scope is None or scope == MemoryScope.DEPARTMENT:
            all_entries.extend(
                self._backend.all_for_scope(MemoryScope.DEPARTMENT, owner=scope_owner)
            )
        if scope is None or scope == MemoryScope.NODE:
            all_entries.extend(self._backend.all_for_scope(MemoryScope.NODE, owner=scope_owner))
        if memory_type:
    ...
    def get_org_summary(self, max_entries: int = 10) -> str:
        ...
        if not entries:
            return "(???????)"
        return "\n".join(
            f"- [{e.memory_type.value}] {e.content}" + (f" [{', '.join(e.tags)}]" if e.tags else "")
            for e in entries
        )
...
class SqliteBlackboardBackend:
    ...
    def append(
        ...
        *,
        ...
        with self._write_txn() as conn:
            ...
            cur = conn.execute(
                "SELECT id FROM memory_entries WHERE org_id=? AND scope=? AND scope_owner=?",
    ...
    def is_duplicate(
        ...
        *,
        ...
        with self._lock:
            rows = self._conn.execute(
                "SELECT payload FROM memory_entries WHERE org_id=? AND scope=? AND scope_owner=?",
                (self._org_id, scope.value, owner),
            ).fetchall()
        for (payload,) in rows:
            try:

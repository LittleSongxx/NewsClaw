...
:class:`newsclaw.orgs.store.JsonOrgStore` so the two
backends are interchangeable through the
:func:`newsclaw.orgs.get_default_store` factory and the
...
_SELECT_COLS = "id, name, description, payload, created_at, updated_at, version"
...
class SqliteOrgStore:
    ...
    def _row_to_org(self, row: tuple) -> OrgV2 | None:
        ...
        try:
            ...
            logger.warning("[orgs_v2 sqlite] dropping malformed org id=%s (%s)", org_id, exc)
            return None
    ...
    def patch(
        ...
        *,
        ...
        with self._write_txn() as conn:
            row = conn.execute(f"SELECT {_SELECT_COLS} FROM orgs WHERE id=?", (org_id,)).fetchone()
            if row is None:
                raise OrgNotFound(org_id)
            org = self._row_to_org(row)
            if org is None:

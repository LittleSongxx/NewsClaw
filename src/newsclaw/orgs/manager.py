...
from newsclaw.memory.types import normalize_tags
...
class OrgNameConflictError(ValueError):
    ...
    v2 alias for v1 ``newsclaw.orgs.manager.OrgNameConflictError``;
...
class OrgManager:
    ...
    def list_organizations_strict(self) -> list[Organization]:
        ...
        for org_id in self._persistence.list_org_ids():
            raw = self._load_org_dict_with_migrations(org_id)
            if raw is None:
                raise FileNotFoundError(f"Organization disappeared during reference scan: {org_id}")
...
def get_org_manager(data_dir: Path) -> OrgManager:
    ...
    ``newsclaw.orgs.manager.OrgManager`` factory pattern.
    """

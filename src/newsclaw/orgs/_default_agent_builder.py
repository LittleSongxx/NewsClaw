...
* Legacy ``<dispatch>`` text parsing is disabled by default and is available
  only through ``NEWSCLAW_ORG_LEGACY_TEXT_DISPATCH`` for compatibility.
...
from newsclaw.runtime.execution_context import UpstreamContext, current_upstream_context_var
...
_NODE_SYSTEM_PREFIX = "[newsclaw orgs_v2 node agent]"
...
class _BrainBackedNodeAgent:
    ...
    handler in :data:`newsclaw.tools.handlers.default_handler_registry`,
    ...
    async def _maybe_dispatch(
        ...
        *,
        ...
        legacy_enabled = os.environ.get("NEWSCLAW_ORG_LEGACY_TEXT_DISPATCH", "0") in {
            "1",
            "true",
            "yes",
            "on",
            ...
            while remaining_steps:
                ...
                async def _run_ready(
                    ...
                    if request.depends_on:
                        ...
                        upstream_context = UpstreamContext(dependencies=tuple(dependencies))
                        await _emit_plan_event(
    ...
    async def _run_no_tools_streaming(
        ...
        *,
        ...
        try:
            from newsclaw.core.stream_accumulator import StreamAccumulator
    ...
    def _record_stream_usage(self, acc: Any) -> None:
        ...
        try:
            ...
            if not callable(record):
                return
            from newsclaw.llm.types import Usage

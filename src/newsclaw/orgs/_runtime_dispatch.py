...
# dispatch module is imported very early (before ``newsclaw.config.settings``
...
def _resolve_delegation_log_dir() -> Path | None:
    """Locate ``data/delegation_logs/`` without forcing ``newsclaw.config``
    import at module load time."""

    try:
        from newsclaw.config import settings  # local import: avoid cycle

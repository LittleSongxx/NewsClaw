...
Replaces v1 ``newsclaw.orgs.command_service.OrgCommandService``
...
:class:`newsclaw.orgs.node_scheduler.CommandDispatcher`
...
3. **Sprint-9 supervisor HTTP takeover**: every command now runs
   through :class:`newsclaw.runtime.supervisor.Supervisor` built
   by :func:`newsclaw.runtime.supervisor_factory.build_supervisor_for_command`.
   The Sprint-5 wall-clock ``_watchdog_loop`` is gone -- stall
   detection is now LLM-evaluated by the supervisor's
   :class:`~newsclaw.runtime.stall_detector.StallDetector` on
   :class:`~newsclaw.runtime.ledger.ProgressLedger` signals.
   Cancellation is cooperative through the supervisor's
   :class:`~newsclaw.runtime.cancel_token.CancellationToken`.
...
if TYPE_CHECKING:  # pragma: no cover -- import-cycle break
    from newsclaw.runtime.supervisor import Supervisor
    ...
    if root_artifact_path is not None:
        ...
        for index, raw_artifact in enumerate(working_artifacts):
            ...
            if isinstance(declared_paths, list) and any(
                str(path).strip().lower().replace("\\", "/") == root_key for path in declared_paths
            ):
...
class ChannelGatewayProtocol(Protocol):
    ...
    Replaces v1 ``from newsclaw.main import get_message_gateway``
...
class OrgCommandService:
    ...
    async def submit(self, request: OrgCommandRequest) -> dict[str, Any]:
        ...
        if request.target_node_id:
            ...
            if callable(resolve_reference):
                ...
                if (
                    node is None
                    or str(getattr(node, "id", request.target_node_id)) != request.target_node_id
                ):
    ...
    def _build_supervisor_llm_client(self, org_id: str) -> Any | None:
        ...
        wrap the process-shared :func:`newsclaw.llm.client.get_default_client`
        in :class:`~newsclaw.runtime.llm_supervisor_client.GatewaySupervisorLLMClient`,
        ...
        try:
            ...
            from newsclaw.config import settings
            from newsclaw.llm.client import get_default_client
            from newsclaw.runtime.llm_supervisor_client import (
    ...
    def _build_node_directory(self, org_id: str) -> list[Any] | None:
        ...
        :class:`~newsclaw.orgs.org_models.OrgNode` to a
        :class:`~newsclaw.runtime.llm_supervisor_brain.NodeDescriptor`
        ...
        """
        try:
            from newsclaw.runtime.llm_supervisor_brain import NodeDescriptor
    ...
    def _require_org_running(self, org_id: str):  # noqa: ANN202 -- duck-typed
        ...
        # v1 imports OrgStatus from newsclaw.orgs.models; v2 stays
    ...
    def _is_bare_cancel_outcome(outcome: Any) -> bool:
        ...
        *that* shape into a best-effort soft landing -- a run that reached a real
        ...
        """
        try:
            from newsclaw.runtime.supervisor import FinalOutcome
    ...
    def _hard_ceiling_seconds(self, org_id: str | None = None) -> int:
        ...
        Lazy import keeps the ``newsclaw.orgs`` <-> ``newsclaw.config``
    ...
    def _cancel_drain_budget_s() -> int:
        ...
        ``newsclaw.orgs`` <-> ``newsclaw.config`` cycle loose and a

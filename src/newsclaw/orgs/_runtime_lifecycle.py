...
# ``newsclaw.orgs.models.OrgStatus`` semantics.
...
class OrgLifecycleManager:
    ...
    * ``on_stop_org`` -- async callback awaited before the
    ...
    def set_on_stop_org(self, callback: Callable[[str, str], Awaitable[None]] | None) -> None:
    ...
    async def stop_org(self, org_id: str, *, reason: str = "stop") -> bool:
        ...
        async with self._lock:
            ...
            ok = (
                True
                if already_stopped
                else await self._state.transition_org_state(org_id, STATE_STOPPED, reason=reason)
            )

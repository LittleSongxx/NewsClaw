...
class MemoryStorage:
    ...
    Usage:
        storage = MemoryStorage(db_path="data/memory/newsclaw.db")
    ...
    def _init_db(self) -> None:
        ...
        if self._is_sync_folder_path() and os.environ.get("NEWSCLAW_ALLOW_SYNC_FOLDER_DB") != "1":
        ...
        from newsclaw.storage.safe_sqlite import SQLiteUnavailable, safe_open_sync
    ...
    def create_snapshot_incremental(
        ...
        *,
        ...
        if self._conn is None or not self._db_path.exists():
            return None
        if self._is_sync_folder_path() and os.environ.get("NEWSCLAW_FORCE_SNAPSHOT") != "1":
            logger.warning(

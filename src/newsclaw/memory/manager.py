...
def _migrate_legacy_db_name(data_dir: Path) -> None:
    """把 rename 前的数据库文件名就地改成当前名字（一次性）。

    旧安装的库里有真实记忆数据：改名而不是新建，避免用户"记忆消失"。
    ``-wal`` / ``-shm`` 跟随一起搬，否则 SQLite 会认为日志文件缺失。
    """
    ...
    try:
        for suffix in ("", "-wal", "-shm"):
            ...
            if source.exists():
                source.rename(data_dir / f"newsclaw.db{suffix}")
    except OSError as exc:  # 权限/占用：沿用旧文件，不阻断启动
        logger.warning("legacy memory db rename skipped: %s", exc)
...
class MemoryManager:
    ...
    def _ensure_context_vars(self) -> None:
        ...
        if not hasattr(self, "_current_session_id_var"):
            self._current_session_id_var = contextvars.ContextVar(
                f"newsclaw_memory_session_id_{id(self)}",
                default=None,
            )
    ...
    def __init__(
        ...
        self._default_user_id = (
            DESKTOP_USER_ID
            if desktop_owner_alignment and os.environ.get("NEWSCLAW_DESKTOP_SESSION_TOKEN")
            else "default"
        )
        ...
        self._session_turns_var: contextvars.ContextVar[list[ConversationTurn] | None] = (
            contextvars.ContextVar(f"newsclaw_memory_session_turns_{id(self)}", default=None)
        )
        ...
        self._session_cited_memories_var: contextvars.ContextVar[list[dict] | None] = (
            contextvars.ContextVar(f"newsclaw_memory_cited_{id(self)}", default=None)
        )
        ...
        db_path = self.data_dir / "newsclaw.db"
        _migrate_legacy_db_name(self.data_dir)
        try:
...
""".format(timestamp=datetime.now().strftime("%Y-%m-%d %H:%M"))
    ...
    def _sync_profile_fact(self, subject: str, predicate: str, content: str) -> None:
        ...
        if profile_mgr is None:
            return
        try:
            from newsclaw.agent.user_profile import resolve_profile_key
        except Exception:
            return
    ...
    def _ensure_relational(self) -> bool:
        ...
        try:
            ...
            try:
                from newsclaw.config import settings as _cfg
    ...
    def contribute_to_compaction(self, **_kwargs):
        ...
        from newsclaw.runtime.context.continuity import CompactionContribution

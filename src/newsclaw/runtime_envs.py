...
``runtime_env.py`` owns NewsClaw's app/agent/channel/module runtime layout.
...
class ExecutionEnvSpec:
    """Resolved NewsClaw-managed execution environment."""
...
def apply_execution_environment(env: dict[str, str], spec: ExecutionEnvSpec) -> dict[str, str]:
    ...
    merged["NEWSCLAW_EXECUTION_ENV_SCOPE"] = spec.scope
    merged["NEWSCLAW_EXECUTION_ENV_KEY"] = spec.key
    merged["NEWSCLAW_EXECUTION_ENV"] = str(spec.venv_path)
    merged["NEWSCLAW_EXECUTION_PYTHON"] = str(spec.python_path)
    merged["NEWSCLAW_EXECUTION_DEPS_HASH"] = spec.deps_hash
    # Compatibility: existing prompts/tools look for NEWSCLAW_AGENT_PYTHON.
    merged["NEWSCLAW_AGENT_PYTHON"] = str(spec.python_path)
    merged["NEWSCLAW_AGENT_BIN"] = bin_path

...
from newsclaw.optional_assets import load_optional_asset_feature
from newsclaw.utils.atomic_io import atomic_json_write, read_json_safe

OPTIONAL_FEATURE_MARKER = "__NEWSCLAW_OPTIONAL_FEATURE_INSTALL__"
...
def _newsclaw_root() -> Path:
    """数据根目录，解析规则与 settings.newsclaw_home 完全一致。"""
...
def _request_store_path() -> Path:
    return _newsclaw_root() / "data" / "optional_feature_requests.json"
...
def _managed_driver_dir() -> Path:
    return _newsclaw_root() / "modules" / "browser" / "playwright-driver" / _playwright_version()
...
def _select_driver_artifact() -> dict:
    ...
    for artifact in release.get("artifacts", []):
        if isinstance(artifact, dict) and artifact.get("platform") == target:
            ...
            mirror_root = os.environ.get("NEWSCLAW_OPTIONAL_ASSET_MIRROR", "").strip()
...
def _optional_asset_cache() -> Path:
    return _newsclaw_root() / "cache" / "optional-assets"
...
def _download_resumable(
    ...
    *,
    ...
    for url in sources:
        ...
        headers = {"User-Agent": "NewsClaw optional-feature"}
        if offset:
...
def cache_chromium_downloads(progress: Any | None = None) -> Path:
    from newsclaw.optional_assets import resolve_optional_asset_mirror
...
async def install_browser_automation(request_id: str) -> dict:
    ...
    async with lock:
        ...
        try:
            ...
            from newsclaw.tools.browser.manager import (

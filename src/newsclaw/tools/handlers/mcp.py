...
def _build_mcp_envelope(
    *,
    ...
    Mirrors the ``[NEWSCLAW_SOURCE]`` pattern used by web_fetch / browser:
    a single line ``[NEWSCLAW_MCP] {json}`` carrying server, tool, status
    ...
    return f"[NEWSCLAW_MCP] {json.dumps(payload, ensure_ascii=False)}"

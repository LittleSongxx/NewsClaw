...
class ConfigHandler:
    ...
    async def _add_endpoint(self, params: dict) -> str:
        ...
        from newsclaw.llm.types import normalize_context_window
    ...
    async def _probe_endpoint_before_enable(self, endpoint: dict, api_key: str) -> dict:
        ...
        from newsclaw.llm.providers.base import LLMProvider
        from newsclaw.llm.types import normalize_base_url
    ...
    def _ext_status(self) -> str:
        lines = ["## 外部扩展模块\n"]
        lines.append(
            "以下模块为可选外部工具，安装后 NewsClaw 自动检测并启用。\n无需重启，下次对话即生效。\n"
        )
        ...
        lines.append("---")
        lines.append("*安装后无需修改 NewsClaw 配置，系统启动时自动检测 PATH。*")
        return "\n".join(lines)

    def _ext_credits(self) -> str:
        lines = ["## 致谢 — 外部扩展模块\n"]
        lines.append("NewsClaw 的工具调用和浏览器访问能力得益于以下开源项目：\n")

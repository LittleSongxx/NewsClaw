...
def _source_marker(meta: WebFetchMeta, *, tool_name: str = "web_fetch") -> str:
    ...
    return f"[NEWSCLAW_SOURCE] {json.dumps(payload, ensure_ascii=False, sort_keys=True)}"
...
class WebFetchHandler:
    ...
    async def _web_fetch(self, params: dict) -> str:
        ...
        try:
            ...
            return "web_fetch 需要 httpx 库。请在设置中心修复 NewsClaw 运行环境，而不是安装到宿主 Python。"
        ...
        try:
            async with httpx.AsyncClient(
                **get_httpx_client_kwargs(timeout=30),
                ...
                headers={
                    "User-Agent": (
                        "Mozilla/5.0 (compatible; NewsClaw/1.0; "

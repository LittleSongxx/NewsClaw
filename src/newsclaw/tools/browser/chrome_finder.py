...
提供 Chrome 安装检测、NewsClaw 专用 profile 管理、Cookie 同步等工具函数。
...
def get_newsclaw_chrome_profile() -> str:
    """
    获取 NewsClaw 专用的 Chrome profile 目录。
    ...
    profile_dir = base_dir / "NewsClaw" / "ChromeProfile"
...
def sync_chrome_cookies(src_user_data: str, dst_profile: str) -> bool:
    """
    同步用户 Chrome 的 cookies 到 NewsClaw profile。
    只复制关键文件以保持登录状态。
    """
...
def detect_chrome_devtools_mcp() -> dict:
    ...
    # NewsClaw-managed Node first, then host PATH (with macOS login-shell fallback).

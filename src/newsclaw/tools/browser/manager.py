...
def _managed_browsers_dir() -> Path:
    ...
    return resolve_data_root() / "modules" / "browser" / "browsers"
...
async def _download_managed_chromium(
    ...
    *,
    ...
    from newsclaw.optional_features import configure_playwright_driver
...
class BrowserManager:
    ...
    async def _try_user_chrome(self, headless: bool, *, use_oa_profile: bool) -> bool:
        ...
        if use_oa_profile:
            from .chrome_finder import get_newsclaw_chrome_profile, sync_chrome_cookies

            user_data = get_newsclaw_chrome_profile()
            if self._chrome_user_data:
                sync_chrome_cookies(self._chrome_user_data, user_data)
            label = "NewsClaw profile"
        else:

...
class MessageGateway:
    ...
    def __init__(
        ...
        from newsclaw.runtime.cancel_token import CancellationToken as _CT
        ...
        # newsclaw.sessions（避免 fork-style 重写后的循环依赖，ADR-0001）。
        try:
            from newsclaw.runtime.session_bridge import register_session_org_lookup
    ...
    async def _handle_feishu_command(self, cmd: str, message: "UnifiedMessage") -> str | None:
        ...
        if sub == "start":
            if adapter and hasattr(adapter, "get_status_info"):
                ...
                lines = [
                    f"NewsClaw Feishu Adapter v{info['version']}",
        ...
        for raw in artifacts:
            ...
            try:
                ...
                if not source.is_relative_to(workspace_root):
                    ...
                    filename = f"{source.stem}_{digest}{source.suffix}" if digest else source.name
    ...
    def _resolve_install_error_for_adapter(self, adapter_name: str) -> str | None:
        ...
        if not self._channel_install_errors:
            return None
        try:
            from newsclaw.channels.deps import CHANNEL_DEPS
        except Exception:
            return None
    ...
    async def _cancel_v2_dispatch(
        ...
        try:
            ...
            return False
        logger.info(
            "[v2 dispatch] cancel fired session=%s text=%r",
            session_key,
            user_text,
        )
        with contextlib.suppress(Exception):
    ...
    async def _try_dispatch_v2(
        ...
        try:
            ...
            session_key = self._get_session_key(message)
            from newsclaw.runtime.session_bridge import get_org_id_for_session
            ...
            from newsclaw.runtime.channel_routing import dispatch_inbound_message_to_v2
            ...
            logger.warning(
                "[v2 dispatch] _try_dispatch_v2 failed (non-fatal): %s",
                exc,
                exc_info=True,
            )
            return False
    ...
    def _get_org_manager(self):
        ...
        """
        try:
            from newsclaw.orgs.store import get_default_org_manager
        ...
        try:
            from newsclaw.orgs.command_service import get_command_service
    ...
    async def _handle_message(self, message: UnifiedMessage) -> None:
        ...
        try:
            ...
            if (
                ...
                if _new_cwd_match is not None:
                    ...
                    try:
                        ...
                        if not any(
                            is_within(candidate_cwd, root) for root in configured_working_roots()
                        ):
            ...
            try:
                ...
                pass
            _desktop_artifacts = list(session.get_metadata("_pending_desktop_artifacts") or [])
    ...
    async def _send_extracted_media(
        ...
        def reply_kwargs(method: Callable[..., Any]) -> dict[str, str]:
            ...
            try:
                ...
                return {}
            if any(
                parameter.name == "reply_to" or parameter.kind is inspect.Parameter.VAR_KEYWORD

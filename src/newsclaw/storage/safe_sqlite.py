...
class SQLiteUnavailable(Exception):
    ...
    :class:`newsclaw.memory.exceptions.MemoryStorageUnavailable`.
...
def _is_sync_folder_path(path: Path) -> bool:
    ...
    Override with ``NEWSCLAW_ALLOW_SYNC_FOLDER_DB=1`` for power users
    who accept the risk.
    """
    text = str(path).lower()
    if os.environ.get("NEWSCLAW_ALLOW_SYNC_FOLDER_DB") == "1":
        return False

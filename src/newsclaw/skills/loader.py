...
DEFAULT_DISABLED_SKILLS: frozenset[str] = frozenset(
    {
        # 与 pyproject.toml 的 builtin_skills/external 清单、skills/catalog.json
        # 保持一致（改名前的旧清单里 50+ 个技能已移入 attic/，留在集合里只会
        # 让"默认启用集"和实际打包集对不上）。预设 agent 引用的技能仍会被加载，
        # 只是默认不启用，用户在技能面板勾选后写入 data/skills.json 生效。
        "chinese-writing",
...
class SkillLoader:
    ...
    def _load_i18n(self, skill_dir: Path, metadata: SkillMetadata) -> None:
        ...
        优先 agents/openai.yaml 的 i18n 字段，回退 .newsclaw-i18n.json。
        """
    ...
    def run_script(
        ...
        if script_path.suffix == ".py":
            ...
            from newsclaw.runtime_manager import get_python_executable

...
class SkillParser:
    ...
    def _build_metadata(self, data: dict, path: Path, body: str = "") -> SkillMetadata:
        ...
        # Extract metadata.newsclaw structured fields (legacy key tolerated)
        raw_metadata = data.get("metadata", {})
        akita_meta: dict = {}
        if isinstance(raw_metadata, dict):
            # ``metadata.newsclaw`` 是当前键名；rename 前发布的技能写的是
            # ``metadata.openakita``，仍在市场里流通，必须继续解析。
            for key in ("newsclaw", "openakita"):
                candidate = raw_metadata.get(key, {})
                if isinstance(candidate, dict) and candidate:
                    akita_meta = candidate
                    break
    ...
    def validate(self, skill: ParsedSkill) -> list[str]:
        ...
        # metadata names identify the upstream skill, while NewsClaw uses the

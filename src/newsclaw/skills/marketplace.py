...
stable representation consumed by NewsClaw's API, desktop bridge, and UI.
"""
...
MARKETPLACE_INSTALL_RECORD = ".newsclaw-marketplace.json"
LEGACY_MARKETPLACE_INSTALL_RECORD = ".openakita-marketplace.json"  # rename 前写入的记录
...
def installed_marketplace_names(
    ...
    if jobs_dir is None:
        ...
        jobs_dir = settings.newsclaw_home / "marketplace" / "jobs"
    ...
    for skill_path in skill_paths:
        ...
        if (
            ...
            continue
        record_file = directory / MARKETPLACE_INSTALL_RECORD
        if not record_file.is_file():
            record_file = directory / LEGACY_MARKETPLACE_INSTALL_RECORD
        receipt = _read_local_metadata(record_file)
...
def normalize_skillhub_response(
    ...
    *,
    ...
    """Convert a SkillHub response to NewsClaw's marketplace contract."""

...
兼容 rename 前写入的 .openakita-i18n.json sidecar 文件。
...
I18N_FILENAME = ".newsclaw-i18n.json"
LEGACY_I18N_FILENAME = ".openakita-i18n.json"  # rename 前的 sidecar 名，继续读取
...
def read_i18n(skill_dir: Path) -> dict[str, dict[str, str]]:
    ...
    回退到 rename 前的 .openakita-i18n.json 格式。
...
def _read_i18n_from_json(skill_dir: Path) -> dict[str, dict[str, str]]:
    """读取 JSON sidecar（当前文件名，回退到 rename 前的名字）。"""

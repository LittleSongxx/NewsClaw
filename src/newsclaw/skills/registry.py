...
# newsclaw.api.server`` after the upstream merge reordered the agents imports).
from newsclaw.agent.capabilities import (
...
class SkillEntry:
    ...
    # metadata.newsclaw 结构化字段（rename 前为 metadata.openakita，仍兼容）
    ...
    # 技能来源 URL（来自 .newsclaw-source 文件，用于区分同名技能）
    ...
    # 国际化（由 agents/openai.yaml i18n 字段注入，兼容旧的 .newsclaw-i18n.json）
    ...
    def from_parsed_skill(
        ...
        *,
        ...
        if skill.path:
            ...
            source_file = parent / ".newsclaw-source"
            if not source_file.is_file():
                # rename 前的 sidecar 名，市场里已装好的技能仍在用
                legacy = parent / ".openakita-source"
                source_file = legacy if legacy.is_file() else source_file
            try:
    ...
    def find_relevant(self, context: str) -> list[SkillEntry]:
        ...
        for skill in self._skills.values():
            ...
            if (
                ...
                or skill.marketplace_name
                and skill.marketplace_name.lower() in context_lower
            ):
                score += 10

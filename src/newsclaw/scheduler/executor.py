...
def _resolve_policy_confirmation_mode():
    ...
    与 ``policy_v2.adapter._build_fallback_context`` 同一语义：调度任务属于
    owner 在无人值守下代为执行，确认模式应与交互入口一致，而不是落到
    "default"（那会让所有写入类工具在无人值守时被拒）。
    """
    from newsclaw.core.policy_v2.context import ConfirmationMode

    try:
        from newsclaw.core.policy_v2 import get_config_v2

        raw = get_config_v2().confirmation.mode
    except Exception:  # 配置不可用时退回引擎默认，保持既有行为
        return ConfirmationMode.DEFAULT
    if isinstance(raw, ConfirmationMode):
        return raw
    try:
        return ConfirmationMode(str(raw))
    except ValueError:
        return ConfirmationMode.DEFAULT
...
只回复 NO_ACTION 或 NEEDS_ACTION，不要有其他内容。"""
    ...
    async def _execute_complex_task_core(
        ...
        try:
            ...
            # 边界，与对话路径一致。
            try:
                from newsclaw.core.policy_v2 import get_config_v2 as _get_cfg
            ...
            _policy_ctx = PolicyContext(
                ...
                # 确认模式必须与 POLICIES.yaml 一致（chat/IM 入口均如此取值）。
                # 漏传会落到引擎内置 default：写类工具（write_file 等
                # MUTATING_SCOPED）在矩阵里一律 CONFIRM，而无人值守下
                # ask_owner 无人应答即拒绝——定时任务的产物写入会全部失败。
                confirmation_mode=_resolve_policy_confirmation_mode(),
                is_unattended=True,

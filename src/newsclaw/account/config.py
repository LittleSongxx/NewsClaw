...
class AccountFeatureConfig:
    """Resolved account provider policy for one NewsClaw distribution."""
    ...
    def from_env(cls, environ: Mapping[str, str] | None = None) -> AccountFeatureConfig:
        ...
        mode = values.get("NEWSCLAW_ACCOUNT_MODE", "disabled").strip().lower()
        if mode not in ACCOUNT_MODES:
            allowed = ", ".join(sorted(ACCOUNT_MODES))
            raise ValueError(f"NEWSCLAW_ACCOUNT_MODE must be one of: {allowed}")
        if mode == "disabled":
            return cls(
                mode=mode,
                enabled=False,
        ...
        if mode == "newsclaw":
            ...
            provider = "newsclaw"
            display_name = values.get("NEWSCLAW_ACCOUNT_DISPLAY_NAME", "NewsClaw Account").strip()
            namespace = values.get("NEWSCLAW_ACCOUNT_CREDENTIAL_NAMESPACE", "newsclaw").strip()
        else:
            raw_base_url = values.get("NEWSCLAW_ACCOUNT_BASE_URL", "").strip()
            client_id = values.get("NEWSCLAW_ACCOUNT_CLIENT_ID", "").strip()
            if not raw_base_url:
                raise ValueError(
                    "NEWSCLAW_ACCOUNT_BASE_URL is required when NEWSCLAW_ACCOUNT_MODE=custom"
                )
            if not client_id:
                raise ValueError(
                    "NEWSCLAW_ACCOUNT_CLIENT_ID is required when NEWSCLAW_ACCOUNT_MODE=custom"
                )
            base_url = _validated_base_url(
                raw_base_url,
                variable="NEWSCLAW_ACCOUNT_BASE_URL",
            )
            provider = values.get("NEWSCLAW_ACCOUNT_PROVIDER", "custom").strip().lower()
            display_name = values.get("NEWSCLAW_ACCOUNT_DISPLAY_NAME", "Account").strip()
            namespace = values.get("NEWSCLAW_ACCOUNT_CREDENTIAL_NAMESPACE", "").strip()
            if not namespace:
                namespace = _derived_custom_namespace(base_url, client_id)

        if not client_id:
            raise ValueError("NEWSCLAW_ACCOUNT_CLIENT_ID must not be empty")
        if not display_name:
            raise ValueError("NEWSCLAW_ACCOUNT_DISPLAY_NAME must not be empty")
        if not _PROVIDER_PATTERN.fullmatch(provider):
            raise ValueError("NEWSCLAW_ACCOUNT_PROVIDER contains unsupported characters")
        if not _NAMESPACE_PATTERN.fullmatch(namespace):
            raise ValueError(
                "NEWSCLAW_ACCOUNT_CREDENTIAL_NAMESPACE contains unsupported characters"
            )
    ...
    def identity_data_dir(self) -> Path:
        ...
        from ..data_root import resolve_home_root

        return resolve_home_root() / "account" / namespace
...
def disabled_credential_usernames(
    ...
    base_url = values.get("NEWSCLAW_ACCOUNT_BASE_URL", "").strip()
    client_id = values.get("NEWSCLAW_ACCOUNT_CLIENT_ID", "").strip()
    if base_url and client_id:
        try:
            normalized = _validated_base_url(base_url, variable="NEWSCLAW_ACCOUNT_BASE_URL")
        except ValueError:
            pass
        else:

"""PKCE and RFC 8628 device authorization with NewsClaw Account."""
    ...
    async def load_refresh_token(self) -> str | None:
        def _load() -> str | None:
            try:
                ...
                # keyring 未安装（base 依赖不含它，属 desktop/all 可选组）。
                # 读取路径按"无已保存令牌"降级为未登录，而不是抛错——
                # 状态页轮询会反复调用本方法，抛错会在 headless 环境刷堆栈。
                logger.debug("[Account] keyring not installed; treating as signed_out")
                return None
            try:
                return keyring.get_password(self.service, self.username)
            except keyring.errors.KeyringError:
                # keyring 已安装但无可用后端（headless Linux / WSL 无 Secret
                # Service）同样降级为未登录。
                logger.debug("[Account] no usable keyring backend; treating as signed_out")
                return None
    ...
    copy = {
        "zh": {
            ...
            "message": (
                "你已成功登录 NewsClaw。"
                if success
                else "登录过程中遇到问题，请返回 NewsClaw 重新登录。"
            ),
            "hint": "现在可以关闭此页面。",
        },
        "en": {
            ...
            "message": (
                "You’re now signed in to NewsClaw."
                if success
                else "Something went wrong. Return to NewsClaw and try signing in again."
            ),
...
<head>
  ...
  <title>{text["page_title"]} · NewsClaw</title>
  <style>
...
class AccountOIDCManager:
    def __init__(
        ...
        *,
        ...
        self._base_url = (
            account_base_url
            or os.environ.get("NEWSCLAW_ACCOUNT_BASE_URL", DEFAULT_ACCOUNT_BASE_URL)
        ).rstrip("/")
        self._client_id = (
            client_id or os.environ.get("NEWSCLAW_ACCOUNT_CLIENT_ID", DEFAULT_ACCOUNT_CLIENT_ID)
        ).strip()
        ...
        if isinstance(self._tokens, KeyringTokenStore):
            ...
            self._vault_lock = FileLock(
                str(resolve_data_root() / "account" / f"{namespace}.lock"),
                thread_local=False,
            )
        ...
        *,
        ...
        async with self._login_lock:
            ...
            try:
                ...
                logger.warning(
                    "NewsClaw Account login failed: %s",
    ...
    async def snapshot(self) -> dict:
        ...
        async with self._credentials():
            try:
                ...
                # The token store itself may also be the failure source
                # (e.g. unusable keyring backend) — guard the probe so the
                # status endpoint degrades to a payload instead of a 500.
                try:
                    if not await self._tokens.load_refresh_token():
                        return {"status": "signed_out"}
                except AccountOIDCError:
                    return {"status": "unavailable", "status_reason": "token_store_unavailable"}

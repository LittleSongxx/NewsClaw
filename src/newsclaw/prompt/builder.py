...
def _load_prompt_asset(relative_path: str) -> str:
    ...
    try:
        prompt_path = resources.files("newsclaw").joinpath("prompts", *parts)
...
def _has_delegation_tools(tool_catalog: Optional["ToolCatalog"]) -> bool:
    """该 Agent 的工具集里是否有委派工具。

    用于避免向"没有委派能力"的 Agent 注入整章协作规则：既白烧 token，
    又会诱导模型调用不存在的工具（NewsClaw 收敛后按 profile 白名单裁剪，
    例如早报管线 Agent 不持有 delegate_*）。
    """
    if tool_catalog is None:
        return False
    raw = getattr(tool_catalog, "_tools", None)
    # ToolCatalog 内部是 {name: tool} 的 dict；同时兼容 list/set 形态，
    # 以便未来重构或测试替身不会静默失效（宁可多注入也不漏判能力）。
    if isinstance(raw, dict):
        names = set(raw)
    elif isinstance(raw, (list, tuple, set)):
        names = {t.get("name") if isinstance(t, dict) else str(t) for t in raw}
    else:
        return False
    return bool(names & {"delegate_to_agent", "delegate_parallel", "spawn_agent"})


def _build_delegation_rules() -> str:
    """协作优先原则（多 Agent 委派），仅在非子 Agent 且有委派工具的 agent 模式下注入。"""
    return (
        "## 协作优先原则\n\n"
...
def build_system_prompt(
    ...
    Args:
        ...
        agent_voice: 当前 Agent 的显示名，用于替换 SOUL.md / NONE-mode 中的
            ``{{agent_name}}`` 占位符。空字符串时回退到 "NewsClaw"。
    ...
    system_parts.append(
        "## Runtime context\n\n"
        "NewsClaw may supply a [NewsClaw runtime context] message before the current "
    ...
    if prompt_mode in (PromptMode.FULL, PromptMode.MINIMAL):
        ...
        if (
            ...
            and mode == "agent"
            and _has_delegation_tools(tool_catalog)
        ):
    ...
    if prompt_mode in (PromptMode.FULL, PromptMode.MINIMAL) and session_context:
        try:
            from newsclaw.agent.working_facts import format_working_facts
    ...
    if _memory_scope in {"pinned_only", "relevant", "full"} and prompt_mode in (
        ...
        if precomputed_memory is not None:
            ...
            #  2) 用户显式设置 NEWSCLAW_PROMPT_VERBOSE_MEMORY_GUIDE=1。
            # 短窗口 / CONSUMER_CHAT 一律 compact —— 这些场景模型通常按 token 数计费，
            # 也是用户最容易感知"慢"的地方。
            _verbose_env = os.environ.get("NEWSCLAW_PROMPT_VERBOSE_MEMORY_GUIDE", "").strip()
...
_DEFAULT_AGENT_VOICE = "NewsClaw"
_DEFAULT_BASE_PROMPT_SELF_INTRO = "你是 NewsClaw，一个帮助用户完成各类任务的 AI 助手。"
...
def _build_identity_section(
    ...
    "NewsClaw" 这一个自称上。空字符串时回退到 ``_DEFAULT_AGENT_VOICE``。
    """
    parts = []
    ...
    if resolved_voice != _DEFAULT_AGENT_VOICE:
        parts.append(
            f"当前 Agent 的自称是「{resolved_voice}」。NewsClaw 仅指运行平台或上游"
...
- `run_shell`、`pip install`、打开带窗口的程序、浏览器自动化等：**全部发生在当前 NewsClaw 进程所在的主机及其图形会话/无头环境中**。
- **默认不等于**用户发消息时所用的设备：IM/手机、另一台电脑、飞书/钉钉客户端所在环境与此**不是同一执行域**；图形窗口**不会**自动出现在用户屏幕上，软件也**不会**自动装到用户个人电脑上。
- 若用户要的是「在我这台电脑上看到窗口 / 本机安装 / 游戏内 overlay」等**用户侧可观测效果**：须通过 **可交付产物**（如脚本、`deliver_artifacts`）、**用户在本机可复制执行的命令/步骤**，或说明需要 **本地运行的 NewsClaw / 远程桌面到同一台机器** 等产品能力；**禁止**仅因宿主侧命令退出码为 0 就声称用户已在其设备上看到效果。
...
def _build_arch_section(
    ...
    if is_sub_agent:
        return (
            f"## 系统概况\n\n"
            f"你是 NewsClaw 多 Agent 系统中的**子 Agent**{model_part}。\n"
    ...
    lines = ["## 系统概况\n"]
    lines.append(f"你运行在 NewsClaw 多 Agent 系统中{model_part}。核心架构：")
...
def _detect_deploy_mode() -> str:
    ...
    try:
        dist = importlib.metadata.distribution("newsclaw")
    ...
    # 检查是否通过 pip 安装
    try:
        importlib.metadata.version("newsclaw")
        return "pip install"
    except Exception:
        pass
...
def _build_memory_section(
    ...
    from newsclaw.memory.types import MEMORY_MD_MAX_CHARS as _MD_MAX
...
def _get_core_memory(memory_manager: Optional["MemoryManager"], max_chars: int = 600) -> str:
    ...
    """
    from newsclaw.memory.types import truncate_memory_md

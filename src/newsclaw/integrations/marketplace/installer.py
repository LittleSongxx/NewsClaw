...
from newsclaw import __version__
from newsclaw.config import settings
from newsclaw.utils.atomic_io import atomic_json_write, read_json_safe
...
class MarketplaceInstallManager:
    def __init__(self, root: Path | None = None) -> None:
        data_root = (
            ...
            if root is not None
            else settings.newsclaw_home / "marketplace"
        )
        self.root = data_root
    ...
    async def _authorize(
        ...
        *,
        ...
        from newsclaw.account.oidc import AccountOIDCError
        ...
        async with self._lock:
            ...
            for other in self._jobs.values():
                if (
                    ...
                    and (
                        other.get("resource_id") == job["resource_id"]
    ...
    async def _run(self, job_id: str, request: Any) -> None:
        ...
        try:
            ...
            await self._fail(
                job, "marketplace_install_failed", detail=f"{type(exc).__name__}: {exc}"
            )
        finally:
    ...
    async def _fail(
        ...
        *,
        ...
        from newsclaw.plugins.installer import safe_install_error
        ...
        logger.error(
            "Marketplace install %s failed (%s): %s", job["id"], code, job["failure_detail"] or code
        )
    ...
    def _verify(job: dict[str, Any], package_path: Path) -> None:
        ...
        if list(manifest.get("dependencies") or []) != list(job.get("dependencies") or []):
            raise MarketplaceInstallError("marketplace_manifest_mismatch")
        requirement = str(manifest.get("newsclaw") or "").strip()
        if requirement:
            try:
    ...
    async def _install_plugin(
        ...
        from newsclaw.api.routes.plugins import (
            InstallProgress,
            _do_install,
        ...
        async def relay() -> None:
            while True:
                ...
                if job is not None:
                    job.update(
                        stage=snapshot["stage"], current_dependency=snapshot.get("dependency", "")
                    )
        ...
        try:
            ...
            from newsclaw.plugins.installer import PluginInstallError, safe_install_error
            ...
            detail = (
                str(exc) if isinstance(exc, PluginInstallError) else f"{type(exc).__name__}: {exc}"
            )
            raise MarketplaceInstallError(
                "marketplace_plugin_install_failed",
                safe_install_error(detail),
                reason=reason,
            ) from exc
        finally:
            relay_task.cancel()
    ...
    async def _install_skill(
        ...
        *,
        ...
        try:
            ...
            from newsclaw.skills.marketplace import MARKETPLACE_INSTALL_RECORD
            ...
            if (
                marketplace_manifest is not None
                and isinstance(resource_name, str)
                and resource_name.strip()
            ):
                await asyncio.to_thread(
                    atomic_json_write,
                    staging / MARKETPLACE_INSTALL_RECORD,
                    {
            ...
            placed = True
            from newsclaw.api.routes.skills import _propagate
            ...
            if first_install:
                ...
                await asyncio.to_thread(
                    MarketplaceInstallManager._enable_new_skill,
                    resource_slug,
                )
        ...
        from newsclaw.api.routes.skills import _resolve_agent
        from newsclaw.skills.allowlist_io import read_allowlist
        from newsclaw.skills.loader import DEFAULT_DISABLED_SKILLS

        _, allowlist = await asyncio.to_thread(read_allowlist)
        enabled = (
            resource_slug in allowlist
        ...
        hot_loaded = (
            ...
            and entry.skill_path is not None
    ...
    def _enable_new_skill(skill_id: str) -> None:
        from newsclaw.skills.allowlist_io import read_allowlist, upsert_skill_ids
        from newsclaw.skills.loader import DEFAULT_DISABLED_SKILLS, SkillLoader
    ...
    def _bind_initial_skill_category(skill_id: str, category: Any, request: Any) -> None:
        ...
        from newsclaw.skills.category_store import CategoryStore
        ...
        if (entry is not None and entry.system_readonly) or (
            loader is not None
            and any(
                skill.system and skill.category == category for skill in loader.registry.list_all()
            )
        ):
        ...
        if store is None:
            store = CategoryStore(
                settings.project_root / "data" / "skills" / "skill_categories.json"
            )
    ...
    async def _install_mcp(source: Path, request: Any) -> None:
        ...
        if transport != "stdio" and not url:
            raise MarketplaceInstallError("marketplace_mcp_manifest_invalid")
        from newsclaw.api.routes.mcp import _get_mcp_catalog, _get_mcp_client
        ...
        from newsclaw.tools.mcp_workspace import add_server_to_workspace

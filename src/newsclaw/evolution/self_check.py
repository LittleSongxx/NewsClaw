    ...
    async def _execute_fix_by_llm_decision(self, analysis: dict, max_retries: int = 1) -> FixRecord:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                # NewsClaw 核心系统代码（core/llm/memory/scheduler/storage/agents 等）一律不允许自动修改。
                #
                ...
                agent._selfcheck_fix_policy = {
                    ...
                    "write_roots": [
                        ...
                        str((project_root / "src" / "newsclaw" / "tools").resolve()),
                        str((project_root / "src" / "newsclaw" / "channels").resolve()),
                    ],
...
2. **禁止** 修改 NewsClaw 核心系统代码（`src/newsclaw/core/`、`src/newsclaw/llm/`、`src/newsclaw/memory/`、`src/newsclaw/scheduler/`、`src/newsclaw/storage/`、`src/newsclaw/agents/` 等）
    ...
    async def _fix_timeout_error(self, sample) -> bool:
        ...
        try:
            ...
            if platform.system() == "Windows":
                ...
                # Linux/Mac 下清理僵尸进程
                await self.shell.run("pkill -9 -f 'newsclaw.*timeout' || true")

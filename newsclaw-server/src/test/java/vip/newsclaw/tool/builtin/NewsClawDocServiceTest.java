 ...
 */
class NewsClawDocServiceTest {

    private final NewsClawDocService service = new NewsClawDocService();

    @Test
    @DisplayName("list(zh) 返回文档且排除 VitePress 首页 index.md")
    void listExcludesIndex() {
        List<NewsClawDocService.DocMeta> docs = service.list("zh");

        assertThat(docs).isNotEmpty();
        assertThat(docs).noneMatch(d -> d.slug().equals("index"));
        // ai-news-ops.md 一定存在，且标题取的是中文 H1 而非文件名。
        assertThat(docs)
                .filteredOn(d -> d.slug().equals("ai-news-ops"))
                .singleElement()
                .satisfies(d -> assertThat(d.title()).isNotBlank().isNotEqualTo("ai-news-ops"));
        assertThat(docs).noneMatch(d -> d.slug().equals("multimodal"));
    }
    ...
    void readStripsFrontmatter() {
        // intro.md 带 frontmatter（title/description/head）。
        String body = service.read("zh", "intro");
    ...
    void readRejectsInvalidSlug() {
        ...
        assertThat(service.read("zh", "multimodal")).isNull();
    }
    ...
    void readRawForToolContract() {
        ...
        assertThat(service.readRawForTool("zh/multimodal.md")).startsWith("Error:");

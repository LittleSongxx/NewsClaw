...
class GzhPackageCoverHealingTest {
    ...
    @Test
    @DisplayName("high-risk compliance hit blocks before files and content-ledger writes")
    void highRiskScanBlocksBeforeDelivery() {
        ...
        assertTrue(out.contains("公众号合规门禁阻断"), "preflight gate must block:\n" + out);
        assertTrue(out.contains("合规扫描命中"), "scan report must explain the block:\n" + out);
        assertTrue(out.contains("广告法极限词") || out.contains("微信诱导"), "should flag the violation");
        assertFalse(out.contains("在线预览"), "blocked content must not mint a preview");
        assertFalse(out.contains("已记入内容日历"), "blocked content must not look deliverable");
        verifyNoInteractions(contentSvc);
    }

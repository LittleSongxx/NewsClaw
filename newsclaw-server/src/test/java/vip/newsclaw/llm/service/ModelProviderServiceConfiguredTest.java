...
class ModelProviderServiceConfiguredTest {
    ...
    @Test
    @DisplayName("环境 Key 可让数据库 disabled 的 Provider 出现在 enabled 视图")
    void environmentCredentialEnablesProviderProjection() {
        String previous = System.getProperty("BAILIAN_API_KEY");
        try {
            System.setProperty("BAILIAN_API_KEY", "test-bailian-key");
            ModelProviderEntity p = cloud("bailian-team", true);
            p.setEnabled(false);
            p.setApiKey("");
            seedProviderRow(p, true);

            List<ProviderInfoDTO> providers = service.listProviders();

            assertEquals(1, providers.size());
            assertEquals("bailian-team", providers.get(0).getId());
            assertTrue(providers.get(0).getConfigured());
            assertTrue(providers.get(0).getEnabled());
        } finally {
            if (previous == null) {
                System.clearProperty("BAILIAN_API_KEY");
            } else {
                System.setProperty("BAILIAN_API_KEY", previous);
            }
        }
    }

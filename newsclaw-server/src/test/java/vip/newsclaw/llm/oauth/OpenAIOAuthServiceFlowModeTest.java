...
import vip.newsclaw.llm.oauth.OpenAIOAuthService.OAuthFlowMode;
 ...
 * the remote NewsClaw server, so the auth code never reaches the server.
 *
...
class OpenAIOAuthServiceFlowModeTest {
    ...
    @AfterEach
    void clearOverride() {
        System.clearProperty("newsclaw.oauth.openai.deployment-mode");
    }
    ...
    void publicHosts_resolveToDeviceCode() throws Exception {
        assertEquals(OAuthFlowMode.DEVICE_CODE, invokeResolve("newsclaw.example.com"));
        assertEquals(OAuthFlowMode.DEVICE_CODE, invokeResolve("api.newsclaw.vip"));
        assertEquals(OAuthFlowMode.DEVICE_CODE, invokeResolve("api.newsclaw.vip:443"));
    ...
    @Test
    @DisplayName("config override newsclaw.oauth.openai.deployment-mode=local forces LOCAL even on remote host")
    void configOverride_forcesLocal() throws Exception {
        System.setProperty("newsclaw.oauth.openai.deployment-mode", "local");
        assertEquals(OAuthFlowMode.LOCAL, invokeResolve("newsclaw.example.com"));
    }
    ...
    void configOverride_forcesDeviceCode() throws Exception {
        System.setProperty("newsclaw.oauth.openai.deployment-mode", "device_code");
        ...
        System.setProperty("newsclaw.oauth.openai.deployment-mode", "server");
        assertEquals(OAuthFlowMode.DEVICE_CODE, invokeResolve("localhost"));
    }
    ...
    void configOverride_forcesManualPaste() throws Exception {
        System.setProperty("newsclaw.oauth.openai.deployment-mode", "manual_paste");
        assertEquals(OAuthFlowMode.MANUAL_PASTE, invokeResolve("localhost"));
        assertEquals(OAuthFlowMode.MANUAL_PASTE, invokeResolve("api.newsclaw.vip"));
    }

    @Test
    @DisplayName("config override 'auto' or unknown falls back to heuristic")
    void configOverride_autoFallsThrough() throws Exception {
        System.setProperty("newsclaw.oauth.openai.deployment-mode", "auto");
        assertEquals(OAuthFlowMode.LOCAL, invokeResolve("localhost"));
        assertEquals(OAuthFlowMode.DEVICE_CODE, invokeResolve("api.newsclaw.vip"));

        System.setProperty("newsclaw.oauth.openai.deployment-mode", "garbage");
        assertEquals(OAuthFlowMode.LOCAL, invokeResolve("localhost"));
    }
    ...
    void pastedUrl_emptyRejected() {
        assertThrows(NewsClawException.class, () -> service.completeFromPastedUrl(null));
        assertThrows(NewsClawException.class, () -> service.completeFromPastedUrl(""));
        assertThrows(NewsClawException.class, () -> service.completeFromPastedUrl("   "));
    }

...
class ObservationNodeLoopGuardTest {
    ...
    @SuppressWarnings("unchecked")
    private static List<vip.newsclaw.agent.GraphEventPublisher.GraphEvent> warningEvents(Map<String, Object> out) {
        var events = (List<vip.newsclaw.agent.GraphEventPublisher.GraphEvent>) out.get(PENDING_EVENTS);
        if (events == null) return List.of();
        return events.stream()
                .filter(e -> vip.newsclaw.agent.GraphEventPublisher.EVENT_WARNING.equals(e.type()))
                .toList();
    }

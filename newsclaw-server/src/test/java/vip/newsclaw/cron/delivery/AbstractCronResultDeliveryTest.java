...
import vip.newsclaw.external.effect.ExternalEffectEntity;
...
class AbstractCronResultDeliveryTest {
    ...
    @Test
    void deliver_externalEffectAlreadySucceeded_doesNotInvokePlatformAgain() {
        when(runMapper.update(any(), any(Wrapper.class))).thenReturn(1, 1);
        ExternalEffectService ledger = mock(ExternalEffectService.class);
        ExternalEffectEntity effect = new ExternalEffectEntity();
        effect.setTarget("feishu-chat-1");
        when(ledger.claim(any())).thenReturn(new ExternalEffectService.ClaimResult(
                ExternalEffectService.ClaimStatus.ALREADY_SUCCEEDED, effect, null));
        AtomicReference<Boolean> delivered = new AtomicReference<>(false);

        AbstractCronResultDelivery strategy = new AbstractCronResultDelivery(runMapper, ledger) {
            @Override public boolean supports(CronJobEntity j) { return true; }
            @Override
            protected DeliveryOutcome doDeliver(CronJobEntity j, AssistantMessage r, CronJobRunEntity run) {
                delivered.set(true);
                return DeliveryOutcome.delivered("should-not-send");
            }
        };
        ...
        assertEquals(DeliveryOutcome.Status.SKIPPED, outcome.status());
        assertEquals("external-effect-already-succeeded", outcome.reason());
        assertFalse(delivered.get(), "the known-success ledger row must suppress a second IM call");
        verify(ledger, never()).markSucceeded(any(), any(), any());
    }

    @Test
    void deliver_platformFailureMarksLedgerButDoesNotRewriteCompletedRunAsFailed() {
        when(runMapper.update(any(), any(Wrapper.class))).thenReturn(1, 1);
        ExternalEffectService ledger = mock(ExternalEffectService.class);
        ExternalEffectEntity effect = new ExternalEffectEntity();
        effect.setId(99L);
        ExternalEffectService.ClaimResult claim = new ExternalEffectService.ClaimResult(
                ExternalEffectService.ClaimStatus.ACQUIRED, effect, "worker-1");
        when(ledger.claim(any())).thenReturn(claim);
        ...
        verify(ledger).markFailed(eq(claim), any(IllegalStateException.class));
        verify(runMapper, times(2)).update(any(), any(Wrapper.class));
        assertEquals("succeeded", run.getStatus(), "IM failure is an external-effect failure, not a content-run failure");
    }
}

...
class TeamDispatchServiceTest {
    ...
    @Test
    @DisplayName("legacy plain generated URLs are attached with inferred package names")
    void settleLegacyPlainGeneratedUrlsAttachesDeliverables() {
        TeamTaskEntity running = task(1L, MEMBER_A);
        running.setStatus(TeamTaskStatus.IN_PROGRESS);
        running.setMetadata("{\"deliverableRequired\":true}");
        TeamTaskEntity completed = task(1L, MEMBER_A);
        ...
        String reply = "在线预览 URL: http://localhost:18080/api/v1/files/generated/preview-1\n"
                + "素材 ZIP URL: http://localhost:18080/api/v1/files/generated/package-1";
        service.settleOutcome(running, reply);

        verify(taskService).addDeliverable(1L, MEMBER_A, "内容预览.html",
                "http://localhost:18080/api/v1/files/generated/preview-1");
        verify(taskService).addDeliverable(1L, MEMBER_A, "内容素材.zip",
                "http://localhost:18080/api/v1/files/generated/package-1");
        verify(taskService).completeTask(1L, null, reply);
    }
    ...
    void runTaskCreatesChildConversationInTeamWorkspace() {
        ...
        when(taskService.getTask(1L)).thenReturn(assigned, done, done);
        when(taskService.completeTask(eq(1L), isNull(), anyString())).thenReturn(List.of());
        when(agentService.chatWithUsage(eq(MEMBER_A), anyString(), anyString(), any(ChatOrigin.class)))
                .thenReturn(AgentService.ChatResult.contentOnly("all done"));
        ...
        verify(conversationService).createChildConversation(
                ...
                eq("lead-conv"),
                eq("team_worker"));
        var origin = forClass(ChatOrigin.class);
        verify(agentService).chatWithUsage(eq(MEMBER_A), anyString(), startsWith("team-task-"), origin.capture());
        assertEquals(WORKSPACE_ID, origin.getValue().workspaceId());
    }

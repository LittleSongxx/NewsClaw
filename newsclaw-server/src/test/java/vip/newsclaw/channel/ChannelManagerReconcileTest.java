...
class ChannelManagerReconcileTest {
    ...
    void setUp() {
        ...
        manager = new ChannelManager(
                ...
                new ObjectMapper(),
                mock(vip.newsclaw.tool.document.GeneratedFileCache.class),
                mock(vip.newsclaw.channel.notification.ApprovalNotificationService.class),
                mock(vip.newsclaw.channel.wecom.cards.WeComCardDispatcher.class),
                mock(vip.newsclaw.channel.wecom.WeComKeepaliveScheduler.class),
                mock(vip.newsclaw.channel.feishu.FeishuMediaUploader.class),
                mock(vip.newsclaw.channel.media.GeneratedFileScrubber.class),
                mock(vip.newsclaw.channel.feishu.FeishuStreamingCardManager.class),
                mock(vip.newsclaw.channel.feishu.cards.FeishuCardDispatcher.class),
                mock(vip.newsclaw.channel.feishu.FeishuClientFactory.class),
                mock(vip.newsclaw.stt.SttService.class),
                election,
                mock(vip.newsclaw.workspace.core.service.ChatUploadLocationResolver.class));
    ...
    void reconcileStopsOnNotFound() {
        ...
        when(channelService.getChannel(43L))
                .thenThrow(new NewsClawException("err.channel.not_found", "渠道不存在: 43"));
    ...
    void followerRetryCancelsOnNotFound() {
        ...
        when(channelService.getChannel(99L))
                .thenThrow(new NewsClawException("err.channel.not_found", "渠道不存在: 99"));

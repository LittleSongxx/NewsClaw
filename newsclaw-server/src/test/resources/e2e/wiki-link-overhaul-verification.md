...
Manual / scripted verification against a live `newsclaw-server` instance for
...
| Auth | `POST /api/v1/auth/login` with the deployment-provided bootstrap password |
...
grep -rn '\[\[页面标题\]\]\|\[\[Title\]\]\|\[\[wikilinks\]\]' \
  newsclaw-server/src/main/resources/prompts/wiki/
```
...
Set `newsclaw.wiki.cascade-delete-enabled=false` in application properties (or
...
- **§4.4 feature-flag kill-switch**: requires a server restart with `newsclaw.wiki.cascade-delete-enabled=false`; out of band for a single live verification pass.

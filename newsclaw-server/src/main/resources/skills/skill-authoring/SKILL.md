...
1. **Builtin (in-repo):** `newsclaw-server/src/main/resources/skills/<name>/SKILL.md` — committed, shipped inside the server JAR. On every startup `BuiltinSkillSeedService` scans `classpath*:skills/*/SKILL.md`, parses each frontmatter, and upserts a row into `mate_skill` keyed by `name`. The SKILL.md is the single source of truth — no SQL seed entry is required.
2. **Custom (runtime):** created by an agent or user through the `skill_manage` tool. Stored as a `mate_skill` row with `skill_type=custom` and exported to the workspace at `~/.newsclaw/skills/<name>/`. Not committed; lives per-installation.
...
- You're adding a reusable workflow that should ship with NewsClaw → builtin.
- You're editing an existing builtin skill under `newsclaw-server/src/main/resources/skills/`.
...
## <Topic sections>  — quick-reference tables, exact commands, concrete recipes
                       (mvn test, paths under newsclaw-server/, etc.).
...
```
newsclaw-server/src/main/resources/skills/<skill-name>/SKILL.md
```
...
1. **Survey peers:** `ls newsclaw-server/src/main/resources/skills/` and read 2-3 SKILL.md files close to your topic — match tone and structure.
...
- [ ] File at `newsclaw-server/src/main/resources/skills/<name>/SKILL.md` (builtin); the directory name equals the frontmatter `name`

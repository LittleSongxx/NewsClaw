...
- Subscriptions persist to `~/.newsclaw/webhook_subscriptions.json`

## How It Works

1. `the agent webhook subscribe` writes to `~/.newsclaw/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.newsclaw/logs/gateway.log | tail -20`

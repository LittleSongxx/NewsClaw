...
In this repo the Node.js surfaces are the front-end packages — `newsclaw-ui`, `newsclaw-webchat`, and the Electron desktop app `newsclaw-desktop`. The Spring Boot backend is a JVM process and is not a target for this skill.
...
- The Electron desktop **main process** (`newsclaw-desktop`) crashes, hangs on startup, or mishandles the bundled Java backend child process
...
ps aux | grep -i 'newsclaw-desktop' | grep -v -- '--type='
...
`newsclaw-ui`, `newsclaw-webchat`, and `newsclaw-desktop` all run `vite` for `dev`. To step through Vite config or a build plugin, run Vite's binary under the inspector instead of the `pnpm dev` wrapper:

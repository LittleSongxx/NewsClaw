...
**1. Built-in tools.** The twenty tools that ship with NewsClaw — registered into the tool table on startup.

**2. MCP servers.** External processes speaking the Model Context Protocol expose tools dynamically. NewsClaw discovers them via `tools/list` and they appear in the registry alongside built-in ones. See [MCP](./mcp).
...
- **Config** — `newsclaw.tools.disclosure.mode`, default `progressive`; set it to `legacy` to restore the old "advertise everything" behavior.
...
| `NewsClawDocTool` | Read built-in project documentation | — |
...
Content tools support [AI-news production](./content-studio): Xiaohongshu package delivery by default, with WeChat drafts as optional compatibility.
...
Reads the built-in NewsClaw project documentation. Lets an agent answer "how does X work in NewsClaw" questions by consulting actual docs rather than guessing.
...
- **Config** — `newsclaw.skill.disclosure.load-skill-tool.enabled`, default true.
...
Tool Guard is how NewsClaw keeps strong tools from doing stupid things. It's **rule-based**, not a flat dangerous-tools list. Each rule says: *for this tool, with these arguments, in this context, do X* — where X is `allow`, `deny`, or `require_approval`.
...
With a large tool catalog, NewsClaw exposes a lightweight directory first and expands concrete schemas only when the task needs them. The progressive bridge reduces context pressure; normalized, enabled tool names are cached so long loops do not repeatedly scan the MCP hot path.

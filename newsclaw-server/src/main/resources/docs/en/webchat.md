...
NewsClaw's WebChat channel lets external websites reach the conversation engine over plain HTTP / SSE, with no JWT. Visitor identity is isolated under a shared API Key via `visitorId + visitorToken` (HMAC-signed).
...
## Embeddable widget (newsclaw-webchat)
...
```html
<script src="https://<your-deployment>/newsclaw-webchat.umd.js"></script>
<script>
  NewsClawWebChat.init({
...
```bash
npm install @newsclaw/webchat
```

```ts
import { init } from '@newsclaw/webchat'
...
Admin-level (require a NewsClaw JWT, outside the permitAll set above):
...
curl -N -X POST "https://newsclaw.example.com/api/v1/channels/webchat/sessions/approve" \
  -H "X-MC-Key: <API Key>" -H "X-Visitor-Token: <visitorToken>" \
  -H "Content-Type: application/json" \
  -d '{"sessionId":"s1","pendingId":"<pendingId>"}'

# Deny (synchronous JSON)
curl -X POST "https://newsclaw.example.com/api/v1/channels/webchat/sessions/deny" \
  -H "X-MC-Key: <API Key>" -H "X-Visitor-Token: <visitorToken>" \
  -H "Content-Type: application/json" \
  -d '{"sessionId":"s1","pendingId":"<pendingId>"}'
```
...
```bash
curl -X POST https://newsclaw.example.com/api/v1/admin/webchat/revoked-visitor \
...
```bash
curl -N -X POST https://newsclaw.example.com/api/v1/channels/webchat/stream \
...
```bash
curl https://newsclaw.example.com/api/v1/channels/webchat/sessions?visitorId=v1 \
  -H "X-MC-Key: your-api-key" \
...
```bash
# upload
curl -X POST https://newsclaw.example.com/api/v1/channels/webchat/upload \
  -H "X-MC-Key: your-api-key" \
...
- Upstream epic issue: https://github.com/LittleSongxx/NewsClaw/issues/355

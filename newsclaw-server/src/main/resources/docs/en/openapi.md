...
The NewsClaw backend integrates [SpringDoc OpenAPI](https://springdoc.org/) (`springdoc-openapi-starter-webmvc-ui`), which auto-generates an OpenAPI 3 document from every `@RestController` and serves an interactive debugging UI. This page covers how to access and use it.
...
# Download the YAML
curl http://localhost:18088/v3/api-docs.yaml -o newsclaw-openapi.yaml
```
...
Global OpenAPI metadata (title, description, version, server URL) is driven by the `OpenApiConfig` bean and overridable via `newsclaw.openapi.*` in `application.yml`:

```yaml
newsclaw:
  openapi:
    title: ${NEWSCLAW_OPENAPI_TITLE:NewsClaw REST API}
    version: ${NEWSCLAW_OPENAPI_VERSION:1.0}
    server-url: ${NEWSCLAW_OPENAPI_SERVER_URL:}   # empty → derived from request host
    description: ${NEWSCLAW_OPENAPI_DESCRIPTION:}  # empty → built-in default
    expose-ui: ${NEWSCLAW_OPENAPI_EXPOSE_UI:true}  # whether the Swagger/OpenAPI paths are public, see the security section below
```

When `server-url` is empty, SpringDoc derives it from the request host so "Try it out" hits the right address; for fixed production URLs (e.g. behind a reverse proxy), set `NEWSCLAW_OPENAPI_SERVER_URL=https://newsclaw.example.com`.

## 🔒 Access control: Swagger is locked down by default in production

Access to the Swagger UI / OpenAPI document paths (`/swagger-ui*`, `/v3/api-docs*`, `/webjars/**`) is controlled by the `newsclaw.openapi.expose-ui` flag and enforced explicitly in `SecurityConfig.filterChain` (no longer relying on the `.anyRequest().permitAll()` fallthrough):
...
- Public production: defaults to `false` (locked down). To temporarily open it on an internal/staging host, set `NEWSCLAW_OPENAPI_EXPOSE_UI=true`.

...
Common questions and real answers. If your question isn't here, check the relevant feature page or open a [GitHub issue](https://github.com/LittleSongxx/NewsClaw/issues).
...
**Java 17 or higher.** NewsClaw uses features introduced in Java 17 (sealed classes, text blocks, records, pattern matching). Verify with `java -version`.
...
- **Ollama** — local GPU inference; NewsClaw auto-detects it on `localhost:11434` at startup
...
NewsClaw can start without a cloud API key (for example with Ollama, OAuth, or no Provider configured). Docker deployments may also pre-populate non-empty Provider keys in `.env`; those values take precedence over the database at runtime and are not persisted. Without environment values, configure Providers after startup in `Settings → Models`.
...
2. Sign up or log in
3. Create an API key
4. In NewsClaw, go to `Settings → Models → DashScope` and paste it
...
NewsClaw implements **sliding-window renewal** — when a token is within 25% of expiry, the server issues a new one in the `X-New-Token` response header. The frontend handles this automatically.
...
Use either the UI or `.env`. In the UI, open `Settings → Models → Add Provider`; in Docker, set the matching Provider key/base URL in `.env`. Environment values take precedence over database values without being written back; Provider metadata, model selection, and enabled state remain UI-managed. No `spring.ai.*` YAML blocks need editing.

The container can start with zero keys, but content generation requires at least one usable Provider. Non-empty `DASHSCOPE_API_KEY`, `OPENAI_API_KEY`, and similar variables are effective at runtime.

### How do I use GPT-4 with NewsClaw?
...
Tune `newsclaw.wiki.digestion-concurrency` in `application.yml`. Default is 2 — bump to 4 or 8 if your LLM quota allows.
...
1. **Confirm auto-extraction is enabled** — check `newsclaw.memory.auto-summarize-enabled` in config
...
4. **Read the logs** — `vip.newsclaw.memory` at DEBUG level shows every attempt
...
4. **Look at the logs**:
   ```bash
   mvn spring-boot:run -Dspring-boot.run.arguments="--logging.level.vip.newsclaw.agent=DEBUG"
   ```
...
**H2 (development / desktop):** stop, copy `./data/newsclaw.mv.db`:

```bash
cp ./data/newsclaw.mv.db ./backup/newsclaw-$(date +%Y%m%d).mv.db
```

**MySQL (supported self-managed deployment):**

```bash
mysqldump -u root -p newsclaw > newsclaw-backup-$(date +%Y%m%d).sql
```

**Docker:**

```bash
docker compose exec -T newsclaw-postgres sh -c 'pg_dump -U "$POSTGRES_USER" "$POSTGRES_DB"' > backup.sql
```

**Desktop** data lives in the per-user directory:

- macOS: `~/Library/Application Support/NewsClaw/`
- Windows: `%APPDATA%/NewsClaw/`
- Linux: `~/.local/share/NewsClaw/`
...
- macOS: `~/Library/Logs/NewsClaw/`
- Windows: `%APPDATA%/NewsClaw/logs/`
- Linux: `~/.local/share/NewsClaw/logs/`
...
**Auto-updates** via electron-updater. On startup, checks GitHub Releases and prompts you when a new version is available. Manual download also available from [Releases](https://github.com/LittleSongxx/NewsClaw/releases).
...
```bash
docker compose logs newsclaw-server
docker compose logs newsclaw-postgres
```
...
```bash
docker compose exec newsclaw-postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB"'
```
...
logging:
  level:
    vip.newsclaw: DEBUG
    vip.newsclaw.agent: DEBUG
    vip.newsclaw.agent.graph: DEBUG
...
```bash
mvn spring-boot:run -Dspring-boot.run.arguments="--logging.level.vip.newsclaw=DEBUG"
```

### How do I access the H2 console?

1. Visit `http://localhost:18088/h2-console`
2. JDBC URL: `jdbc:h2:file:./data/newsclaw`
3. Username: `sa`
4. Password: (empty)
...
cd newsclaw-ui
npm run build
ls ../newsclaw-server/src/main/resources/static/
...
- [GitHub Issues](https://github.com/LittleSongxx/NewsClaw/issues) — when the docs don't answer your question

...
The recommended NewsClaw deployment. One `docker compose up -d` brings up PostgreSQL, SearXNG, and the backend service.

> The current product mainline is AI-news discovery, evidence verification,
> Feishu notification, and Xiaohongshu material delivery. The Compose project
> and service containers retain the `newsclaw` technical prefix for verified
> volume, network, and operations compatibility; the public product and
> repository are NewsClaw.
...
|---|---|---|---|
| `newsclaw-postgres` | `postgres:16` | Business data | compose network only (no host port by default) |
| `searxng` | Built from `./docker/searxng/` | Keyless search fallback | `8088` |
| `newsclaw-server` | Built from `newsclaw-server/Dockerfile` | Spring Boot backend + embedded browser | `18080` |
...
- **Upstream ships with only `html` output enabled**, while newsclaw calls `GET /search?q=...&format=json`. The default image responds to JSON requests with an HTML error page, `SearXNGSearchProvider` fails to parse it, returns empty results, and the UI shows "search temporarily unavailable".
...
# 2. Hit it from inside the newsclaw-server container
docker exec newsclaw-server wget -qO- 'http://newsclaw-searxng:8080/search?q=test&format=json' | head -5
# If this fails, compose networking is the problem.

# 3. Ask an agent to search and tail backend logs
docker compose logs -f newsclaw-server | grep "搜索 provider"
# Expect: 搜索 provider 解析: searxng (source=keyless-fallback)
```

### Using an external SearXNG instance

If you're already running SearXNG elsewhere, point newsclaw at it via `.env`:
...
The backend runtime stage (`newsclaw-server/Dockerfile` stage 3) is based on `mcr.microsoft.com/playwright:v1.62.0-noble` (Ubuntu Noble 24.04, glibc) and installs on top of it:
...
`vip.newsclaw.tool.browser.BrowserLauncher` tries each strategy in order until one succeeds:

1. `CONFIG_CDP` — if `NEWSCLAW_BROWSER_CDP_URL` is set, attach to that running Chrome
2. `CONFIG_PATH` — if `NEWSCLAW_BROWSER_CHROME_PATH` or `CHROME_PATH` env is set, use that exe
3. `CONFIG_CHANNEL` — if `NEWSCLAW_BROWSER_CHANNEL=chrome|msedge`, use the Playwright channel
...
An LLM generating a malicious URL to dump cloud credentials is therefore a closed loop. If you genuinely need to scrape internal infrastructure from a specific host, either disable via `newsclaw.browser.ssrf-check-enabled` or edit the `UrlSafetyChecker` allowlist. **Think twice before doing this in production.**
...
| `NEWSCLAW_SETTING_KEY` | Encryption key for sensitive `mate_system_setting` values; set and back it up |
| `NEWSCLAW_CORS_ALLOWED_ORIGINS` | Production allowlist, e.g. `https://newsclaw.example.com` |
...
**Already optimal.** `newsclaw-server/pom.xml` lists repositories in the order `Maven Central → Google CDN → Aliyun`; Central direct is fastest over US/EU backbones.

### China servers

Flip to Aliyun-first. Either edit the `mvn` lines in `newsclaw-server/Dockerfile` to add `-Paliyun-first`, or (easier) expose it as a build arg:
...
```sh
docker compose build --build-arg MAVEN_PROFILE=aliyun-first newsclaw-server
```
...
| `SEARXNG_BASE_URL` | `http://newsclaw-searxng:8080` | Point at an external SearXNG instance |
| `NEWSCLAW_BROWSER_CDP_URL` | — | Attach to an external Chrome CDP sidecar |
| `NEWSCLAW_BROWSER_CHROME_PATH` | — | Override the bundled Chromium with a host-installed browser |
| `NEWSCLAW_BROWSER_CHANNEL` | — | Force a Playwright channel (`chrome`, `msedge`, ...) |

**LLM API keys (DashScope, OpenAI, Anthropic, DeepSeek, Kimi, etc.) may be pre-populated in `.env`, or added after startup in `Settings → Models → Add Provider`.** Non-empty environment values take precedence at runtime, are not written back to the database, and support reload through a restart. The container can start with zero Provider keys, but content generation needs at least one usable Provider.
...
**`newsclaw-server` stays unhealthy at startup**
`docker compose logs newsclaw-server` and look for Flyway migration errors. Nine times out of ten, a special character in `DB_PASSWORD` got eaten by the shell — wrap the value in double quotes in `.env`.

**Browser tool reports "Target page closed" or SIGBUS**
`shm_size: 2gb` didn't take effect. Check the actual value with `docker inspect newsclaw-server | grep ShmSize`. Upgrade Docker Engine to 24.0+ if it's still showing 64 MB.
...
```sh
git pull
docker compose build newsclaw-server   # only rebuild the backend

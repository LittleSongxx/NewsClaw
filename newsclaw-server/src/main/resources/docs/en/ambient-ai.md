...
NewsClaw's answer is the opposite: **the messengers your team already uses are the hardware.**
...
| | Conversational AI | Ambient AI (NewsClaw) |
|---|---|---|
...
Only NewsClaw fills out the right column completely, because only NewsClaw has all of these at once:
...
- `vip.newsclaw.cron.service.CronJobLifecycleService` — task lifecycle management
- `vip.newsclaw.cron.service.CronJobRunner` — single execution
- `vip.newsclaw.cron.delivery.ChannelCronResultDelivery` — routes agent output to the channel
- `vip.newsclaw.cron.delivery.CronDeliveryListener` — listens to `CronJobCompletedEvent`
- `vip.newsclaw.cron.CronChatOriginFactory` — tags conversations with "this came from cron job X" so sessions stay traceable

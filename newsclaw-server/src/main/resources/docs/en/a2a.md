...
NewsClaw can expose enabled agents through an A2A JSON-RPC endpoint and can call other A2A peers from agent tools.
...
`base-url` is required for production deployments. If it is empty, NewsClaw derives the Agent Card URL from the incoming request, which depends on proxy headers being correct.
...
- `POST /api/a2a` requires an existing NewsClaw Bearer token and supports `message/send`, `message/stream`, `tasks/get`, and `tasks/cancel`.

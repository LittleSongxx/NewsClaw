...
**How NewsClaw is put together, in one page.**

If you're using NewsClaw, read [Introduction](./intro). If you're building on NewsClaw — adding tools, new channels, custom memory providers, new agent graph nodes — read this page.
...
│                   │                                               │
│  ┌────────────────▼──────────────────────────────────────────┐  │
│  │            Spring Boot Backend (vip.newsclaw.*)                │  │
│  │                                                             │  │
│  │  ┌────────────┐  ┌────────────┐  ┌────────────────────┐   │  │
│  │  │    auth     │  │  channel    │  │      agent          │   │  │
...
```
newsclaw/
├── newsclaw-server/          # Spring Boot backend (the heart)
│   └── src/main/java/vip/mate/
│       ├── NewsClawApplication.java
...
├── newsclaw-ui/              # Vue 3 admin console
├── newsclaw-desktop/         # Electron desktop shell
├── newsclaw-webchat/         # Embeddable chat widget
...
**NewsClaw's agent runtime is not a class hierarchy.** There's no `BaseAgent` → `ReActAgent` → `MyCustomAgent` inheritance chain. The runtime is a **StateGraph** (from `spring-ai-alibaba-graph`) composed of nodes and conditional edges, assembled at runtime by `AgentGraphBuilder`.
...
- `agent/graph/state/NewsClawStateKeys.java` — the keys for the shared state object
- `agent/graph/state/NewsClawStateAccessor.java` — typed accessor for the state map (don't touch the map directly)
...
**Adding agent behavior** — create a new node in `agent/graph/node/` or a new edge dispatcher in `agent/graph/edge/`. Wire it into `AgentGraphBuilder`. Read and write state through `NewsClawStateAccessor`.
...
Implement `vip.newsclaw.channel.ChannelAdapter` (or `StreamingChannelAdapter` for streaming). Register as a Spring bean. Add webhook endpoints via `ChannelWebhookController`. See [Channels](./channels).
...
Implement `vip.newsclaw.memory.spi.MemoryProvider` to plug in a custom memory backend (vector, graph, external service). Multiple providers can be stacked per agent. See [Memory](./memory).
...
For deeper customization, add a new node in `agent/graph/node/` or a new dispatcher in `agent/graph/edge/`. Wire it into `AgentGraphBuilder` behind a config flag. State access goes through `NewsClawStateAccessor`.

---

## Persistence — one schema, two databases

NewsClaw uses **MyBatis Plus** (not JPA) for database access. Conventions:
...
- Flyway validates migration checksums on startup. `FlywayRepairConfig` only runs `repair()` when the explicit `newsclaw.flyway.auto-repair` switch is enabled; keep that switch off in production so drift and partial failures remain visible
...
NewsClaw uses **Spring MVC**, not Spring WebFlux. WebFlux is explicitly excluded from the dependency graph.
...
2. **Desktop** — Electron shell in `newsclaw-desktop/` bundles JRE 21 and the JAR. Users never install Java. Auto-update via electron-updater.
3. **Docker** — `docker-compose.yml` with MySQL. Production deployment.

The webchat widget in `newsclaw-webchat/` is a fourth path — an embeddable chat UI you can drop into any website. It talks to the same backend APIs.

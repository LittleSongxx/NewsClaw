...
|------|--------------------|------------------|
| **`builtin`** | Ships with NewsClaw under `skills/` in the classpath | The core team |
...
| `content` | UTF-8 text (defaults: ≤1 MB per file, ≤50 MB per bundle — configurable via `newsclaw.skill.upload.max-entry-size-mb` / `max-total-size-mb`) |
...
- **Two-pass extraction** — the entire archive is buffered in memory first (cap-protected, 50 MB by default via `newsclaw.skill.upload.max-total-size-mb`), `SKILL.md` is located and the wrapper-dir prefix computed, then entries are classified. **Zip entry order no longer affects the result.**
...
newsclaw:
  skill:
    workspace:
      root: ${user.home}/.newsclaw/skills
      auto-init: true
...
- **Built-in** — skills that ship with NewsClaw
...
A skill that's installed isn't necessarily a skill that runs — it might need an API key, a CLI tool, a NewsClaw feature flag toggled on.
...
No `~/.zshrc` edit, no newsclaw restart needed.
...
ACP (Agent Client Protocol) is a protocol that lets external agent clients (Claude Code, Codex, other compatible clients) plug into NewsClaw as skills.

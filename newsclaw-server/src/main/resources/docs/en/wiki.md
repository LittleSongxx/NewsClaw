...
NewsClaw's LLM Wiki does something different. Drop raw material into a knowledge base and the system reads it, digests it, and writes structured Wiki pages — each with a summary, backlinks, and provenance pointers back to the source passage. You can open any page and read it. You can edit it. Agents read summaries automatically and pull full pages on demand.
...
NewsClaw's LLM Wiki **is the same idea, raised into a product**:
...
- Not an isolated tool — the **knowledge layer of NewsClaw's agent operating system**, threaded through memory, agents, and channel delivery
...
- **Scan a local directory** — desktop only. Point at a folder and NewsClaw walks it recursively, respecting `.gitignore`, importing everything that looks like text.
...
Emergency kill-switch: set `newsclaw.wiki.cascade-delete-enabled=false`
...
- **Fail-closed security** — paths are normalized then symlink-resolved (closing TOCTOU) and validated against an allowed-roots allowlist; under the production profile an empty allowlist rejects everything. Set the `newsclaw.wiki.allowed-source-roots` allowlist.
...
The `newsclaw.wiki` block in `application.yml` controls global knobs (chunk size, parallelism, auto-process-on-upload). Per-KB knobs (ingest mode, step models, fallback chain) live inside the KB's `configContent` JSON and are edited through the config UI.

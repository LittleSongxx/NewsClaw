...
This page is source-aligned with the Spring MVC controllers under `newsclaw-server/src/main/java`. The route inventory below was rebuilt from controller annotations; when it conflicts with an older feature page, this page and the source code are the contract.
...
Source: `vip.newsclaw.common.result.R` (`R.java`). Three fields:
...
Source: `vip.newsclaw.common.result.ResultCode`. Two categories:
...
**Sliding renewal**: when a JWT is near expiry (default < 2h remaining), the response header returns a fresh token — `X-New-Token: <newJwt>` (with `Access-Control-Expose-Headers: X-New-Token`). Clients should watch for and replace the locally stored token. JWT TTL defaults to 24h (`newsclaw.jwt.expiration=86400000`).
...
| `endUserId` | string | null | Third-party end-user ID, isolates memory when one NewsClaw account fronts many end-users |

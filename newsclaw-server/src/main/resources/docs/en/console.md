...
The admin console is the Vue 3 SPA that ships with every NewsClaw deployment. It runs in your browser (or inside the Electron desktop window), talks to the Spring Boot backend over REST + SSE, and exposes every capability NewsClaw has — chat, agents, knowledge, tools, skills, channels, security, cron jobs, usage analytics — behind one login.
...
NewsClaw uses domain-driven Pinia stores. Each store owns its slice of state exclusively.
...
npm run build     # vue-tsc + vite build into ../newsclaw-server/.../static
npm run lint      # ESLint
```

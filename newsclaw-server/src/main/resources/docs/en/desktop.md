...
That's the desktop app in four sentences. No Java to install. No browser to open. No docker compose file. No port to remember. NewsClaw's desktop edition bundles Electron, a JRE 21 runtime, and the packaged Spring Boot server JAR into a single installer. **Your users never know Java is underneath.**
...
JAR_FILE=$(ls -1 target/newsclaw-server-*.jar | grep -v sources | head -n 1)
cp "$JAR_FILE" ../newsclaw-desktop/resources/app.jar
...
Frontend assets can be **hot-updated independently** — a frontend-only fix doesn't require a new installer. See `newsclaw-desktop/scripts/` and `desktop-ui-hot-update.md` for the hot-update build flow.
...
| OS | Path |
|-----|------|
| macOS | `~/Library/Application Support/NewsClaw/data/` |
| Windows | `%APPDATA%/NewsClaw/data/` |
| Linux | `~/.config/NewsClaw/data/` |
...
| Setting | Purpose |
|---------|---------|
| `appId` | `vip.newsclaw.newsclaw` — system registration and code signing |
...
- **macOS** — right-click → **Open** to bypass Gatekeeper (first launch). Production: Apple Developer certificate + notarization. See `newsclaw-desktop/CODESIGNING.md`.
...
2. Check logs:
   - macOS: `~/Library/Application Support/NewsClaw/logs/`
   - Windows: `%APPDATA%/NewsClaw/logs/`
   - Linux: `~/.config/NewsClaw/logs/`
3. Launch from terminal to see console output
4. Confirm backend port isn't blocked

### WeCom auth popup

The WeCom QR-code authorization flow **must open in an in-app popup** (not the system browser) so the `postMessage` callback works. NewsClaw handles this in `setWindowOpenHandler` — `work.weixin.qq.com` domain opens as an in-app popup window.

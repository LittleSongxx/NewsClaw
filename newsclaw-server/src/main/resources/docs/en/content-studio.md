...
**A verified event in. An auditable content deliverable out.**

NewsClaw content production is not generic writing. It starts from verified AI-industry events, source claims, and Wiki evidence pages; runs through Team Runs, compliance, and human review; and delivers Feishu results plus a human-uploadable **Xiaohongshu** material package. WeChat draft tooling is retained as optional compatibility, not the default delivery path.

The workflow reuses web search, official-page read-only capture, HTML rendering, structured memory, cron, and the Skill runtime while associating factual evidence and final artifacts with the same event and runId.

---

## The seeded employee

Content production runs as part of the AI-news team. It receives a verified event, reads evidence and editorial constraints, and follows the discipline of confirming before an outward, irreversible step.
...
|---|---|---|
| **① Topic** | Confirms a topic from verified AI-news events, editorial preferences, and daily-radar candidates | `ai_news_event`, Memory, cron |
| **② Research** | Aligns official sources, claims, and Wiki evidence; conflicted events cannot proceed | event evidence, official capture, Wiki |
...
| **⑦ Deliver** | Stops at human confirmation after compliance scanning; defaults to preview, ZIP materials, and Feishu notification | `xhs_package`, Content Calendar, Feishu |
...
## Default delivery and optional compatibility

### WeChat Official Account (optional compatibility) — `gzh_article`
...
- **3 to 18 vertical 3:4 cards** (cover + content + closing), rendered from HTML templates. `xhs_package` hard-validates the image count and event-evidence boundary, and rejects an invalid package.
...
- **Online preview and ZIP package** — rendered card PNGs are the preview; after review, download the ZIP and upload it manually in the Xiaohongshu creator console.
...
- **Secrets encrypted at rest.** `weixinoa.app_secret` and other sensitive settings are **AES-GCM encrypted** (key from `NEWSCLAW_SETTING_KEY`, machine-derived fallback), ciphertext carries an `enc:v1:` prefix, and legacy plaintext is read transparently and upgraded on next write.
...
**Draft-box-first.** Mass-send and publish are outward, irreversible, and rate-limited, so NewsClaw drafts and you press “publish” in the Official Account backstage. The optional `publish` action is gated through the [approval](./security) flow.
...
2. Set **`NEWSCLAW_SETTING_KEY`** to a stable secret and **back it up** — it decrypts existing ciphertext; losing it means re-entering secrets.
3. Add the server's **public IP** to the Official Account backstage whitelist. The publish chain will tell you if it's missing.

Xiaohongshu needs no API key — NewsClaw produces a downloadable preview and card package by default.

---

## Limitations & non-goals

- **No mass-send / one-click push to all followers.** Rate-limited and outward-irreversible; NewsClaw can optionally draft, while you publish from the backstage.
- **No automated Xiaohongshu publishing.** The current mainline has no general official publish API, browser login automation, or CAPTCHA handling. It creates a preview and ZIP materials for human upload and never bypasses platform risk controls or human verification.

...
You opened NewsClaw because you want AI to do work for you. Not because you want to learn new software.
...
| Step | What | Time |
|------|------|------|
| 1 | Double-click, log in with `admin` / the deployment bootstrap secret | 10s |
...
**A fresh NewsClaw install has an empty provider list. That's deliberate — you don't need to see 16 providers, you need one that works.**
...
NewsClaw's memory doesn't require manual management. After each conversation, the system automatically extracts key information and writes it to memory. Next time, the agent works with that context.
...
|---------|-------------------|
| Backend won't start | Port 18088 is taken. Check `<userData>/logs/newsclaw.log` (macOS: `~/Library/Application Support/NewsClaw/logs/newsclaw.log`) |
...
| Still broken | [GitHub Issues](https://github.com/LittleSongxx/NewsClaw/issues) with the tail of `app.log` |

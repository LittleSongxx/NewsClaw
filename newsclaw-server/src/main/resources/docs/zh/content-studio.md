...
NewsClaw 的内容生产不是通用写作场景：它从已核验的 AI 行业事件、来源 claims 和 Wiki 证据页开始，经过 Team Run、合规与人工审核，交付飞书结果和可人工上传的**小红书**素材包。微信公众号草稿能力保留为可选兼容路径，但不属于默认交付主线。

这条流水线复用联网搜索、官方页面只读抓取、HTML 渲染、结构化记忆、定时任务与 Skill 运行时，并把事实依据与最终产物关联到同一事件和 runId。

---

## 预置员工

内容生产以 AI 动态团队的一部分运行。它接收已核验事件，读取证据、编辑约束和人工确认后的选题，再遵循“外向、不可逆那一步之前先确认”的纪律。
...
| 阶段 | 做什么 | 靠谁 |
|---|---|---|
| **① 选题** | 从已核验 AI 动态事件、编辑偏好和每日雷达候选中确认选题 | `ai_news_event`、Memory、cron |
| **② 搜集** | 对齐官方来源、claims 与 Wiki 证据；冲突事件不得继续生产 | 事件证据、官方抓取、Wiki |
...
| **⑦ 交付** | 合规扫描后停在人工确认点；默认交付预览、ZIP 素材和飞书通知 | `xhs_package`、内容日历、飞书 |
...
- **3 到 18 张竖版 3:4 卡片**（封面 + 内容 + 结尾），由 HTML 模板渲染；`xhs_package` 会硬校验图片数量和事件证据边界，不合格时拒绝打包。
- **标题四件套**(数字 / 悬念 / 情绪 / 对比,≤20 字)+ 碎句正文配 emoji 分段 + 3–8 个话题标签(大词 + 中词 + 长尾)。
- **在线预览与 ZIP 素材包**——渲染出的卡片 PNG 本身就是预览；审核通过后下载 ZIP，在小红书创作中心人工上传。
...
- **密钥加密落库。** `weixinoa.app_secret` 等敏感设置以 **AES-GCM 加密**存储(密钥取自 `NEWSCLAW_SETTING_KEY`,回退机器派生密钥),密文带 `enc:v1:` 前缀,旧明文读时透明兼容、下次写入自动升级。
...
**草稿箱优先。** 群发/发表是外向、不可逆且受频次限制的动作，由 NewsClaw 出草稿、你在公众号后台点“发表”。可选的 `publish` 动作走 [审批](./security) 流。
...
2. 把 **`NEWSCLAW_SETTING_KEY`** 设成一个稳定密钥并**做好备份**——它用来解密已有密文,丢了就得重填密钥。
3. 把服务器**公网 IP** 加进公众号后台白名单。发布链会在缺失时提示你。

小红书无需 API key——NewsClaw 默认产出可下载的预览和卡片素材包。

---

## 约束与非目标

- **不做群发 / 一键推送全部粉丝。** 受频次限制且外向不可逆；NewsClaw 仅可选地产出草稿，你在后台发表。
- **不做自动小红书发布。** 当前主线没有通用官方发布 API、浏览器自动登录或验证码处理；系统生成预览和素材 ZIP，由人工上传，绝不绕过任何平台风控或人机验证。

...
NewsClaw 推荐的部署方式。一条 `docker compose up -d` 起三个容器：PostgreSQL、SearXNG 和后端服务。

> 当前产品主线是 AI 动态发现、证据核验、飞书通知和小红书素材交付。Compose 项目名与
> 服务容器名保留 `newsclaw` 技术前缀，以维持已验证的本地卷、网络和运维命令兼容；
> 对外产品和仓库名称为 NewsClaw。
...
| 服务 | 镜像 | 作用 | 暴露端口 |
|---|---|---|---|
| `newsclaw-postgres` | `postgres:16` | 业务数据存储 | 仅容器网络内（默认不映射宿主端口） |
| `searxng` | 本地构建 `./docker/searxng/` | 无 API Key 搜索兜底 | `8088` |
| `newsclaw-server` | 本地构建 `newsclaw-server/Dockerfile` | Spring Boot 后端 + 内置浏览器 | `18080` |
...
- **上游默认只开 `html` 格式**，newsclaw 后端请求的是 `GET /search?q=...&format=json` —— 默认配置下直接返回 HTML 错误页，`SearXNGSearchProvider` 解析失败返回空列表，UI 显示"搜索暂时不可用"
...
后端镜像以 `mcr.microsoft.com/playwright:v1.62.0-noble` 为基础（Ubuntu Noble 24.04，glibc），由 `newsclaw-server/Dockerfile` 的第三阶段拉起，额外装：
...
`vip.newsclaw.tool.browser.BrowserLauncher` 按如下优先级启动浏览器，任何一级命中即止：

1. `CONFIG_CDP` —— 配了 `NEWSCLAW_BROWSER_CDP_URL` → 直接 attach 已运行的 Chrome
2. `CONFIG_PATH` —— 配了 `NEWSCLAW_BROWSER_CHROME_PATH` 或 `CHROME_PATH` env → 用指定 exe
3. `CONFIG_CHANNEL` —— 配了 `NEWSCLAW_BROWSER_CHANNEL=chrome|msedge` → 走 Playwright channel
...
`docker-compose.yml` 给 `newsclaw-server` 设了 `shm_size: 2gb`。Docker 默认给每个容器只 64 MB `/dev/shm`，Chromium 用共享内存做 GPU / 页面渲染，跑 3 个 tab 就会 SIGBUS 挂掉，表现为 Playwright `TargetClosedError: Target page, context or browser has been closed`。**不要改小这个值**。
...
也就是说 LLM 生成一个恶意 URL 指向云元数据端点偷凭据这条路是封死的。如果你有内网抓取需求需要放行特定地址，关 `newsclaw.browser.ssrf-check-enabled` 或改 `UrlSafetyChecker` 的白名单。**生产环境谨慎**。
...
| 变量 | 说明 |
|---|---|
| `DB_PASSWORD` | 应用账号密码（最小权限角色，仅拥有 `newsclaw` schema），建议 16+ 位 + 大小写 + 数字 + 符号 |
...
| `NEWSCLAW_SETTING_KEY` | `mate_system_setting` 敏感值加密密钥，建议显式设置并备份 |
| `NEWSCLAW_CORS_ALLOWED_ORIGINS` | 生产白名单，如 `https://newsclaw.example.com` |
...
**默认就是最快的**：`newsclaw-server/pom.xml` 里 `<repositories>` 的优先级是 `Maven Central → Google CDN → Aliyun`，Central 直连最快。

### 中国服务器

切 Aliyun 优先：改 `newsclaw-server/Dockerfile` 的 `mvn` 命令加 `-Paliyun-first`，或者（更简单）在 `docker-compose.yml` 加一行 `build args` 传进去。
...
| `SEARXNG_BASE_URL` | `http://newsclaw-searxng:8080` | 想接外部 SearXNG 实例时填 |
| `NEWSCLAW_BROWSER_CDP_URL` | — | 接外部 Chrome sidecar（CDP 端点） |
| `NEWSCLAW_BROWSER_CHROME_PATH` | — | 用宿主机 Chrome 覆盖镜像内置 |
| `NEWSCLAW_BROWSER_CHANNEL` | — | `chrome` / `msedge` 等，强制指定 Playwright channel |

**LLM 的 API Key（DashScope / OpenAI / Anthropic / DeepSeek / Kimi / ...）可以在 `.env` 里预置，也可以启动后在 UI「设置 → 模型 → 添加供应商」里添加。** 非空环境变量优先于数据库值，只做运行时覆盖，不会回写数据库；容器零 Provider Key 仍能启动，但实际生成内容前需要配置至少一个可用 Provider。
...
**`newsclaw-server` 启动前就 unhealthy**
`docker compose logs newsclaw-server` 看 Flyway 迁移是否成功。通常是 DB_PASSWORD 含特殊字符被 shell 吃了 —— 用双引号包住。

**浏览器工具报 "Target page closed" / SIGBUS**
`shm_size: 2gb` 没生效。`docker inspect newsclaw-server | grep ShmSize` 看实际值。老版本 Docker Engine 要升级到 24.0+。
...
```sh
git pull
docker compose build newsclaw-server   # 只重建后端

...
常见问题 + 真答案。你的问题不在这里就看对应的功能页，或者去 [GitHub issue](https://github.com/LittleSongxx/NewsClaw/issues) 开一个。
...
**Java 17 或更高。** NewsClaw 用了 Java 17 引入的特性。用 `java -version` 验证。
...
- **Ollama**——本地 GPU 推理；NewsClaw 启动时在 `localhost:11434` 自动探测
- **ChatGPT OAuth**——有 ChatGPT Plus 或 Pro 订阅的话走浏览器 OAuth 流程——**不需要 API Key**
- **OpenRouter 免费档**——200+ 免费模型，一个 key 就能访问

启动 NewsClaw 不要求必须配置云端 API Key；可以使用 Ollama、OAuth 或空配置启动。Docker 部署也支持在 `.env` 中预置非空 Provider Key，环境变量优先于数据库值且不会回写数据库；不使用环境变量时，仍可在启动后通过 UI 的 `设置 → 模型` 配置。
...
2. 注册或登录
3. 创建一个 API Key
4. 在 NewsClaw 里进 `设置 → 模型 → DashScope` 粘贴
...
用户名为 `admin`，密码使用部署者生成的一次性 bootstrap secret。**不要使用共享默认值，任何真实部署都要立刻改。**

### 我的 JWT token 老过期

NewsClaw 实现了**滑动窗口续签**——token 剩余 25% 时服务器在响应头 `X-New-Token` 里发一个新 token。前端自动处理。
...
可通过 UI 或 `.env` 配置。UI 路径是 `设置 → 模型 → 添加供应商`；Docker 路径是在 `.env` 填写对应 Provider 的 Key/Base URL。环境变量优先于数据库值且不会回写数据库，Provider 元数据、模型选择和启用状态仍在 UI 中维护；没有环境变量时保持原有 UI 流程。

容器没有任何 Key 也能启动，但实际调用需要至少一个可用 Provider。非空的 `DASHSCOPE_API_KEY`、`OPENAI_API_KEY` 等会在运行时生效。

### 怎么在 NewsClaw 里用 GPT-4？
...
调 `application.yml` 里的 `newsclaw.wiki.digestion-concurrency`。默认 2——LLM 额度允许就调到 4 或 8。
...
1. **确认自动提取开着**——检查 `newsclaw.memory.auto-summarize-enabled`
...
4. **看日志**——`vip.newsclaw.memory` 在 DEBUG 级别显示每一次尝试
...
**H2（开发 / 桌面）：** 停服务，拷贝 `./data/newsclaw.mv.db`：
...
**自动更新**通过 electron-updater。启动时检查 GitHub Releases 并弹提示。也可以手动从 [Releases](https://github.com/LittleSongxx/NewsClaw/releases) 下载。
...
- [GitHub Issues](https://github.com/LittleSongxx/NewsClaw/issues)——文档没答案的时候去这里

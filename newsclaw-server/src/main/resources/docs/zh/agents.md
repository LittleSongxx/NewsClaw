...
description: NewsClaw 的多智能体系统支持 ReAct 推理循环和 Plan-and-Execute 任务拆解两种模式。Agent 之间可以互相委派，实现真正的多智能体协作。
head:
  - - meta
    - name: keywords
...
这组默认拒绝列表可通过 `newsclaw.delegation.child-denied-tools` 调整。
...
不是所有问题都值得深度推理，但有些问题需要。NewsClaw 支持按 Agent、按对话打开深度思考模式：

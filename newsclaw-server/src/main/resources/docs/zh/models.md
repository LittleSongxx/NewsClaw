...
NewsClaw 不关心你用哪个 LLM。它通过五个协议适配器跟所有主流供应商对话，支持 15+ 个云端供应商和 4 个本地运行时，你可以在运行时**不动 Agent 配置**直接切模型。NewsClaw 唯一的意见是——**从一个开始，需要再加**，不是第一天就把所有东西配好。
...
::: tip 1.4.0 新增
Gemini 不再走 OpenAI 兼容层——NewsClaw 直接对接 Google 的**原生 `generateContent` API**。
:::

很多产品把 Gemini 当成"又一个 OpenAI 兼容端点"来接，结果在系统指令、函数调用、内联图片这些地方处处碰壁。NewsClaw 走的是 Gemini 自己的协议：
...
有 ChatGPT Plus 或 Pro 账号？NewsClaw 可以通过**浏览器 OAuth** 对接 OpenAI 的 chat 端点——你按平常方式登录，你的订阅被直接使用。GPT-4o、o3、o4-mini 立刻可用。

`设置 → 模型 → 添加供应商 → OpenAI OAuth`。浏览器窗口弹出。Token 交换在后端完成，**凭证不离开你的机器**。

### 设备授权（Device Authorization Grant）—— 远程 / 无头部署专用

浏览器回调式 OAuth 要求 IDP 的重定向能落回 *你的浏览器* 能访问的某个 `localhost` 端口。这事儿在 NewsClaw 跑你笔记本上时没问题，一旦你把它放到服务器、容器、或任何不向客户端暴露 loopback socket 的宿主上，就立刻坏掉。
...
| `newsclaw.oauth.openai.deployment-mode` | 行为 |
|---|---|
...
- **浏览器回调** —— 本地安装，浏览器弹窗，点完授权 token 落到 NewsClaw
...
提供模型列表的供应商（OpenAI、Ollama、LM Studio、OpenRouter 等）支持**模型发现**——一键让 NewsClaw 拉取这个供应商下的所有模型。
...
::: tip 1.4.0 新增（historical issue #79）
...
**小米 MiMo 思考模式多轮修复**（historical issue #189）：MiMo 思考模式的 `reasoning_content` 现在能在多轮对话里正确保留，不再在后续轮次丢失。
...
NewsClaw 用一个**活跃模型**作为全局默认。没有指定自己模型的 Agent 都用它。
...
::: tip 1.4.0 新增
- **按会话选模型**（historical issue #150）：在聊天界面里可以为**当前这一条会话**临时切换模型，不影响全局活跃模型和别的会话。详见 [聊天与消息](./chat)。
...
::: tip 1.3.0 新增
让纯文本主模型也能"看图回答"，参见 historical issue #87。
:::
...
什么时候触发？运行时由 `MultimodalRouter` 决策（[源码](https://github.com/LittleSongxx/NewsClaw/blob/main/newsclaw-server/src/main/java/vip/mate/llm/routing/MultimodalRouter.java)）：
...
::: tip
模型配置可以通过 `设置 → 模型` 管理，也可以在 Docker 部署时通过 `.env` 预置 Provider Key 和 Base URL。没有任何 `spring.ai.*` 的 YAML 需要手动修改；非空环境变量优先于数据库/UI 值，且只做运行时覆盖，不会把 secret 写回数据库。
:::
...
Provider API Key/Base URL 支持通过 `.env` 配置（例如 `DASHSCOPE_API_KEY`、`OPENAI_API_KEY`）；环境变量优先于同一 Provider 的数据库值，但不会替换 Provider 元数据，也不会回写数据库。没有环境变量时，仍可在「设置 → 模型 → 添加供应商」中完成全部配置。新装实例可先用 `.env` 预置 Key，再在 UI 中选择模型、启用 Provider 和调整路由。
...
NewsClaw 不再把所有模型统一当作 128K。运行时预算解析顺序是：**管理员覆盖值 → 本地模型实时探测或供应商超限反馈的短期缓存 → 内置模型目录 → 原有全局默认值**。模型列表为了避免 I/O，只显示覆盖值或目录值；未知模型仍由调用方使用全局默认。这会直接影响系统提示词、记忆、Wiki、历史消息与工具 schema 的预算分配。

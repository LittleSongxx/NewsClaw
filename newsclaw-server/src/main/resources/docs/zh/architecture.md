...
**NewsClaw 是怎么拼起来的，一页讲完。**

**用** NewsClaw 的人看 [项目介绍](./intro)。**在 NewsClaw 上面建东西**的人——加工具、新渠道、自定义记忆 provider、新的 Agent 图节点——看这一页。
...
```
newsclaw/
├── newsclaw-server/          # Spring Boot 后端（心脏）
...
│       ├── skills/           # 捆绑的技能包
│       └── static/           # 前端构建产物
├── newsclaw-ui/              # Vue 3 管理控制台
├── newsclaw-desktop/         # Electron 桌面壳
├── newsclaw-webchat/         # 可嵌入的聊天小部件
...
**NewsClaw 的 Agent 运行时不是一个类层次。** 没有 `BaseAgent` → `ReActAgent` → `MyCustomAgent` 的继承链。运行时是一张**由节点和条件边组成的 StateGraph**（来自 `spring-ai-alibaba-graph`），在运行时由 `AgentGraphBuilder` 装配。
...
- `agent/graph/state/NewsClawStateKeys.java`——共享 state 对象的 key
- `agent/graph/state/NewsClawStateAccessor.java`——state map 的类型化访问器（**别直接动 map**）
...
**加 Agent 行为**——在 `agent/graph/node/` 创建新节点，或在 `agent/graph/edge/` 创建新边 dispatcher。把它接进 `AgentGraphBuilder`。通过 `NewsClawStateAccessor` 读写 state。
...
实现 `vip.newsclaw.channel.ChannelAdapter`（或支持流式的 `StreamingChannelAdapter`）。注册成 Spring bean。通过 `ChannelWebhookController` 加 webhook 端点。见 [多渠道接入](./channels)。
...
实现 `vip.newsclaw.memory.spi.MemoryProvider` 来插入自定义记忆后端（向量、图、外部服务）。**多个 provider 可以在 Agent 上堆叠**。见 [记忆系统](./memory)。
...
更深的定制——在 `agent/graph/node/` 加新节点或在 `agent/graph/edge/` 加新 dispatcher。在配置 flag 后面接进 `AgentGraphBuilder`。State 访问走 `NewsClawStateAccessor`。

---

## 持久化 —— 一份 schema，两种数据库

NewsClaw 用 **MyBatis Plus**（不是 JPA）做数据库访问。约定：
...
- Flyway 在启动时校验迁移 checksum；`FlywayRepairConfig` 只有显式打开 `newsclaw.flyway.auto-repair` 才会执行 `repair()`。生产环境应保持关闭，让脚本漂移和部分失败清晰暴露
...
NewsClaw 用 **Spring MVC**，不是 Spring WebFlux。**WebFlux 在依赖图里被明确排除。**
...
2. **桌面**——`newsclaw-desktop/` 里的 Electron 壳捆绑 JRE 21 和 JAR。**用户永远不装 Java**。通过 electron-updater 自动更新。
3. **Docker**——`docker-compose.yml` 带 MySQL。生产部署。

`newsclaw-webchat/` 里的 webchat widget 是第四条路——一个你可以丢进任何网站的可嵌入聊天 UI。它和同样的后端 API 对话。

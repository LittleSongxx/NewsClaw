...
| `content` | UTF-8 文本（默认单文件 ≤1 MB、bundle ≤50 MB，可通过 `newsclaw.skill.upload.max-entry-size-mb` / `max-total-size-mb` 调整） |
...
- **两遍扫描**——先把所有条目缓存（受总大小上限保护，默认 50 MB，可用 `newsclaw.skill.upload.max-total-size-mb` 调整），定位 `SKILL.md` 算出 wrapper 前缀，再分类。**条目顺序不再影响结果**。
...
技能装进来不一定能用——它可能需要某个 API key、某个 CLI 工具、某个 NewsClaw 开关被打开。
...
这个菜单的显示条件只看**当前选中了员工、且该员工没有关闭技能**（前端 `currentAgent && !skillsDisabled`）——和全局的渐进式披露开关无关。全局把 `newsclaw.skill.disclosure.load-skill-tool.enabled` 设为 `false` 只会让后端不注册 `load_skill` 工具，菜单照样弹（员工会回退用 `readSkillFile` 之类的方式拉技能）。
...
ACP（Agent Client Protocol）是一种把外部 Agent 客户端（Claude Code、Codex、其他兼容客户端）以技能身份接入 NewsClaw 的协议。

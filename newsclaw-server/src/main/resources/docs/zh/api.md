...
本页以 `newsclaw-server/src/main/java` 下的 Spring MVC Controller 注解为准。下面的路由索引由源码注解重建；如果它和旧功能页冲突，以本页和源码为接口契约。
...
源码：`vip.newsclaw.common.result.R`（`R.java`）。三个字段：
...
源码：`vip.newsclaw.common.result.ResultCode`。注意区分两类：
...
**滑动续期**：JWT 接近过期（默认剩余 < 2 小时）时，响应头回传新 token —— `X-New-Token: <newJwt>`（同时设 `Access-Control-Expose-Headers: X-New-Token`）。客户端应监听并替换本地存储的 token。JWT TTL 默认 24 小时（`newsclaw.jwt.expiration=86400000`）。

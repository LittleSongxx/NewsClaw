...
NewsClaw 后端集成了 [SpringDoc OpenAPI](https://springdoc.org/)（`springdoc-openapi-starter-webmvc-ui`），自动把所有 `@RestController` 的端点生成为 OpenAPI 3 文档，并提供在线调试 UI。本页说明如何访问和使用。
...
全局 OpenAPI 元信息（标题、描述、版本、服务器地址）由 `OpenApiConfig` Bean 驱动，可通过 `application.yml` 的 `newsclaw.openapi.*` 覆盖：
...
newsclaw:
  openapi:
    ...
    server-url: ${NEWSCLAW_OPENAPI_SERVER_URL:}   # 留空则从请求 host 推导
    description: ${NEWSCLAW_OPENAPI_DESCRIPTION:}  # 留空则用内置默认描述
    expose-ui: ${NEWSCLAW_OPENAPI_EXPOSE_UI:true}  # 是否公开 Swagger/OpenAPI 路径，见下方安全章节
```

`server-url` 留空时由 SpringDoc 从请求 host 推导，避免 "Try it out" 打到错误地址；生产若需固定（如反代后），设 `NEWSCLAW_OPENAPI_SERVER_URL=https://newsclaw.example.com`。

## 🔒 访问控制：Swagger 生产默认收口

Swagger UI / OpenAPI 文档路径（`/swagger-ui*`、`/v3/api-docs*`、`/webjars/**`）的访问由 `newsclaw.openapi.expose-ui` 开关控制，并由 `SecurityConfig.filterChain` 显式强制（不再依赖 `.anyRequest().permitAll()` 兜底）：
...
- 公网生产：默认 `false`，已收口。如确需在内网/预发环境临时打开，设 `NEWSCLAW_OPENAPI_EXPOSE_UI=true`。

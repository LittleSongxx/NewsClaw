...
import vip.newsclaw.auth.sso.provider.SsoProvider;
...
public class SsoService {
    ...
    public Map<String, String> handleAuthorize(String providerId) {
        SsoProvider provider = registry.get(providerId)
                .orElseThrow(() -> new NewsClawException("err.sso.unknown_provider",
    ...
    public SsoCallbackResponse handleCallback(String providerId, String code, String state) {
        ...
        if (identity != null) {
            ...
            if (user == null || !Boolean.TRUE.equals(user.getEnabled())) {
                throw new NewsClawException("err.sso.account_disabled",
                        403, "账号已停用或不存在");
            }
    ...
    public LoginResponse handleBind(String bindToken, String username, String password) {
        ...
        if (user == null || user.getPassword() == null
            ...
            throw new NewsClawException("err.auth.invalid_credentials",
                    401, "用户名或密码错误");
        }
        ...
        try {
            ...
            throw new NewsClawException("err.sso.already_bound",
                    409, "该飞书账号已绑定到其他用户");
        }
    ...
    private UserEntity createSsoUser(String providerId, SsoUserInfo info, boolean retry) {
        ...
        try {
            ...
            if (retry) {
                throw new NewsClawException("err.sso.concurrent_create_failed",

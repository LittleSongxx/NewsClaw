...
import vip.newsclaw.auth.sso.repository.SsoStateMapper;
...
public class SsoStateService {
    ...
    public void verifyState(String state) {
        ...
        if (dot <= 0 || dot >= state.length() - 1) {
            throw new NewsClawException("err.sso.state_invalid", 400, "state 格式无效");
        }
        ...
        if (!expected.equals(signature)) {
            throw new NewsClawException("err.sso.state_invalid", 400, "state 签名校验失败");
        }
    ...
    public BindTokenClaims verifyBindToken(String bindToken) {
        if (bindToken == null || bindToken.isBlank()) {
            throw new NewsClawException("err.sso.bind_token_missing", 400, "缺少 bind_token");
        }
        // 1. 验签 + 过期
        Claims claims;
        try {
            ...
            throw new NewsClawException("err.sso.bind_token_invalid",
                    400, "bind_token 无效或已过期");
        }

        String jti = claims.getId();
        if (jti == null) {
            throw new NewsClawException("err.sso.bind_token_invalid", 400, "bind_token 缺少 jti");
        }
        ...
        try {
            ...
            throw new NewsClawException("err.sso.bind_token_used",

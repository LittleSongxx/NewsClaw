...
import vip.newsclaw.llm.embedding.EmbeddingModelFactory;
...
public class WikiEmbeddingService {
    ...
    private final vip.newsclaw.llm.service.ModelProviderService modelProviderService;
     ...
     * {@code newsclaw.wiki.embedding-text-version-current} property exists only
    ...
    void verifyConfiguredInputVersion() {
        ...
        if (cmp == 0) {
            ...
            log.warn("[WikiEmbedding] Configured embedding input version {} is older than builder {}; "
                    ...
                    + "Clear newsclaw.wiki.embedding-text-version-current to use the builder default.",
            ...
            throw new IllegalStateException(
                    "Configured embedding input version " + configured + " is newer than builder version "
                            ...
                            + "upgrade the application or clear newsclaw.wiki.embedding-text-version-current.");
        }
    }
    ...
    private vip.newsclaw.wiki.job.WikiEmbeddingProviderFailingException circuitOpen(
        ...
        return new vip.newsclaw.wiki.job.WikiEmbeddingProviderFailingException(message, failures, remaining);
    }

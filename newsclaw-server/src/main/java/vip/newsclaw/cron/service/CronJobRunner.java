...
public class CronJobRunner {
    ...
    public void executeJob(CronJobEntity job, String triggerType) {
        ...
        try {
            started = lifecycle.startRun(job, userMessage, triggerType, conversationId,
                    buildRunIdempotencyKey(job, triggerType));
        } catch (Exception e) {
        ...
        if (!started.created()) {
            log.info("[CronRunner] Suppressed duplicate {} run for job {} (run={})",
                    triggerType, job.getId(), run.getId());
            return;
        }
    ...
    private void executeWikiProcess(CronJobEntity job, String triggerType) {
        ...
        try {
            CronJobLifecycleService.SystemStartResult started = lifecycle.startSystemRun(
                    job, triggerType, buildRunIdempotencyKey(job, triggerType));
            run = started.run();
            if (!started.created()) {
                log.info("[CronRunner] Suppressed duplicate {} system run for job {} (run={})",
                        triggerType, job.getId(), run.getId());
                return;
            }
        } catch (Exception e) {
    ...
    /**
     * Builds the durable key before the run row is inserted. Package-visible
     * for deterministic scheduler regression tests; callers must still enter
     * through {@link #executeJob(CronJobEntity, String)} so lifecycle state is
     * persisted before any long-running work begins.
     */
    static String buildRunIdempotencyKey(CronJobEntity job, String triggerType) {
        ...
        ZoneId zone = ZoneId.of(job != null && job.getTimezone() != null && !job.getTimezone().isBlank()
                ? job.getTimezone() : "Asia/Shanghai");
        LocalDateTime minute = LocalDateTime.now(zone).withSecond(0).withNano(0);
        return "scheduled:" + jobId + ":" + minute.format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm"));
    }
}

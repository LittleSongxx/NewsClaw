...
import vip.newsclaw.cron.delivery.CronJobCompletedEvent;
...
public class CronJobLifecycleService {
    ...
    public record StartResult(CronJobRunEntity run, Long originMessageId, boolean created) {
        /** Keeps existing tests and integration callers source-compatible. */
        public StartResult(CronJobRunEntity run, Long originMessageId) {
            this(run, originMessageId, true);
        }
    }

    public record SystemStartResult(CronJobRunEntity run, boolean created) {
    }
    ...
    public StartResult startRun(CronJobEntity job, String userMessage, String triggerType,
        ...
        return startRun(job, userMessage, triggerType, conversationId, null);
    }

    /**
     * T1 with a durable request key. A scheduled tick that is replayed by a
     * second node returns the original run and never repeats its long LLM call.
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public StartResult startRun(CronJobEntity job, String userMessage, String triggerType,
                                String conversationId, String idempotencyKey) {
        CronJobRunEntity existing = findByIdempotencyKey(job, idempotencyKey);
        if (existing != null) {
            return new StartResult(existing, null, false);
        }
        ...
        run.setIdempotencyKey(normalizeIdempotencyKey(idempotencyKey));
        run.setStartedAt(LocalDateTime.now());
        run.setDeliveryStatus("NONE");
        try {
            runMapper.insert(run);
        } catch (DuplicateKeyException duplicate) {
            CronJobRunEntity raced = findByIdempotencyKey(job, idempotencyKey);
            if (raced != null) {
                return new StartResult(raced, null, false);
            }
            throw duplicate;
        }
        ...
        return new StartResult(run, originMessageId, true);
    }
    ...
    public CronJobRunEntity startSystemRun(CronJobEntity job, String triggerType) {
        return startSystemRun(job, triggerType, null).run();
    }

    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public SystemStartResult startSystemRun(CronJobEntity job, String triggerType, String idempotencyKey) {
        CronJobRunEntity existing = findByIdempotencyKey(job, idempotencyKey);
        if (existing != null) {
            return new SystemStartResult(existing, false);
        }
        CronJobRunEntity run = new CronJobRunEntity();
        run.setCronJobId(job.getId());
        ...
        try {
            runMapper.insert(run);
            return new SystemStartResult(run, true);
        } catch (DuplicateKeyException duplicate) {
            CronJobRunEntity raced = findByIdempotencyKey(job, idempotencyKey);
            if (raced != null) {
                return new SystemStartResult(raced, false);
            }
            throw duplicate;
        }
    }
    ...
    private CronJobRunEntity findByIdempotencyKey(CronJobEntity job, String idempotencyKey) {
        if (job == null || job.getId() == null || idempotencyKey == null || idempotencyKey.isBlank()) {
            return null;
        }
        return runMapper.selectOne(new LambdaQueryWrapper<CronJobRunEntity>()
                .eq(CronJobRunEntity::getCronJobId, job.getId())
                .eq(CronJobRunEntity::getIdempotencyKey, idempotencyKey));
    }
    ...
    private static String normalizeIdempotencyKey(String idempotencyKey) {
        ...
        return normalized.length() <= 191 ? normalized : normalized.substring(0, 191);
    }
}

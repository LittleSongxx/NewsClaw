...
public class CronJobService implements ApplicationRunner {
     ...
     * JDBC provider is configured in {@link vip.newsclaw.cron.config.ShedLockConfig}
    ...
    public CronJobDTO update(Long id, CronJobDTO dto, Long workspaceId) {
        ...
        if (newName != null && newAgentId != null) {
            ...
            if (collision != null) {
                throw new NewsClawException("err.cron.duplicate_name",
    ...
    private String toSpringCron(String cron) {
        ...
        if (parts.length != 5) {
            throw new NewsClawException("Cron 表达式必须是 5 字段（分 时 日 月 周）");
        }
        // 标准化 day-of-week
        ...
        try {
            ...
            throw new NewsClawException("Cron 表达式非法: " + e.getMessage());
        }
        return springCron;
    }
    ...
    private void validateDto(CronJobDTO dto, Long workspaceId) {
        ...
        if (dto.getCronExpression() == null || dto.getCronExpression().isBlank()) {
            throw new NewsClawException("err.cron.expression_required", "Cron 表达式不能为空");
        }
        ...
        if (!"wiki_process".equals(taskType) && dto.getAgentId() == null) {
            throw new NewsClawException("err.cron.agent_required", "请选择关联 Agent");
        }
        ...
        if (("text".equals(taskType) || "reminder".equals(taskType))
            ...
            throw new NewsClawException("err.cron.trigger_required", "触发消息不能为空");
        }
        ...
        try {
            ...
            if (v.isNumber()) {
                ...
                try {
                    ...
                    throw new NewsClawException("err.cron.wiki_kb_invalid", "知识库 ID 格式不合法");
                }
            } else {
                throw new NewsClawException("err.cron.wiki_kb_invalid", "知识库 ID 格式不合法");
            }

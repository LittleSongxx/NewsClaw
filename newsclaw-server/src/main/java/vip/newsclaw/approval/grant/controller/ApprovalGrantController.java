package vip.newsclaw.approval.grant.controller;
...
import vip.newsclaw.approval.grant.entity.ApprovalGrant;
import vip.newsclaw.approval.grant.entity.ApprovalResolutionLog;
import vip.newsclaw.agent.repository.AgentMapper;
import vip.newsclaw.approval.grant.repository.ApprovalGrantMapper;
import vip.newsclaw.approval.grant.repository.ApprovalResolutionLogMapper;
import vip.newsclaw.approval.grant.service.ApprovalGrantService;
...
public class ApprovalGrantController {
    ...
    public R<Void> revoke(@PathVariable Long id,
        ...
        if (existing == null || (existing.getDeleted() != null && existing.getDeleted() == 1)) {
            throw new NewsClawException("err.approval.grant_not_found", 404, "grant not found");
        }
        ...
        if (!isOwner && !isAdmin) {
            throw new NewsClawException("err.approval.revoke_forbidden", 403,
    ...
    /**
     * Enforces the §2.4.5 6-cell matrix. Throws {@link NewsClawException} with
        ...
        switch (scope) {
            ...
            case ApprovalGrant.ScopeType.USER -> {
                if (body.scopeId == null || !body.scopeId.equals(String.valueOf(actorId))) {
                    throw new NewsClawException("err.approval.user_scope_self_only", 403,
            ...
            case ApprovalGrant.ScopeType.WORKSPACE -> {
                ...
                if (!String.valueOf(workspaceId).equals(body.scopeId)) {
                    throw new NewsClawException("err.approval.workspace_scope_mismatch", 400,
            ...
            default -> throw new NewsClawException("err.approval.invalid_scope", 400,
    ...
    private void requireExistingAgent(String scopeId) {
        ...
        try {
            ...
            throw new NewsClawException("err.approval.agent_not_found", 400,
    ...
    private void requireAdminPlusPassword(boolean isAdmin, String rawPassword, Long actorId) {
        if (!isAdmin) {
            throw new NewsClawException("err.approval.admin_required", 403, "admin role required");
        }
        if (rawPassword == null || rawPassword.isEmpty()) {
            throw new NewsClawException("err.approval.password_required", 403,
    ...
    private static void validate(CreateGrantRequest body) {
        if (body.scopeType == null || body.scopeType.isEmpty()) {
            throw new NewsClawException("err.approval.scope_type_required", 400, "scope_type is required");
        }
        if (body.scopeId == null || body.scopeId.isEmpty()) {
            throw new NewsClawException("err.approval.scope_id_required", 400, "scope_id is required");
        }
        if (body.maxSeverity == null
            ...
            throw new NewsClawException("err.approval.invalid_severity", 400,
        ...
        if (body.grantKind == null
            ...
            throw new NewsClawException("err.approval.invalid_grant_kind", 400,
        ...
        if ("UNTIL_TIMESTAMP".equals(body.grantKind) && body.expireAt == null) {
            throw new NewsClawException("err.approval.expire_at_required", 400,
        ...
        if ("UNTIL_CONVERSATION_END".equals(body.grantKind)
            ...
            throw new NewsClawException("err.approval.kind_scope_mismatch", 400,
    ...
    private Long resolveUserId(Authentication auth) {
        if (auth == null || auth.getName() == null) {
            throw new NewsClawException("err.auth.unauthenticated", 401, "未登录");
        }

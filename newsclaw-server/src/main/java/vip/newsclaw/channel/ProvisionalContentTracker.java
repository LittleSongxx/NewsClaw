...
public final class ProvisionalContentTracker {
    ...
    private static final String METRIC_SUPERSEDED = "newsclaw.narration.superseded";
    ...
    public String stageNarration(String text, ContentKind kind) {
        ...
        boolean duplicate = sameNarration(previous, text);
        pendingText = text;
        ...
        // A model may replay the same stage narration while it is waiting for
        // a tool result. Treat that as an idempotent refresh, not as a signal
        // that the previous (possibly provisional) line is now publishable.
        // Publishing it here would promote an unverified rehearsal into the
        // Feishu append-only history and make every later snapshot repeat it.
        if (duplicate) {
            return null;
        }
    ...
    /**
     * Whether the narration most recently staged by {@link #stageNarration}
     * is provisional. Channel renderers use this to keep a rehearsal in a
     * replaceable live slot instead of permanently appending it to a card.
     */
    public boolean pendingIsProvisional() {
    ...
    private static boolean sameNarration(String left, String right) {
        if (left == null || right == null) {
            return left == right;
        }
        return normalizeNarration(left).equals(normalizeNarration(right));
    }

    private static String normalizeNarration(String text) {
        return text.trim()
                .replaceAll("\\s+", " ")
                // Model gateways occasionally insert a space before or after
                // Chinese punctuation when replaying the same span.
                .replaceAll("\\s+([，。！？；：、])", "$1")
                .replaceAll("([，。！？；：、])\\s+", "$1");
    }

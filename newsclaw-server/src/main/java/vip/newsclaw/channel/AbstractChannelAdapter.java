...
import vip.newsclaw.channel.health.ChannelHealth;
...
public abstract class AbstractChannelAdapter implements ChannelAdapter {
    ...
    protected AbstractChannelAdapter(ChannelEntity channelEntity,
        ...
        this.config = new java.util.LinkedHashMap<>(parseConfig(channelEntity.getConfigJson()));
        // Keep credentials out of channel rows when Docker is the source of
        // truth.  Non-blank environment values override the row only for the
        // supported domestic IM adapters; all other channel options retain the
        // existing database configuration.
        EnvironmentConfig.applyChannelOverrides(channelEntity.getChannelType(), this.config);
    }
    ...
    public void sendApprovalNotice(String targetId,
        ...
        sendMessage(targetId,
                vip.newsclaw.channel.notification.ApprovalNotificationService.staticBuildText(notice));
    }

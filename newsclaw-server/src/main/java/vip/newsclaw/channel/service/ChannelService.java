...
public class ChannelService {
     ...
     */
    private final ObjectProvider<vip.newsclaw.channel.tool.ChannelToolService> channelToolServiceProvider;
    ...
    public ChannelEntity getChannel(Long id) {
        ...
        if (channel == null) {
            throw new NewsClawException("err.channel.not_found", "渠道不存在: " + id);
        }
        return channel;
    }
    ...
    public ChannelEntity createChannel(ChannelEntity channel) {
        ...
        if (channel.getName() == null || channel.getName().isBlank()) {
            throw new NewsClawException("err.channel.name_required", "渠道名称不能为空");
        }
        if (channel.getChannelType() == null || channel.getChannelType().isBlank()) {
            throw new NewsClawException("err.channel.type_required", "渠道类型不能为空");
        }
    ...
    private void invalidateChannelCaches(Long channelId, String channelType) {
        ...
        vip.newsclaw.channel.tool.ChannelToolService cts =
    ...
    private String enrichWebChatConfig(String incomingConfigJson, String existingConfigJson) {
        ...
        try {
            ...
            throw new NewsClawException("WebChat 渠道配置序列化失败: " + e.getMessage());
        }
    }

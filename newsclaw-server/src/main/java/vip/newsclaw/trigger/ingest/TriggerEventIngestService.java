...
import vip.newsclaw.trigger.repository.TriggerEventMapper;
...
public class TriggerEventIngestService {
    ...
    @Value("${newsclaw.workflow.trigger.async-dispatch:true}")
    private boolean asyncDispatch;

    @Value("${newsclaw.workflow.trigger.dispatch-pool-size:8}")
    private int dispatchPoolSize;

    @Value("${newsclaw.workflow.trigger.dispatch-queue-capacity:256}")
    ...
    private IngestResult processSingle(TriggerEntity trigger, TriggerEventEnvelope envelope) {
        ...
        // explicitly opts out via newsclaw.workflow.trigger.async-dispatch=false.
     ...
     * {@code newsclaw.workflow.trigger.dedup-sweep-interval-ms}. The
     * initial delay matches the cadence so a JVM that just started doesn't
     * race {@code recordDedupRow} for the same window.
     */
    @Scheduled(
            fixedDelayString = "${newsclaw.workflow.trigger.dedup-sweep-interval-ms:300000}",
            initialDelayString = "${newsclaw.workflow.trigger.dedup-sweep-initial-delay-ms:300000}")

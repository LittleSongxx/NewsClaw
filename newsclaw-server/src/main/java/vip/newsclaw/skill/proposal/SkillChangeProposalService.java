...
import vip.newsclaw.skill.model.SkillOrigin;
import vip.newsclaw.skill.runtime.SkillSecurityService;
import vip.newsclaw.skill.runtime.SkillValidationResult;
import vip.newsclaw.skill.routine.model.SkillRoutineCandidateEntity;
import vip.newsclaw.skill.routine.repository.SkillRoutineCandidateMapper;
import vip.newsclaw.skill.service.SkillService;
import vip.newsclaw.tool.builtin.SkillManageTool;
...
public class SkillChangeProposalService {
    ...
    public SkillChangeProposalEntity propose(SkillProposalDraft draft) {
        if (draft == null) {
            throw new NewsClawException(400, "Skill proposal is required");
        }
        ...
        if (after.length() > MAX_CONTENT_CHARS) {
            throw new NewsClawException(400, "Skill proposal content exceeds " + MAX_CONTENT_CHARS + " characters");
        }
        if (nullToEmpty(before).equals(after)) {
            throw new NewsClawException(409, "Skill proposal does not change the current content");
        }
    ...
    public SkillChangeProposalEntity get(Long workspaceId, Long id) {
        SkillChangeProposalEntity proposal = proposalMapper.selectOne(new LambdaQueryWrapper<SkillChangeProposalEntity>()
                ...
                .eq(SkillChangeProposalEntity::getDeleted, 0));
        if (proposal == null) {
            throw new NewsClawException(404, "Skill proposal not found");
        }
        return proposal;
    }
    ...
    public SkillChangeProposalEntity approve(Long workspaceId, Long id, SkillProposalReviewRequest review) {
        ...
        if (STATUS_BLOCKED.equals(proposal.getStatus())) {
            throw new NewsClawException(409, "Security-blocked proposal cannot be approved");
        }
        if (STATUS_REJECTED.equals(proposal.getStatus())) {
            throw new NewsClawException(409, "Rejected proposal cannot be approved");
        }
    ...
    public SkillChangeProposalEntity reject(Long workspaceId, Long id, SkillProposalReviewRequest review) {
        SkillChangeProposalEntity proposal = get(workspaceId, id);
        if (STATUS_APPLIED.equals(proposal.getStatus())) {
            throw new NewsClawException(409, "Applied proposal must be rolled back, not rejected");
        }
    ...
    public SkillChangeProposalEntity apply(Long workspaceId, Long id) {
        ...
        if (!STATUS_APPROVED.equals(proposal.getStatus())) {
            throw new NewsClawException(409, "Only an approved proposal can be applied");
        }
        ...
        if (claimed != 1) {
            ...
            throw new NewsClawException(409, "Skill proposal is being applied by another request");
        }
        ...
        if (!isSuccess(result)) {
            throw new NewsClawException(409, "Skill proposal could not be applied: " + trim(result, 800));
        }
    ...
    public SkillChangeProposalEntity rollback(Long workspaceId, Long id, String reviewer, String note) {
        SkillChangeProposalEntity proposal = get(workspaceId, id);
        if (!STATUS_APPLIED.equals(proposal.getStatus())) {
            throw new NewsClawException(409, "Only an applied proposal can be rolled back");
        }
        ...
        if (proposal.getSnapshotId() != null) {
            ...
            if (!isSuccess(result)) {
                throw new NewsClawException(409, "Created skill could not be archived during rollback: " + trim(result, 800));
            }
        } else {
            throw new NewsClawException(409, "Proposal has no usable restore snapshot");
        }
    ...
    private static String materializeAfter(String action, SkillProposalDraft draft, String before) {
        return switch (action) {
            ...
            default -> throw new NewsClawException(400, "Unsupported Skill proposal action: " + action);
        };
    }
    ...
    private static void validateActionTarget(String action, String name, SkillEntity existing) {
        if ("create".equals(action) && existing != null) {
            throw new NewsClawException(409, "Skill '" + name + "' already exists; use edit or patch");
        }
        if (!"create".equals(action) && existing == null) {
            throw new NewsClawException(404, "Skill '" + name + "' does not exist; use create");
        }
        if (existing != null && Boolean.TRUE.equals(existing.getBuiltin())) {
            throw new NewsClawException(409, "Builtin Skill '" + name + "' cannot be changed by a proposal");
        }
    }

    private static String requireContent(String content) {
        if (content == null || content.isBlank()) {
            throw new NewsClawException(400, "Proposal content is required");
        }
        return content.trim();
    }
    ...
    private static String patchOnce(String before, String oldText, String newText) {
        if (before == null || before.isBlank() || oldText == null || oldText.isBlank() || newText == null) {
            throw new NewsClawException(400, "Patch proposal needs an existing Skill, oldText and newText");
        }
        int first = before.indexOf(oldText);
        if (first < 0 || before.indexOf(oldText, first + oldText.length()) >= 0) {
            throw new NewsClawException(409, "Patch proposal target must exist exactly once");
        }
    ...
    private static String normalizeAction(String value) {
        ...
        if (!"create".equals(action) && !"edit".equals(action) && !"patch".equals(action)) {
            throw new NewsClawException(400, "Only create, edit and patch Skill proposals are supported");
        }
        return action;
    }
    ...
    private static String normalizeName(String value) {
        ...
        if (!name.matches("^[a-z0-9][a-z0-9._-]{0,63}$")) {
            throw new NewsClawException(400, "Invalid Skill proposal name");
        }
        return name;
    }

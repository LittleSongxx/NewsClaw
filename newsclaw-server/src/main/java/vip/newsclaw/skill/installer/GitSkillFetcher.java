...
public class GitSkillFetcher {
     ...
     * Resolution order: {@code newsclaw.skill.github-token} property → {@code GITHUB_TOKEN}
    ...
    public GitSkillFetcher(
            ...
            @Value("${newsclaw.skill.github-token:}") String configuredGithubToken) {
    ...
    public SkillBundle fetch(String repoUrl, String ref, String subPath) {
        Path tempDir = null;
        try {
            tempDir = Files.createTempDirectory("newsclaw-skill-install-");

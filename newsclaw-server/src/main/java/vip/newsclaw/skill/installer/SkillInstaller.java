package vip.newsclaw.skill.installer;
...
import vip.newsclaw.skill.installer.model.*;
...
import vip.newsclaw.skill.workspace.SkillWorkspaceEvent;
 ...
 * <p>
 * 支持异步安装（task_id 轮询模式），参考 NewsClaw 实现。
 *
...
public class SkillInstaller {
    ...
    public Map<String, Object> installFromBundle(SkillBundle bundle, boolean enable, boolean overwrite,
        ...
        if (skillName == null || skillName.isBlank()) {
            throw new vip.newsclaw.exception.NewsClawException("err.skill.name_required", "Cannot determine skill name from bundle");
        }
        ...
        boolean exists = skillService.listSkills(workspaceId).stream()
                .anyMatch(s -> s.getName().equals(skillName));
        if (exists && !overwrite) {
            throw new vip.newsclaw.exception.NewsClawException("err.skill.name_exists",

...
/**
 * Turns a qualified recurring-request cluster into a reviewable, class-level
 * skill proposal.
 *
 ...
 *
 * <p>The routine miner never writes a live skill. It records the synthesized
 * diff with its cross-session evidence; a workspace administrator approves it
 * before the normal skill-management pipeline performs a security scan,
...
public class SkillRoutinePromoter {
    ...
    /** Proposal-first sink. Optional setter preserves existing isolated constructors. */
    ...
    private boolean promote(SkillRoutineCandidateEntity candidate) {
        ...
        if (proposalService == null) {
            log.warn("[SkillRoutine] Proposal service unavailable; candidate {} will not mutate Skill registry",
                    candidate.getId());
            return false;
        }
        SkillChangeProposalEntity proposal;
        try {
            proposal = proposalService.propose(new SkillProposalDraft(candidate.getWorkspaceId(), candidate.getAgentId(),
                    "ROUTINE", conversationIds.isEmpty() ? null : conversationIds.get(0), candidate.getId(),
                    "create", name, content, null, null,
                    "routine=" + candidate.getSignature() + "; occurrences=" + candidate.getOccurrenceCount()
                            + "; distinctDays=" + candidate.getDistinctDayCount()));
        } catch (Exception e) {
            log.info("[SkillRoutine] Candidate {} proposal rejected: {}", candidate.getId(), e.getMessage());
            return false;
        }

        candidate.setStatus(SkillRoutineCandidateEntity.STATUS_PROPOSED);
        candidate.setPromotedSkillName(name);
        candidate.setProposalId(proposal.getId());
        candidate.setPromotedAt(LocalDateTime.now());
        candidateMapper.updateById(candidate);
        log.info("[SkillRoutine] Proposed routine '{}' → skill '{}' for agent={} ({} occurrences over {} days)",

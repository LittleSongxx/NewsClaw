...
public class SkillRoutineJob {
    ...
    @Scheduled(cron = "${newsclaw.skill.routine.cron:0 0 3 * * *}")
    ...
    public void run() {
        ...
        try {
            ...
            int proposed = properties.isAutoPromote() ? promoter.promoteQualified() : 0;
            if (mined > 0 || proposed > 0) {
                log.info("[SkillRoutine] Sweep complete — {} candidate(s) refreshed, {} proposal(s) created",
                        mined, proposed);
            }
        } catch (Exception e) {

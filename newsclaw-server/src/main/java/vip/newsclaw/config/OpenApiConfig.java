 ...
 * {@code newsclaw.openapi.expose-ui} 开关控制：本地/默认 profile 公开，
...
public class OpenApiConfig {
    ...
    @Bean
    public OpenAPI newsclawOpenAPI(
            @Value("${newsclaw.openapi.title:NewsClaw REST API}") String title,
            @Value("${newsclaw.openapi.description:#{null}}") String description,
            @Value("${newsclaw.openapi.version:1.0}") String version,
            @Value("${newsclaw.openapi.server-url:}") String serverUrl) {

        Info info = new Info()
                .title(title)
                .version(version)
                .description(defaultIfBlank(description, ""
                        + "NewsClaw 多用户 AI Agent 平台的 REST API。"

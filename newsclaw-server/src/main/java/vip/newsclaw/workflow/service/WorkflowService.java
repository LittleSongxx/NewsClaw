package vip.newsclaw.workflow.service;
...
import vip.newsclaw.workflow.compiler.WorkflowCompiler;
import vip.newsclaw.workflow.model.WorkflowEntity;
import vip.newsclaw.workflow.model.WorkflowRevisionEntity;
import vip.newsclaw.workflow.repository.WorkflowMapper;
import vip.newsclaw.workflow.repository.WorkflowRevisionMapper;
...
public class WorkflowService {
     ...
     * {@link vip.newsclaw.workflow.compiler.WorkflowCompileFailedException} when
    ...
    private void requireUniqueName(Long workspaceId, String name, Long excludeId) {
        if (name == null || name.isBlank()) {
            throw new NewsClawException("err.workflow.name_required", 400,
                    "工作流名称不能为空");
        }
        if (workspaceId == null) {
            return;
        ...
        if (count != null && count > 0) {
            throw new NewsClawException("err.workflow.duplicate_name", 409,
                    "工作区内已存在同名工作流: " + name);
        }
    }

package vip.newsclaw.kbopen.controller;
...
import vip.newsclaw.kbopen.auth.KbApiKeyContext;
import vip.newsclaw.kbopen.auth.RequireKbScope;
import vip.newsclaw.kbopen.research.KbResearchSessionRegistry;
import vip.newsclaw.kbopen.research.KbResearchSessionRegistry.Session;
import vip.newsclaw.kbopen.research.KbResearchSessionRegistry.Status;
import vip.newsclaw.kbopen.research.KbResearchSessionRegistry.TooManyConcurrentException;
import vip.newsclaw.wiki.service.WikiKnowledgeBaseService;
import vip.newsclaw.wiki.service.WikiResearchService;
import vip.newsclaw.wiki.service.WikiResearchService.ResearchResult;
 ...
 *   <li>Per-key <b>concurrency cap</b> ({@code newsclaw.kbopen.research.max-concurrent-per-key})
...
public class KbOpenResearchController {
    ...
    public R<Map<String, Object>> startResearch(
        ...
        if (topic == null || topic.isBlank()) {
            throw new NewsClawException(400, "topic is required");
        }
        if (kbService.getById(kbId) == null) {
            throw new NewsClawException(404, "Knowledge base not found: " + kbId);
        }
        ...
        try {
            ...
            throw new NewsClawException(429, e.getMessage());
        }
    ...
    public R<Map<String, Object>> cancel(
        ...
        if (session.status() != Status.RUNNING) {
            throw new NewsClawException(409, "Session is not running (status: " + session.status() + ")");
        }
    ...
    private Session requireSessionOwnership(HttpServletRequest request, Long kbId, String sessionId) {
        ...
        if (session.isEmpty()) {
            throw new NewsClawException(404, "Research session not found: " + sessionId);
        }
        // A caller can only access sessions they started
        if (!session.get().keyId().equals(ctx.keyId())) {
            throw new NewsClawException(403, "Session does not belong to this API key");
        }

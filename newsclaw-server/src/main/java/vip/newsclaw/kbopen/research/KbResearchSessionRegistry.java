package vip.newsclaw.kbopen.research;
 ...
 *       {@code newsclaw.kbopen.research.session-ttl} (default 30 min) so the map
 *       cannot grow without bound.</li>
 *   <li>{@link #startIfAllowed} enforces a per-key concurrency cap
 *       ({@code newsclaw.kbopen.research.max-concurrent-per-key}, default 3) as a
...
public class KbResearchSessionRegistry {
    ...
    public KbResearchSessionRegistry(
            @Value("${newsclaw.kbopen.research.max-concurrent-per-key:3}") int maxConcurrentPerKey,
            @Value("${newsclaw.kbopen.research.session-ttl:PT30M}") Duration sessionTtl) {

...
public class ActionNode implements NodeAction {
    ...
    private final vip.newsclaw.channel.web.ChatStreamTracker streamTracker;
    ...
    private vip.newsclaw.agent.progress.ProgressLedgerService progressLedgerService;

    public ActionNode(ToolExecutionExecutor executor) {
        this(executor, null);
    }

    public ActionNode(ToolExecutionExecutor executor, vip.newsclaw.channel.web.ChatStreamTracker streamTracker) {
    ...
    /** Setter injection so existing constructors stay source-compatible. */
    public void setProgressLedgerService(vip.newsclaw.agent.progress.ProgressLedgerService s) {
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        ...
        String currentPhase = state.value(NewsClawStateKeys.CURRENT_PHASE, "");
        ...
        // 获取工作区活动目录
        String workspaceBasePath = state.value(NewsClawStateKeys.WORKSPACE_BASE_PATH, "");
        ...
        vip.newsclaw.agent.context.ChatOrigin origin = accessor.chatOrigin();
        ...
        NewsClawStateAccessor.OutputBuilder output = NewsClawStateAccessor.output()
    ...
    private void pinSkillConstraints(String conversationId, Set<String> skillNames) {
        ...
        for (String skillName : skillNames) {
            try {
                vip.newsclaw.skill.runtime.model.ResolvedSkill skill = skillRuntimeService.findActiveSkill(
    ...
    private boolean loadedSkillsRequireAction(String conversationId, Set<String> skillNames) {
        ...
        for (String skillName : skillNames) {
            try {
                vip.newsclaw.skill.runtime.model.ResolvedSkill skill =
    ...
    static boolean manifestRequiresActionCompletion(vip.newsclaw.skill.manifest.SkillManifest manifest) {
    ...
    void autoRecordToolCalls(String conversationId,
        ...
        List<vip.newsclaw.agent.progress.ProgressLedgerService.AutoRecordEntry> batch = new java.util.ArrayList<>();
        for (ToolResponseMessage.ToolResponse resp : responses) {
            ...
            batch.add(new vip.newsclaw.agent.progress.ProgressLedgerService.AutoRecordEntry(

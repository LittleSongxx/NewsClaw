 ...
 */
public final class NewsClawStateKeys {

    private NewsClawStateKeys() {
    }
     ...
     * tighter cap ({@code newsclaw.goal.max-hard-continuations-per-run}, clamped
     * to {@link vip.newsclaw.goal.config.GoalProperties#MAX_HARD_CONTINUATIONS_CEILING})
     ...
     * read-only when invoking {@link vip.newsclaw.agent.graph.executor.ToolExecutionExecutor}

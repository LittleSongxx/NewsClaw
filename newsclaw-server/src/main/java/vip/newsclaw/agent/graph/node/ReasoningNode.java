...
import vip.newsclaw.agent.context.LoopMessageBudgeter;
import vip.newsclaw.agent.context.RuntimeContextInjector;
...
import vip.newsclaw.agent.graph.guard.ActionCompletionPolicy;
...
import static vip.newsclaw.agent.graph.state.NewsClawStateKeys.*;
...
public class ReasoningNode implements NodeAction {
    ...
    private static NewsClawStateAccessor.OutputBuilder reasonOutput() {
        return NewsClawStateAccessor.output().continueReasoning(false);
    }
    ...
    /**
     * File tools are only appropriate when the request affirmatively asks for
     * an artifact. A bare keyword is deliberately insufficient: requests such
     * as "不要导出 PDF，直接回复" still belong in the chat stream.
     */
    private static final Pattern EXPLICIT_ARTIFACT_REQUEST_PATTERN = Pattern.compile(
            "(?i)(?:"
                    + "(?:生成|创建|导出|下载|保存|写入|发送|提供|给我|输出|写成|转成|转为|以)"
                    + "\\s*(?:一份|一个|成|为)?\\s*(?:word|docx|pdf|pptx|xlsx|附件|文件|文档)"
                    + "|(?:需要|要|给我|提供)\\s*(?:一份|一个)?\\s*(?:word|docx|pdf|pptx|xlsx)"
                    + "(?:\\s*(?:文件|文档|附件))?"
                    + "|(?:word|docx|pdf|pptx|xlsx)\\s*(?:文件|文档|附件|格式)?"
                    + "\\s*(?:输出|导出|下载|生成|保存|发送)"
                    + "|(?:markdown|\\bmd\\b)\\s*(?:文件|附件)"
                    + ")");
    private static final Pattern NEGATED_ARTIFACT_PREFIX_PATTERN = Pattern.compile(
            "(?i)(?:不要|不需要|无需|不用|不必|别|禁止|不希望|不想|避免|not|without|不)\\s*$");
    ...
    static OptionalInt requestedLongFormChars(String userMessage) {
        if (userMessage == null || userMessage.isBlank()) return OptionalInt.empty();
        Matcher tenThousand = CHINESE_TEN_THOUSAND_CHARS_PATTERN.matcher(userMessage);
        if (tenThousand.find()) return OptionalInt.of(10_000);
        Matcher matcher = ARABIC_CHAR_COUNT_PATTERN.matcher(userMessage);
        int best = 0;
        while (matcher.find()) {
            String rawNumber = matcher.group(1).replace(",", "").replace("，", "");
            double value;
            try { value = Double.parseDouble(rawNumber); }
            catch (NumberFormatException ignored) { continue; }
            String unit = matcher.group(2);
            if ("万".equals(unit)) value *= 10_000;
            else if ("千".equals(unit) || "k".equals(unit) || "K".equals(unit)) value *= 1_000;
            best = Math.max(best, (int) Math.round(value));
        }
        return best >= LONG_FORM_MIN_REQUEST_CHARS ? OptionalInt.of(best) : OptionalInt.empty();
    }

    static List<ToolCallback> filterLongFormArtifactTools(String userMessage, List<ToolCallback> callbacks) {
        String currentRequest = currentUserRequest(userMessage);
        if (callbacks == null || callbacks.isEmpty()
                || requestedLongFormChars(currentRequest).isEmpty()
                || explicitlyRequestsArtifactDelivery(currentRequest)) return callbacks;
        return callbacks.stream().filter(callback -> {
    ...
    static boolean hasDisallowedLongFormArtifactCall(String userMessage,
        ...
        if (toolCalls == null || toolCalls.isEmpty()
                || requestedLongFormChars(currentRequest).isEmpty()
                || explicitlyRequestsArtifactDelivery(currentRequest)) return false;
    ...
    private static String currentUserRequest(String userMessage) {
        ...
        return memoryEnd >= 0 ? userMessage.substring(memoryEnd + "</memory-context>".length()).trim() : userMessage;
    }

    static boolean explicitlyRequestsArtifactDelivery(String request) {
        if (request == null || request.isBlank()) return false;
        Matcher matcher = EXPLICIT_ARTIFACT_REQUEST_PATTERN.matcher(request);
        while (matcher.find()) {
            String prefix = request.substring(Math.max(0, matcher.start() - 16), matcher.start());
            if (!NEGATED_ARTIFACT_PREFIX_PATTERN.matcher(prefix).find()) return true;
        }
        return false;
    }
    ...
    private static boolean shouldContinueLongForm(String userMessage, String longFormDraft,
        ...
        if (requested.isEmpty() || (maxIterations > 0 && iteration + 1 >= maxIterations)) return false;
    ...
    private final vip.newsclaw.wiki.service.WikiContextService wikiContextService;
    /**
     ...
     */
    private final vip.newsclaw.skill.runtime.SkillCatalogRenderer skillCatalogRenderer;
     ...
     */
    private vip.newsclaw.agent.runtime.RunningConversationRegistry runningConversationRegistry;

    public void setRunningConversationRegistry(
            vip.newsclaw.agent.runtime.RunningConversationRegistry runningConversationRegistry) {
    ...
    public ReasoningNode(ChatModel chatModel, AgentToolSet toolSet, String reasoningEffort,
                         ...
                         ConversationWindowManager conversationWindowManager,
                         ChatStreamTracker streamTracker, int maxOutputTokens,
                         vip.newsclaw.wiki.service.WikiContextService wikiContextService) {
    ...
    /**
     * Primary constructor with the runtime {@link vip.newsclaw.skill.runtime.SkillCatalogRenderer}.
    ...
    public ReasoningNode(ChatModel chatModel, AgentToolSet toolSet, String reasoningEffort,
                         ...
                         ChatStreamTracker streamTracker, int maxOutputTokens,
                         vip.newsclaw.wiki.service.WikiContextService wikiContextService,
                         vip.newsclaw.skill.runtime.SkillCatalogRenderer skillCatalogRenderer) {
        this(chatModel, toolSet, reasoningEffort, supportsReasoningEffort, streamingHelper,
     ...
     * {@link vip.newsclaw.agent.progress.ProgressLedgerService} was wired in —
    ...
    public ReasoningNode(ChatModel chatModel, AgentToolSet toolSet, String reasoningEffort,
                         ...
                         vip.newsclaw.wiki.service.WikiContextService wikiContextService,
                         vip.newsclaw.skill.runtime.SkillCatalogRenderer skillCatalogRenderer,
                         vip.newsclaw.tool.disclosure.ToolDisclosureService toolDisclosureService) {
        this(chatModel, toolSet, reasoningEffort, supportsReasoningEffort, streamingHelper,
                conversationWindowManager, streamTracker, maxOutputTokens, wikiContextService,
    ...
    /**
     * Primary constructor with the {@link vip.newsclaw.agent.progress.ProgressLedgerService}.
    ...
    public ReasoningNode(ChatModel chatModel, AgentToolSet toolSet, String reasoningEffort,
                         ...
                         vip.newsclaw.skill.runtime.SkillCatalogRenderer skillCatalogRenderer,
                         vip.newsclaw.tool.disclosure.ToolDisclosureService toolDisclosureService,
                         vip.newsclaw.agent.progress.ProgressLedgerService progressLedgerService) {
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        NewsClawStateAccessor accessor = new NewsClawStateAccessor(state);
        ...
        String workspaceBasePath = state.value(vip.newsclaw.agent.graph.state.NewsClawStateKeys.WORKSPACE_BASE_PATH, "");
        String agentIdStr = state.value(NewsClawStateKeys.AGENT_ID, "");
        String userMsg = state.value(NewsClawStateKeys.USER_MESSAGE, "");
        String runtimeModelName = state.value(NewsClawStateKeys.RUNTIME_MODEL_NAME, "");
        String runtimeProviderId = state.value(NewsClawStateKeys.RUNTIME_PROVIDER_ID, "");
        ...
        if (progressLedgerService != null && conversationId != null && !conversationId.isBlank()) {
            try {
                vip.newsclaw.agent.progress.ProgressLedger ledger =
        ...
        if (runningConversationRegistry != null && conversationId != null && !conversationId.isBlank()) {
            try {
                List<vip.newsclaw.agent.runtime.EnvironmentNotification> notes =
        ...
        try {
            ...
            if (result.isPromptTooLong() && conversationWindowManager != null) {
                ...
                // NewsClawStateAccessor.agentId() returns String per state
     ...
     * iteration-boundary toggle ({@code newsclaw.stream.iteration-events})
     ...
     */
    // Package-private so black-box tests in vip.newsclaw.agent.graph.node can
    ...
    static String renderEnvironmentNotifications(
        ...
        for (vip.newsclaw.agent.runtime.EnvironmentNotification n : notes) {
    ...
    List<Message> buildNonHistoryPrefix(String systemPrompt,
                                        ...
                                        String agentIdStr,
                                        String userMsg,
                                        vip.newsclaw.agent.context.ChatOrigin chatOrigin,
        ...
        boolean projectRecalled = userMsg != null
                && userMsg.contains(vip.newsclaw.memory.service.StructuredMemoryService.PROJECT_RECALLED_MARKER);
        if (projectRecalled) {

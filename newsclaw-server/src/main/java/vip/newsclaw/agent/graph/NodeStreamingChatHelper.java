...
public class NodeStreamingChatHelper {
     ...
     *
     * <p>Stored as {@link vip.newsclaw.llm.failover.FallbackEntry} (providerId +
     * ChatModel) so the chain walker can consult {@link vip.newsclaw.llm.failover.ProviderHealthTracker}
     * — cooldown state is keyed by providerId, not by ChatModel instance.</p>
     */
    private final List<vip.newsclaw.llm.failover.FallbackEntry> fallbackChain;

    /** Optional cache-metrics aggregator; {@code null} in tests or when the bean is absent. */
    private final vip.newsclaw.llm.cache.LlmCacheMetricsAggregator cacheMetrics;

    /** Optional per-provider health tracker; {@code null} in tests or when bean absent. */
    private final vip.newsclaw.llm.failover.ProviderHealthTracker healthTracker;
    ...
    /** Optional durable trace sink; absent in isolated unit tests and legacy embedders. */
    private vip.newsclaw.llm.trace.LlmRoutingTraceService routingTraceService;

    /** Stable graph-build context used by the durable trace sink. */
    private Long routingAgentId;
     ...
     * <p>
     * Defaults to {@link vip.newsclaw.llm.chatmodel.HttpTimeouts#DEFAULT_STREAM_IDLE_TIMEOUT}
    ...
    private long streamIdleTimeoutSec =
            vip.newsclaw.llm.chatmodel.HttpTimeouts.DEFAULT_STREAM_IDLE_TIMEOUT.toSeconds();
    ...
    @Deprecated
    public NodeStreamingChatHelper(ChatStreamTracker streamTracker, ChatModel fallbackModel,
                                   vip.newsclaw.llm.cache.LlmCacheMetricsAggregator cacheMetrics) {
     ...
     */
    public NodeStreamingChatHelper(ChatStreamTracker streamTracker,
                                   List<vip.newsclaw.llm.failover.FallbackEntry> fallbackChain,
                                   vip.newsclaw.llm.cache.LlmCacheMetricsAggregator cacheMetrics) {
    ...
    public NodeStreamingChatHelper(ChatStreamTracker streamTracker,
                                   List<vip.newsclaw.llm.failover.FallbackEntry> fallbackChain,
                                   vip.newsclaw.llm.cache.LlmCacheMetricsAggregator cacheMetrics,
                                   vip.newsclaw.llm.failover.ProviderHealthTracker healthTracker) {
     ...
     * the {@link vip.newsclaw.llm.failover.AvailableProviderPool} disabled. Kept
     ...
     * {@link vip.newsclaw.llm.failover.AvailableProviderPool} hookup gates both
    ...
    public NodeStreamingChatHelper(ChatStreamTracker streamTracker,
                                   ...
                                   vip.newsclaw.llm.failover.ProviderHealthTracker healthTracker,
                                   String primaryProviderId,
                                   vip.newsclaw.llm.failover.AvailableProviderPool providerPool) {
    ...
    /**
     * Attach durable routing observability without changing the constructor
     * matrix used by existing graph tests. The workspace is resolved from the
     * graph's agent by the trace service, which also keeps this helper free of
     * persistence dependencies.
     */
    public void setRoutingTraceContext(vip.newsclaw.llm.trace.LlmRoutingTraceService traceService,
    ...
    private static List<vip.newsclaw.llm.failover.FallbackEntry> wrap(ChatModel m) {
        ...
        return m == null ? List.of() : List.of(new vip.newsclaw.llm.failover.FallbackEntry("__legacy__", m));
    }
     ...
     * {@link vip.newsclaw.llm.failover.AvailableProviderPool.RemovalSource} for
     ...
     * {@link vip.newsclaw.llm.failover.ProviderHealthTracker}'s cooldown instead.</p>
     */
    private static vip.newsclaw.llm.failover.AvailableProviderPool.RemovalSource hardRemovalSource(ErrorType type) {
        ...
        return switch (type) {
            case AUTH_ERROR -> vip.newsclaw.llm.failover.AvailableProviderPool.RemovalSource.AUTH_ERROR;
            case BILLING -> vip.newsclaw.llm.failover.AvailableProviderPool.RemovalSource.BILLING;
            default -> null;
        };
    }
    ...
    private StreamResult streamCallInternal(ChatModel chatModel, Prompt prompt,
        ...
        if (primarySkipped) {
            ...
            traceRouting("PRIMARY", primaryProviderId, primaryModelName, conversationId, phase,
                    0, 0, "SKIPPED", primaryOutOfPool ? "POOL_REMOVED" : "COOLDOWN", 0);
            if (broadcast) {
        ...
        for (int i = 0; i < fallbackChain.size(); i++) {
            vip.newsclaw.llm.failover.FallbackEntry entry = fallbackChain.get(i);
            ...
            traceRouting("FALLBACK", entry.providerId(), entry.modelName(), conversationId, phase,
                    ...
                    System.currentTimeMillis() - fallbackStartMs);
    ...
    private void traceRouting(String role, String providerId, String modelName,
                              String conversationId, String phase, int attemptNo,
                              int fallbackOrdinal, String outcome, String failureCategory,
                              long durationMs) {
        if (routingTraceService == null) {
            return;
        }
        routingTraceService.record(new vip.newsclaw.llm.trace.LlmRoutingTraceService.RoutingTrace(
                null, routingAgentId, conversationId, phase, role, providerId, modelName,
                attemptNo, fallbackOrdinal, outcome, failureCategory, Math.max(0, durationMs), null));
    }

    private static String routingOutcome(StreamResult result) {
        if (result == null) {
            return "RETRY";
        }
        return result.errorMessage() == null || result.errorType() == ErrorType.NONE
                ? "SUCCEEDED" : "FAILED";
    }

    private static String routingFailure(StreamResult result, ErrorType retryType) {
        if (result != null && result.errorType() != null && result.errorType() != ErrorType.NONE) {
            return result.errorType().name();
        }
        return retryType == null || retryType == ErrorType.NONE ? null : retryType.name();
    }
        ...
        Disposable subscription = streamWithIdleGuard
                .doOnNext(chatResponse -> {
                    ...
                    if (chatResponse.getMetadata() != null && chatResponse.getMetadata().getUsage() != null) {
                        ...
                        var cache = vip.newsclaw.llm.cache.CacheUsageExtractor.extract(usage);

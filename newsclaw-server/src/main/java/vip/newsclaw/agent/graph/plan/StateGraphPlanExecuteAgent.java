package vip.newsclaw.agent.graph.plan;
...
import vip.newsclaw.agent.BaseAgent;
import vip.newsclaw.agent.delegation.DelegatedUsageAccumulator;
import vip.newsclaw.agent.GraphEventPublisher;
import vip.newsclaw.agent.StructuredStreamCapable;
...
public class StateGraphPlanExecuteAgent extends BaseAgent implements StructuredStreamCapable {
    ...
    private final vip.newsclaw.agent.AgentToolSet toolSet;
    ...
    public StateGraphPlanExecuteAgent(ChatClient chatClient, ConversationService conversationService,
                                      ...
                                      org.springframework.ai.chat.model.ChatModel chatModel,
                                      ConversationWindowManager conversationWindowManager,
                                      vip.newsclaw.agent.AgentToolSet toolSet) {
    ...
    public Flux<AgentService.StreamDelta> chatStructuredStream(String userMessage, String conversationId,
        ...
        try {
            ...
            inputs.put(NewsClawStateKeys.REQUESTER_ID, requesterId != null ? requesterId : "");
        ...
        try {
            ...
            if (ctx != null) {
                ...
                if (!ctx.completedResults().isEmpty()) {
                    ...
                    List<Message> messages = (List<Message>) inputs.get(NewsClawStateKeys.MESSAGES);
            ...
            if (toolCallPayload != null && !toolCallPayload.isEmpty()) {
                inputs.put(NewsClawStateKeys.PRE_APPROVED_TOOL_CALL, toolCallPayload);
            }
    ...
    private Flux<AgentService.StreamDelta> executeStream(Map<String, Object> inputs) {
        ...
        final String usageConversationId = (String) inputs.get(NewsClawStateKeys.CONVERSATION_ID);
        ...
        return BaseAgent.routingStartupDelta(inputs).concatWith(compiledGraph.stream(inputs, config)
                .flatMapIterable(output -> {
                    ...
                    boolean contentAlreadyStreamed = output.state()
                            .value(NewsClawStateKeys.CONTENT_STREAMED, false);
                    boolean thinkingAlreadyStreamed = output.state()
                            .value(NewsClawStateKeys.THINKING_STREAMED, false);
                    ...
                    finalPromptTokens.set(output.state().value(NewsClawStateKeys.PROMPT_TOKENS, 0));
                    finalCompletionTokens.set(output.state().value(NewsClawStateKeys.COMPLETION_TOKENS, 0));
                    finalCacheReadTokens.set(output.state().value(NewsClawStateKeys.CACHE_READ_TOKENS, 0));
                    finalCacheWriteTokens.set(output.state().value(NewsClawStateKeys.CACHE_WRITE_TOKENS, 0));
                    finalReasoningTokens.set(output.state().value(NewsClawStateKeys.REASONING_TOKENS, 0));
                    finalModelName.set(output.state().value(NewsClawStateKeys.RUNTIME_MODEL_NAME, ""));
                    finalProviderId.set(output.state().value(NewsClawStateKeys.RUNTIME_PROVIDER_ID, ""));
    ...
    private Map<String, Object> buildInitialState(String userMessage, String conversationId) {
        ...
        inputs.put(NewsClawStateKeys.SYSTEM_PROMPT,
                systemPrompt != null ? systemPrompt : "你是一个有帮助的AI助手。");
        inputs.put(NewsClawStateKeys.CONVERSATION_ID, conversationId);
        inputs.put(NewsClawStateKeys.AGENT_ID, agentId != null ? agentId : "");
        inputs.put(NewsClawStateKeys.WORKSPACE_BASE_PATH, workspaceBasePath != null ? workspaceBasePath : "");
        // 注入会话消息（复用 NewsClawStateKeys.MESSAGES，与 ReAct 一致）
        inputs.put(NewsClawStateKeys.MESSAGES, messages);
        // 注入 working context
        inputs.put(PlanStateKeys.WORKING_CONTEXT, workingContext);
        inputs.put(PlanStateKeys.CURRENT_STEP_INDEX, 0);
        inputs.put(NewsClawStateKeys.CONTENT_STREAMED, false);
        inputs.put(NewsClawStateKeys.THINKING_STREAMED, false);
        inputs.put(NewsClawStateKeys.STREAMED_CONTENT, "");
        inputs.put(NewsClawStateKeys.STREAMED_THINKING, "");
        inputs.put(NewsClawStateKeys.REQUESTER_ID, "");
        inputs.put(NewsClawStateKeys.PROMPT_TOKENS, 0);
        inputs.put(NewsClawStateKeys.COMPLETION_TOKENS, 0);
        inputs.put(NewsClawStateKeys.CACHE_READ_TOKENS, 0);
        inputs.put(NewsClawStateKeys.CACHE_WRITE_TOKENS, 0);
        inputs.put(NewsClawStateKeys.REASONING_TOKENS, 0);
        inputs.put(NewsClawStateKeys.RUNTIME_MODEL_NAME, modelName != null ? modelName : "");
        inputs.put(NewsClawStateKeys.RUNTIME_PROVIDER_ID, runtimeProviderId != null ? runtimeProviderId : "");
        inputs.put(NewsClawStateKeys.TRACE_ID, UUID.randomUUID().toString().substring(0, 8));

        if (currentTurn.routingDecision() != null
                && (currentTurn.routingDecision().strategy() != vip.newsclaw.llm.routing.model.MultimodalRoutingDecision.Strategy.NONE
                        || !currentTurn.routingDecision().skipped().isEmpty())) {
            inputs.put(NewsClawStateKeys.ROUTING_DECISION, currentTurn.routingDecision().toMap());
        }
        ...
        vip.newsclaw.agent.context.ChatOrigin origin = vip.newsclaw.agent.context.ChatOriginHolder.get();
        ...
        inputs.put(NewsClawStateKeys.CHAT_ORIGIN, origin);
        ...
        if (goalService != null && conversationId != null && !conversationId.isBlank()) {
            try {
                vip.newsclaw.goal.model.GoalEntity active =
                        goalService.findActiveByConversation(conversationId);
                if (active != null) {
                    inputs.put(NewsClawStateKeys.ACTIVE_GOAL, active);
                }
            } catch (Exception e) {
                log.warn("[{}] findActiveByConversation failed: {}", agentName, e.getMessage());
            }
        }
        inputs.put(NewsClawStateKeys.GOAL_EVALUATED_THIS_RUN, false);
        inputs.put(NewsClawStateKeys.GOAL_FOLLOWUP_INJECTED, false);
        inputs.put(NewsClawStateKeys.GOAL_FOLLOWUP_PROMPT, "");

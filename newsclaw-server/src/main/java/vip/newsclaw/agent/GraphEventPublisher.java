...
public final class GraphEventPublisher {
    ...
    /**
     * Terminal {@link vip.newsclaw.agent.graph.state.FinishReason} for the turn,
     ...
     *
     * @param reason {@link vip.newsclaw.agent.graph.state.FinishReason#getValue()}
    ...
    public static List<GraphEvent> extractEvents(NodeOutput output) {
        ...
        return output.state().<List<GraphEvent>>value(NewsClawStateKeys.PENDING_EVENTS)
                .orElse(List.of());
    }

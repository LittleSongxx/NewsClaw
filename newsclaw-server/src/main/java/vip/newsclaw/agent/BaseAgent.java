...
import vip.newsclaw.approval.ApprovalPlaceholderUtil;
import vip.newsclaw.llm.model.ModelConfigEntity;
import vip.newsclaw.llm.routing.MediaCaptionService;
import vip.newsclaw.llm.routing.MultimodalRouter;
import vip.newsclaw.llm.routing.model.MultimodalRoutingDecision;
...
public abstract class BaseAgent {
     ...
     * by {@link vip.newsclaw.llm.service.ModelCapabilityService}). Empty set = unknown model,
     ...
     */
    protected vip.newsclaw.agent.AgentToolSet toolSet;
     ...
     */
    protected vip.newsclaw.goal.service.GoalService goalService;
    ...
    private Message toSpringMessage(MessageEntity message) {
        ...
        // the model gets replaced. This is NewsClaw's persistence-aware analog
     ...
     * {@link vip.newsclaw.agent.AgentService.StreamDelta} the SSE accumulator can
     * persist. Returns an empty Flux when no routing happened this turn so we
     * don't emit zero-value events.
     */
    @SuppressWarnings("unchecked")
    public static reactor.core.publisher.Flux<vip.newsclaw.agent.AgentService.StreamDelta> routingStartupDelta(
            java.util.Map<String, Object> inputs) {
        Object decision = inputs.get(vip.newsclaw.agent.graph.state.NewsClawStateKeys.ROUTING_DECISION);
        if (decision instanceof java.util.Map<?, ?> map && !map.isEmpty()) {
            return reactor.core.publisher.Flux.just(vip.newsclaw.agent.AgentService.StreamDelta.event(
                    vip.newsclaw.agent.GraphEventPublisher.EVENT_ROUTING_DECISION,
     ...
     * directory ({@code newsclaw.chat.upload.base-dir}, default

...
import vip.newsclaw.plugin.model.PluginInfo;
import vip.newsclaw.plugin.repository.PluginMapper;
...
public class PluginManager {
    ...
    public void loadPlugin(Path jarPath) throws Exception {
        ...
        try {
            ...
            if (!NewsClawPlugin.class.isAssignableFrom(entryClass)) {
                throw new PluginException("Entrypoint class " + manifest.getEntrypoint() +
                        " does not implement NewsClawPlugin");
            }
            NewsClawPlugin plugin = (NewsClawPlugin) entryClass.getDeclaredConstructor().newInstance();
    ...
    private PluginManifest readManifest(Path jarPath) throws IOException {
        try (JarFile jarFile = new JarFile(jarPath.toFile())) {
            ZipEntry entry = jarFile.getEntry("newsclaw-plugin.json");
            if (entry == null) {
                throw new PluginException("No newsclaw-plugin.json found in " + jarPath);
            }

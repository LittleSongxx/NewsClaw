...
import vip.newsclaw.workspace.document.MemorySearchHit;
...
public class WorkspaceMemoryTool {
    ...
    public String read_workspace_memory_file(
        ...
        memoryRecallTracker.trackActiveRetrieval(
                parsedAgentId, filename, content, file.getOwnerKey(), file.getScope());
    ...
    @vip.newsclaw.tool.ConcurrencyUnsafe("workspace memory write — concurrent writes to the same file would clobber each other")
    @Tool(description = """
    ...
    @vip.newsclaw.tool.ConcurrencyUnsafe("workspace memory edit — find/replace must serialize per file")
    @Tool(description = """
        ...
        for (MemorySearchHit hit : hits) {
            if (retrieved.add(hit.filename())) {
                ...
                if (file != null && file.getContent() != null) {
                    memoryRecallTracker.trackActiveRetrieval(
                            parsedAgentId, hit.filename(), file.getContent(), file.getOwnerKey(), file.getScope());
                }
            }
        }

...
public class GzhPackageTool {
    ...
    public String gzh_package(String title, String markdown, String coverImageUrl,
                              String author, String topic, @Nullable ToolContext ctx) {
        return gzh_package(title, markdown, coverImageUrl, author, topic, null, ctx);
    }
    ...
    public String gzh_package(
            ...
            String topic,
            @ToolParam(description = "AI 动态事件 id；传入后自动把公众号内容条目回链到该事件", required = false)
            String eventId,
        ...
        // Compliance is a pre-delivery gate. Producing a downloadable package
        // and a calendar row first makes an unsafe artifact look deliverable,
        // even when a later review task correctly refuses to approve it.
        ComplianceScanner.Result scan = ComplianceScanner.scan(title + "\n" + markdown);
        if (scan.hasHighRisk()) {
            return "⛔ 公众号合规门禁阻断：尚未生成预览、素材包或内容日历记录。\n"
                    + ComplianceScanner.report(scan)
                    + "\n请改写命中词后重新调用 gzh_package。";
        }
        ...
        // Markdown links make generated files discoverable by the Team Run
    ...
    private void linkNewsEvent(String eventId, Long itemId, @Nullable ToolContext ctx, StringBuilder out) {
        ...
        try {
            ...
            aiNewsEventService.linkContent(workspaceFromContext(ctx), id, itemId, "gzh");
            out.append("\n🔗 已回链 AI 动态事件（event id: ").append(id).append("）。");
        } catch (Exception e) {
            out.append("\n⚠️ 内容已入日历，但事件回链失败：").append(e.getMessage());
            log.warn("[GzhPackage] AI news event link failed: {}", e.getMessage());
        }
    }

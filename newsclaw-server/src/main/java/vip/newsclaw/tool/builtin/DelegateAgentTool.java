...
import vip.newsclaw.agent.AgentService.ChatResult;
...
import vip.newsclaw.task.AsyncTaskService;
...
public class DelegateAgentTool {
     ...
     *
     * <p>Configurable via {@code newsclaw.delegation.parallel-timeout-seconds};
     ...
     */
    @Value("${newsclaw.delegation.parallel-timeout-seconds:300}")
     ...
     * {@code newsclaw.delegation.child-denied-tools}.
     */
     ...
     * {@code newsclaw.delegation.child-denied-tools} as a comma-separated
     ...
     */
    @Value("${newsclaw.delegation.child-denied-tools:}")
    ...
    @vip.newsclaw.tool.ConcurrencyUnsafe("spawns a child agent session and writes to mate_conversation; serialize to keep session graph deterministic")
    @Tool(description = """
    ...
    @vip.newsclaw.tool.ConcurrencyUnsafe("internally fans out to its own thread pool; outer executor must not double-parallelize")
    @Tool(description = """
    ...
    private String buildInheritedContextPrefix(String parentConversationId) {
        try {
            List<vip.newsclaw.workspace.conversation.model.MessageEntity> messages =
    ...
    /**
     * RFC-03 Lane C2 — format a list of parent {@link vip.newsclaw.workspace.conversation.model.MessageEntity}
    ...
    static String formatInheritedContext(
        ...
        List<vip.newsclaw.workspace.conversation.model.MessageEntity> dialogue = messages.stream()
        ...
        sb.append("--- Parent conversation recent context (")
          ...
          .append(") ---\n");
        for (vip.newsclaw.workspace.conversation.model.MessageEntity m : dialogue) {

package vip.newsclaw.tool.service;
...
public class ToolService {
    ...
    public ToolEntity getTool(Long id) {
        ...
        if (tool == null) {
            throw new NewsClawException("err.tool.not_found", "工具不存在: " + id);
        }
    ...
    public void deleteTool(Long id) {
        ...
        if (Boolean.TRUE.equals(tool.getBuiltin())) {
            throw new NewsClawException("err.tool.builtin_readonly", "内置工具不可删除");
        }
    ...
    public ToolEntity setDisclosureTier(Long id, String tier) {
        ToolEntity tool = getTool(id);
        tool.setDisclosureTier(vip.newsclaw.tool.disclosure.DisclosureTier.fromToken(tier).token());
        toolMapper.updateById(tool);
        return enrichRuntimeNames(tool);
    }
    ...
    private List<ToolEntity> enrichRuntimeNames(List<ToolEntity> tools) {
        ...
        vip.newsclaw.agent.AgentToolSet set;
        try {
    ...
    private static void enrichRuntimeNames(ToolEntity tool, vip.newsclaw.agent.AgentToolSet set) {

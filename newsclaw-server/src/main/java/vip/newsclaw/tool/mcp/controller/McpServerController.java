package vip.newsclaw.tool.mcp.controller;
...
import vip.newsclaw.tool.mcp.model.McpToolDescriptor;
import vip.newsclaw.tool.mcp.runtime.McpClientManager.ConnectionResult;
import vip.newsclaw.tool.mcp.service.McpServerService;
 ...
 * 属于系统管理级能力，因此每个端点都要求数据库中的全局 admin 角色；
 * workspace admin 不能注册进程或修改所有租户共享的 MCP runtime。
 *
...
public class McpServerController {
    ...
    public R<McpServerEntity> setDisclosureTier(@PathVariable Long id,
        ...
        if (!vip.newsclaw.tool.disclosure.DisclosureTier.isValidToken(tier)) {
     ...
     *       {@code NewsClawException("err.mcp.not_found")} which the global handler

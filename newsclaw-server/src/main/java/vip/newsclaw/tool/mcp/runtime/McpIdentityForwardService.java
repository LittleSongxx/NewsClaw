package vip.newsclaw.tool.mcp.runtime;
 ...
 *
 * <p><b>Identity typing.</b> Not every requester is a NewsClaw-authenticated
 * account. The resolved identity is typed so the REST backend can tell
 * "NewsClaw authenticated this user" apart from "this is an external/anonymous
 ...
 *       NewsClaw account backs it; {@code sub} = the visitor id. The backend must
...
public class McpIdentityForwardService {
    ...
    ResolvedIdentity classify(ToolContext ctx) {
        ...
        // "web" channel (built by ChatOrigin.web()) carries NewsClaw's assertion
        ...
        // Authenticated web account: NewsClaw vouches for this user. Prefer the
        ...
        // no NewsClaw account — forward with an explicit trust downgrade so the
        // backend knows it is NOT an authenticated NewsClaw user.
    ...
    private PrivateKey signingKey() {
        ...
        synchronized (this) {
            ...
            if (pem == null || pem.isBlank()) {
                log.error("[McpIdentity] token mode enabled but newsclaw.mcp.identity-forward.token.private-key-pem is empty; "

...
import vip.newsclaw.llm.cache.AnthropicCacheOptionsFactory;
...
public class AnthropicChatModelBuilder implements ChatModelBuilder {
    ...
    AnthropicApi buildAnthropicApi(ModelProviderEntity provider, Integer readTimeoutOverride) {
        if (provider == null || !modelProviderService.isProviderConfigured(provider.getProviderId())) {
            throw new NewsClawException("err.agent.anthropic_not_configured",
                    "Anthropic Provider 未完成配置，请在模型设置中填写有效的 API Key 和 Base URL");
        }
        String apiKey = provider.getApiKey();
        if (!modelProviderService.hasUsableApiKey(apiKey)) {
            throw new NewsClawException("err.agent.anthropic_key_invalid",
    ...
    /**
     * Overload — accepts a per-model read-timeout override
     * (seconds). Null / zero / negative falls back to {@link vip.newsclaw.llm.chatmodel.HttpTimeouts#DEFAULT_READ_TIMEOUT}
    ...
    static RestClient.Builder applyHttpTimeouts(RestClient.Builder builder, Integer readTimeoutOverride) {
        HttpClient httpClient = HttpClient.newBuilder()
                .connectTimeout(vip.newsclaw.llm.chatmodel.HttpTimeouts.CONNECT_TIMEOUT)
                .build();
        JdkClientHttpRequestFactory rf = new JdkClientHttpRequestFactory(httpClient);
        rf.setReadTimeout(vip.newsclaw.llm.chatmodel.HttpTimeouts.resolveReadTimeout(readTimeoutOverride));
     ...
     * {@link vip.newsclaw.llm.chatmodel.HttpTimeouts#DEFAULT_STREAM_IDLE_TIMEOUT}).
    ...
    static WebClient.Builder applyHttpTimeoutsToWebClient(WebClient.Builder builder, Integer readTimeoutOverride) {
        ...
        connector.setReadTimeout(vip.newsclaw.llm.chatmodel.HttpTimeouts.resolveReadTimeout(readTimeoutOverride));

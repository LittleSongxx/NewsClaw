 ...
 *   <li><b>Brand scrub</b>: replace {@code "NewsClaw"}/{@code "newsclaw"}
 ...
 *       names are stripped of the {@code mcp_} prefix so NewsClaw's tool
...
public class ClaudeCodeIdentityChatModelDecorator implements ChatModel {
    ...
    Prompt transform(Prompt original) {
        ...
        for (Message msg : source) {
            if (msg instanceof SystemMessage sm && !systemSeen) {
                ...
                // by the time NewsClaw stores the AssistantMessage the names
    ...
    private ChatOptions transformOptions(ChatOptions options) {
        ...
        if (hasToolNames) {
            ...
            // a wider registry. If NewsClaw populates it (most paths use callbacks
     ...
     * NewsClaw's tool implementation never sees the prefix.
     */

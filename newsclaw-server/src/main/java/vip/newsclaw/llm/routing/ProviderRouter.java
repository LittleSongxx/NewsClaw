package vip.newsclaw.llm.routing;
...
import vip.newsclaw.llm.service.ModelCapabilityService.Modality;
...
public class ProviderRouter {
    ...
    public ModelConfigEntity selectPrimary(Long agentId, ModelConfigEntity globalDefault) {
        ...
        // Environment selection is the deployment-level policy and therefore
        // leads workspace-specific preferences. This keeps the primary model
        // and the failover chain consistent with the Docker .env contract.
        List<ProviderModelRef> preferred = new ArrayList<>(environmentPreferences());
        preferred.addAll(bindingService.getPreferredProviderModels(agentId));

package vip.newsclaw.llm.service;
...
import vip.newsclaw.llm.anthropic.oauth.ClaudeCodeOAuthService;
import vip.newsclaw.llm.event.ModelConfigChangedEvent;
import vip.newsclaw.llm.failover.AvailableProviderPool;
import vip.newsclaw.llm.failover.ProviderHealthTracker;
import vip.newsclaw.llm.failover.ProviderInitProbe;
import vip.newsclaw.llm.failover.ProviderRequirements;
import vip.newsclaw.llm.model.*;
import vip.newsclaw.llm.probe.ModelContextWindowResolver;
import vip.newsclaw.llm.repository.ModelProviderMapper;
import vip.newsclaw.config.ConversationWindowProperties;
...
public class ModelProviderService {
     ...
     * {@code newsclaw-ui/src/views/Settings/Models/composables/useProviderForm.ts}
     * and the routing fallback in {@code newsclaw-ui/src/api/index.ts}.</p>
     */
    ...
    private List<ProviderInfoDTO> listProvidersInternal(boolean enabledOnly) {
        ...
        // Do not apply the enabled predicate in SQL.  A provider can be
        // enabled by a deployment-owned .env credential even when its
        // database row deliberately remains enabled=false (environment
        // configuration is runtime-only and must not mutate the row).  The
        // effectiveProvider(...) projection below applies that override, and
        // the post-projection filter keeps ordinary disabled catalog entries
        // out of the enabled-only view.
        ...
        List<ModelProviderEntity> providers = modelProviderMapper.selectList(qw).stream()
                .map(this::effectiveProvider)
                // NULL is retained for legacy rows created before the explicit
                // enabled flag was introduced; the old SQL path treated such
                // rows as visible in detached/test projections as well.
                .filter(provider -> !enabledOnly
                        || provider.getEnabled() == null
    ...
    public ProviderInfoDTO updateProviderConfig(String providerId, ProviderConfigRequest request) {
        // Mutating paths must use the raw row.  Environment overrides are
        // runtime-only and must never be accidentally persisted by a UI save.
        ModelProviderEntity provider = rawProvider(providerId);
        ...
        return toProviderInfo(effectiveProvider(provider), modelConfigService.listModelsByProvider(providerId));
    }
    ...
    public ProviderInfoDTO createCustomProvider(CreateCustomProviderRequest request) {
        if (!StringUtils.hasText(request.getId()) || !StringUtils.hasText(request.getName())) {
            throw new NewsClawException("err.llm.provider_fields_required", "Provider id 和名称不能为空");
        }
        ...
        if (!PROVIDER_ID_PATTERN.matcher(request.getId()).matches()) {
            throw new NewsClawException("err.llm.provider_id_invalid",
        ...
        if (existing != null) {
            throw new NewsClawException("err.llm.provider_exists", "Provider 已存在: " + request.getId());
        }
    ...
    public void deleteCustomProvider(String providerId) {
        ...
        if (!Boolean.TRUE.equals(provider.getIsCustom())) {
            throw new NewsClawException("err.llm.provider_builtin_readonly", "内置 Provider 不支持删除");
        }
    ...
    public List<ModelProviderEntity> listFallbackChain() {
        ...
        return modelProviderMapper.selectList(qw).stream()
                .map(this::effectiveProvider)
                .toList();
    }
    ...
    private EnableResult pickReplacementDefaultIfNeeded(String disabledProviderId) {
        ...
        List<ModelProviderEntity> candidates = modelProviderMapper.selectList(
                ...
                .ne(ModelProviderEntity::getProviderId, disabledProviderId)
                        ...
                        .orderByAsc(ModelProviderEntity::getName)).stream()
                .map(this::effectiveProvider)
    ...
    private void tryAutoActivateModel(String providerId, ModelProviderEntity provider) {
        ...
        try {
            ...
            ModelProviderEntity defaultProvider = effectiveProvider(
                    modelProviderMapper.selectById(currentDefault.getProvider()));
    ...
    private ModelProviderEntity getProvider(String providerId) {
        ModelProviderEntity provider = rawProvider(providerId);
        return effectiveProvider(provider);
    }

    private ModelProviderEntity rawProvider(String providerId) {
        ModelProviderEntity provider = modelProviderMapper.selectById(providerId);
        if (provider == null) {
            throw new NewsClawException("err.llm.provider_not_found", "Provider 不存在: " + providerId);
        }
        return provider;
    }

    /** Apply .env credentials without changing the detached DB entity. */
    private ModelProviderEntity effectiveProvider(ModelProviderEntity source) {
        if (source == null) {
            return null;
        }
        ModelProviderEntity effective = new ModelProviderEntity();
        BeanUtils.copyProperties(source, effective);
        String apiKey = EnvironmentConfig.providerApiKey(source.getProviderId());
        if (apiKey != null) {
            effective.setApiKey(apiKey);
            // A non-blank env credential is an explicit opt-in for a built-in
            // provider.  Keep the database flag untouched, but expose the
            // effective row as enabled so a fresh Docker install can use the
            // provider without a second UI toggle.
            effective.setEnabled(true);
        }
        String baseUrl = EnvironmentConfig.providerBaseUrl(source.getProviderId());
        if (baseUrl != null) {
            effective.setBaseUrl(baseUrl);
        }
        return effective;
    }
    ...
    private boolean isProviderConfigured(ModelProviderEntity provider) {
        ...
        // Callers that read directly from MyBatis (startup probes, fallback
        // selection, and a few legacy services) still receive the effective
        // .env-backed view here.

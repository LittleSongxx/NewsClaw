...
import vip.newsclaw.llm.repository.ModelConfigMapper;
...
public class ModelConfigService {
    ...
    public ModelConfigEntity getModel(Long id) {
        ...
        if (entity == null) {
            throw new NewsClawException("err.llm.model_config_not_found", "模型配置不存在: " + id);
        }
        return entity;
    }

    public ModelConfigEntity getDefaultModel() {
        // A Docker deployment may own model selection in .env. Resolve that
        // chain before the database's is_default flag, while still requiring a
        // usable credential so a missing primary key can fall through to the
        // next configured entry or the legacy database default.
        ModelConfigEntity environmentModel = findEnvironmentDefaultModel();
        ...
        if (!candidates.isEmpty()) {
            ...
            throw new NewsClawException("err.llm.no_configured_provider",
                    "所有已启用的模型 Provider 均未完成配置（默认 Provider: " + unconfiguredProvider
                    + "），请在「设置 → 模型」中填写 API Key");
        }
        throw new NewsClawException("err.llm.no_available_model", "没有可用的模型配置");
    }

    /**
     * Find the first enabled model from the environment-owned chain. Provider
     * liveness is deliberately not checked here: a temporarily unavailable
     * primary must still be selected so the runtime failover walker can record
     * the failure and try the configured backup model.
     */
    private ModelConfigEntity findEnvironmentDefaultModel() {
        for (EnvironmentConfig.ModelSelection selection : EnvironmentConfig.configuredModelChain()) {
            ModelConfigEntity candidate = findEnabledModel(
                    selection.providerId(), selection.modelName());
            if (candidate == null) continue;
            try {
                if (modelProviderService != null
                        && !modelProviderService.isProviderConfigured(selection.providerId())) {
                    continue;
                }
            } catch (Exception ignored) {
                // Preserve the database fallback if provider metadata is still
                // warming up during application bootstrap.
                continue;
            }
            return candidate;
        }
    ...
    /**
     * Idempotently materialize a model named by .env. The normal admin/UI path
     * remains unchanged; this helper only makes environment-only Docker
     * deployments restart-safe when a provider catalog does not contain a
     * newly released model yet.
     */
    public ModelConfigEntity ensureEnvironmentModel(String providerId, String modelName) {
        ...
        ModelConfigEntity existing = modelConfigMapper.selectOne(new LambdaQueryWrapper<ModelConfigEntity>()
                .eq(ModelConfigEntity::getProvider, providerId.trim())
                .eq(ModelConfigEntity::getModelName, modelName.trim())
        ...
        if (existing != null) {
            if (!Boolean.TRUE.equals(existing.getEnabled())) {
                ...
                publishConfigChanged("environment-model-enabled");
            }
            return existing;
        }
        return addModelToProvider(providerId.trim(), modelName.trim(), modelName.trim(), false);
    }
    ...
    public ModelConfigEntity updateModel(ModelConfigEntity entity) {
        ...
        if (existing.getIsDefault() && Boolean.FALSE.equals(entity.getEnabled())) {
            throw new NewsClawException("err.llm.cannot_disable_default", "默认模型不能被禁用，请先切换默认模型");
        }
    ...
    public void deleteModel(Long id) {
        ...
        if (Boolean.TRUE.equals(entity.getIsDefault())) {
            throw new NewsClawException("err.llm.cannot_delete_default", "默认模型不能删除，请先切换默认模型");
        }
    ...
    public ModelConfigEntity addModelToProvider(String providerId, String modelId, String displayName, boolean builtin) {
        if (!StringUtils.hasText(providerId) || !StringUtils.hasText(modelId)) {
            throw new NewsClawException("err.llm.provider_model_required", "Provider 和模型标识不能为空");
        }
        ModelConfigEntity existing = modelConfigMapper.selectOne(new LambdaQueryWrapper<ModelConfigEntity>()
                .eq(ModelConfigEntity::getProvider, providerId)
                ...
                .last("LIMIT 1"));
        if (existing != null) {
            throw new NewsClawException("err.llm.model_exists", "模型已存在: " + modelId);
        }
     ...
     *
     * @throws NewsClawException when the model is unknown or the value is
    ...
    public ModelConfigEntity updateModelContextWindow(String providerId, String modelId, Integer maxInputTokens) {
        ModelConfigEntity entity = modelConfigMapper.selectOne(new LambdaQueryWrapper<ModelConfigEntity>()
                ...
                .last("LIMIT 1"));
        if (entity == null) {
            throw new NewsClawException("err.llm.model_not_found", "模型不存在: " + modelId);
        }
        ...
        if (value > 0 && (value < MIN_CONTEXT_WINDOW || value > MAX_CONTEXT_WINDOW)) {
            throw new NewsClawException("err.llm.context_window_out_of_range",
    ...
    public void removeModelFromProvider(String providerId, String modelId) {
        ...
        if (Boolean.TRUE.equals(entity.getBuiltin())) {
            throw new NewsClawException("err.llm.builtin_readonly", "内置模型不支持删除");
        }
    ...
    public ModelConfigEntity setDefaultModel(Long id) {
        ...
        if (!Boolean.TRUE.equals(entity.getEnabled())) {
            throw new NewsClawException("err.llm.only_enabled_default", "只有启用状态的模型才能设为默认");
        }
        clearDefaultFlag();
    ...
    public ModelConfigEntity setDefaultModel(String providerId, String modelName) {
        ModelConfigEntity entity = modelConfigMapper.selectOne(new LambdaQueryWrapper<ModelConfigEntity>()
                ...
                .last("LIMIT 1"));
        if (entity == null) {
            throw new NewsClawException("err.llm.model_not_found", "模型不存在: " + providerId + "/" + modelName);
        }
        if (!Boolean.TRUE.equals(entity.getEnabled())) {
    ...
    private void validateModel(ModelConfigEntity entity, Long currentId) {
        if (!StringUtils.hasText(entity.getName())) {
            throw new NewsClawException("err.llm.name_required", "模型名称不能为空");
        }
        if (!StringUtils.hasText(entity.getProvider())) {
            entity.setProvider("dashscope");
        }
        if (!StringUtils.hasText(entity.getModelName())) {
            throw new NewsClawException("err.llm.id_required", "模型标识不能为空");
        }
        ModelConfigEntity duplicate = modelConfigMapper.selectOne(new LambdaQueryWrapper<ModelConfigEntity>()
                ...
                .last("LIMIT 1"));
        if (duplicate != null) {
            throw new NewsClawException("err.llm.id_exists", "模型标识已存在: " + entity.getProvider() + "/" + entity.getModelName());
        }
    }

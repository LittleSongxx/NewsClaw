 ...
 */
@Data
@ConfigurationProperties(prefix = "newsclaw.memory")
public class MemoryProperties {
    ...
    /**
     * Require every durable memory writer to pass source, size, conflict and
     * token-budget admission before it mutates an always-on file.
     */
    private boolean governanceEnabled = true;

    /** Maximum estimated tokens admitted for one durable memory candidate. */
    private int writeMaxTokens = 320;

    /** Per workspace/agent/owner token budget for active long-term memories. 0 = unlimited. */
    private int longTermTokenBudget = 2400;

    /** Reject article-like AI-news evidence; event facts belong in the event/Wiki domain. */
    private boolean rejectNewsBody = true;

package vip.newsclaw.memory.nudge;
...
import vip.newsclaw.memory.service.StructuredMemoryService;
...
public class MemoryNudgeService {
    ...
    public void maybeNudge(Long agentId, String conversationId, int messageCount, String ownerKey) {
        ...
        try {
            ...
            if (conversation == null || conversation.getWorkspaceId() == null || conversation.getWorkspaceId() <= 0
                ...
                log.warn("[Nudge] Refusing unscoped or mismatched auto-memory write: agent={}, conv={}",
                        agentId, conversationId);
                return;
            }
            doNudge(agentId, conversationId, ownerKey, conversation.getWorkspaceId());
    ...
    private void doNudge(Long agentId, String conversationId, String ownerKey, Long workspaceId) {
        ...
        try {
            ...
            for (JsonNode entry : root) {
                ...
                MemoryWriteDecision decision = admit(workspaceId, agentId, ownerKey, type, key, content, conversationId);
                if (!decision.admitted()) {
                    log.info("[Nudge] Rejected entry '{}/{}': {}", type, key, decision.reason());
                    continue;
                }
                try {
    ...
    private MemoryWriteDecision admit(Long workspaceId, Long agentId, String ownerKey,
                                      String type, String key, String content, String conversationId) {
        ...
        return governanceService.admit(new MemoryWriteRequest(workspaceId, agentId, ownerKey,
                type, key, content, "nudge", conversationId, "conversation:" + conversationId));
    }

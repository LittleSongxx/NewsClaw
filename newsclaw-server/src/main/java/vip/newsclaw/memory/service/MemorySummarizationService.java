...
import vip.newsclaw.memory.governance.MemoryWriteDecision;
import vip.newsclaw.memory.governance.MemoryWriteGovernanceService;
import vip.newsclaw.memory.governance.MemoryWriteRequest;
...
public class MemorySummarizationService {
    ...
    /** Enforces durable-memory admission in production without widening legacy constructors. */
    private MemoryWriteGovernanceService governanceService;

    @org.springframework.beans.factory.annotation.Autowired(required = false)
    public void setGovernanceService(MemoryWriteGovernanceService governanceService) {
    ...
    private void doAnalyzeAndUpdate(Long agentId, String conversationId, String ownerKey) {
        ...
        if (conversation == null || conversation.getWorkspaceId() == null || conversation.getWorkspaceId() <= 0
                || conversation.getAgentId() == null || !conversation.getAgentId().equals(agentId)) {
            log.warn("[Memory] Refusing unscoped or mismatched auto-memory write: agent={}, conv={}",
        ...
        try {
            ...
            // 6. 应用更新
            applyUpdates(agentId, root, dailyFilename, dailyContent, ownerKey, workspaceId, conversationId);
    ...
    private void applyUpdates(Long agentId, JsonNode root, String dailyFilename,
        ...
        applyUpdates(agentId, root, dailyFilename, existingDailyContent, ownerKey, 1L, null);
    }

    private void applyUpdates(Long agentId, JsonNode root, String dailyFilename,
                              String existingDailyContent, String ownerKey,
                              Long workspaceId, String conversationId) {
        // Daily entry: 追加模式
        ...
        if (!dailyNode.isNull() && dailyNode.isTextual()) {
            ...
            if (!entry.isEmpty()) {
                ...
                if (saveMemoryIfAdmitted(agentId, workspaceId, ownerKey, "daily",
                        "daily_" + LocalDate.now(), dailyFilename, newContent, conversationId)) {
        ...
        if (!memoryNode.isNull() && memoryNode.isTextual()) {
            ...
            if (!content.isEmpty()) {
                if (saveMemoryIfAdmitted(agentId, workspaceId, ownerKey, "memory_document",
                        "memory_md", "MEMORY.md", content, conversationId)) {
        ...
        if (!profileNode.isNull() && profileNode.isTextual()) {
            ...
            if (!content.isEmpty()) {
                if (saveMemoryIfAdmitted(agentId, workspaceId, ownerKey, "profile",
                        "profile_md", "PROFILE.md", content, conversationId)) {
        ...
        applyStructuredEntries(agentId, root.path("structured_entries"), ownerKey, workspaceId, conversationId);
    }

    private void applyStructuredEntries(Long agentId, JsonNode entriesNode, String ownerKey) {
        applyStructuredEntries(agentId, entriesNode, ownerKey, 1L, null);
    }
    ...
    private void applyStructuredEntries(Long agentId, JsonNode entriesNode, String ownerKey,
        ...
        for (JsonNode entry : entriesNode) {
            ...
            MemoryWriteDecision decision = admit(workspaceId, agentId, ownerKey, type, key, content,
                    "auto-summary", conversationId);
            if (!decision.admitted()) {
                log.info("[Memory] Rejected auto-summary entry '{}/{}': {}", type, key, decision.reason());
                continue;
            }
            try {
                structuredMemoryService.remember(agentId, type, key, content, "auto-summary", ownerKey);
                markApplied(decision);
                written++;
            } catch (Exception e) {
                markFailed(decision, e.getMessage());
    ...
    private boolean saveMemoryIfAdmitted(Long agentId, Long workspaceId, String ownerKey,
                                         String type, String key, String filename, String content,
                                         String conversationId) {
        MemoryWriteDecision decision = admit(workspaceId, agentId, ownerKey, type, key, content,
                "auto-summary", conversationId);
        if (!decision.admitted()) {
            log.info("[Memory] Rejected auto-summary write '{}': {}", filename, decision.reason());
            return false;
        }
        try {
            saveMemory(agentId, filename, content, ownerKey);
            markApplied(decision);
            return true;
    ...
    private MemoryWriteDecision admit(Long workspaceId, Long agentId, String ownerKey,
                                      String type, String key, String content, String source,
                                      String conversationId) {
        if (governanceService == null) {
            return MemoryWriteDecision.bypassed();
        }
        String sourceRef = conversationId == null || conversationId.isBlank()
                ? "auto-summary" : "conversation:" + conversationId;
        return governanceService.admit(new MemoryWriteRequest(workspaceId, agentId, ownerKey,
                type, key, content, source, conversationId, sourceRef));
    }

    private void markApplied(MemoryWriteDecision decision) {
        if (governanceService != null) {
            governanceService.markApplied(decision.ledgerId());
        }
    }

    private void markFailed(MemoryWriteDecision decision, String reason) {
        if (governanceService != null && decision != null && decision.admitted()) {
            governanceService.markFailed(decision.ledgerId(), reason);
        }
    }
    ...
    private boolean isPersonal(String ownerKey) {
        return ownerKey != null && !ownerKey.isBlank()
                && !vip.newsclaw.memory.identity.MemoryOwnerResolver.SYSTEM_OWNER.equals(ownerKey);
    }

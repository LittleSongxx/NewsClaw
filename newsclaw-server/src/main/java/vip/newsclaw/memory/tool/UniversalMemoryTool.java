...
import vip.newsclaw.memory.event.MemoryWriteEvent;
...
public class UniversalMemoryTool {
    ...
    /** Mandatory in the application; optional setter keeps isolated schema tests stable. */
    ...
    public String remember(
        ...
        try {
            ...
            String ownerKey = memoryProperties.isLifecycleMediatorEnabled()
                    ...
                    : null;
            decision = admit(origin, parsedAgentId, ownerKey, content, source);
    ...
    private MemoryWriteDecision admit(ChatOrigin origin, Long agentId, String ownerKey,
        ...
        String sourceRef = source == null || source.isBlank()
                ? (conversationId == null || conversationId.isBlank() ? "agent-tool" : "conversation:" + conversationId)
                : "context:" + source.trim();
        return governanceService.admit(new MemoryWriteRequest(origin.workspaceId(), agentId, ownerKey,
                "freeform", "lesson_" + Integer.toUnsignedString(content.trim().hashCode(), 16), content.trim(),
                "user-requested", conversationId, sourceRef));
    }

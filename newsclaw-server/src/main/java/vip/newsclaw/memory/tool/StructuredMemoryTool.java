package vip.newsclaw.memory.tool;
...
public class StructuredMemoryTool {
    ...
    /** Mandatory in the application; optional setter keeps focused legacy tests lightweight. */
    ...
    public String remember_structured(
        ...
        MemoryWriteDecision decision = null;
        try {
            ...
            if (origin.agentId() != null && !origin.agentId().equals(parsedAgentId)) {
                return error("agentId 与当前会话上下文不一致");
            }
            decision = admit(origin, parsedAgentId, writeOwner(toolContext),
                    type.trim().toLowerCase(), key.trim(), content.trim());
            if (!decision.admitted()) {
                return error("未写入长期记忆: " + decision.reason());
            }
    ...
    private MemoryWriteDecision admit(ChatOrigin origin, Long agentId, String ownerKey,
                                      String type, String key, String content) {
        ...
        String sourceRef = conversationId == null || conversationId.isBlank()
                ? "agent-tool" : "conversation:" + conversationId;
        return governanceService.admit(new MemoryWriteRequest(origin.workspaceId(), agentId, ownerKey,
                type, key, content, "user-requested", conversationId, sourceRef));
    }
    ...
    private void markFailed(MemoryWriteDecision decision, String reason) {
        if (decision != null && decision.admitted() && governanceService != null) {

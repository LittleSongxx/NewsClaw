"""
Global pytest fixtures for NewsClaw test suite.
"""
...
def test_settings(tmp_workspace: Path):
    ...
    os.environ["NEWSCLAW_PROJECT_ROOT"] = str(tmp_workspace)
    ...
    os.environ.pop("NEWSCLAW_PROJECT_ROOT", None)
...
# the web_search provider refactor (see src/newsclaw/tools/tool_hints.py).
#
...
# "✅ NewsClaw 任务完成 / daily research"，体感极差且查不到源头。
#
...
def _disable_desktop_notifications(monkeypatch):
    ...
    modules: list = []
    try:
        from newsclaw.agent import desktop_notify as _dn_core

        modules.append(_dn_core)
    except Exception:
        pass
    try:
        from newsclaw.agent import desktop_notify as _dn_agent
...
def _default_org_review_off(monkeypatch):
    ...
    monkeypatch.setenv("NEWSCLAW_ORG_LEGACY_TEXT_DISPATCH", "1")
...
def _isolate_policy_v2_singletons():
    ...
    """
    try:
        from newsclaw.core.policy_v2.death_switch import (
            reset_death_switch_tracker,
        )
        from newsclaw.core.policy_v2.session_allowlist import (
            reset_session_allowlist_manager,
        )
        from newsclaw.core.policy_v2.skill_allowlist import (

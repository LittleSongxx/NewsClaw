...
from newsclaw.api.routes.memory import router
from newsclaw.memory.manager import MemoryManager
from newsclaw.memory.owner_migration import align_desktop_owner
from newsclaw.memory.relational.store import RelationalMemoryStore
from newsclaw.memory.relational.types import MemoryEdge, MemoryNode
from newsclaw.memory.types import MemoryType, SemanticMemory


@pytest.fixture
def manager(tmp_path, monkeypatch):
    monkeypatch.delenv("NEWSCLAW_DESKTOP_SESSION_TOKEN", raising=False)
    mm = MemoryManager(
        tmp_path / "memory",
        tmp_path / "MEMORY.md",
        search_backend="fts5",
...
def test_upgrade_restores_list_graph_and_session_ownership(manager, monkeypatch):
    ...
    monkeypatch.setenv("NEWSCLAW_DESKTOP_SESSION_TOKEN", "test-desktop-launch")
    ...
    backups = list(manager.data_dir.glob("newsclaw.db.bak.desktop_owner.*"))
    ...
    assert len(list(manager.data_dir.glob("newsclaw.db.bak.desktop_owner.*"))) == 1
...
def test_shared_or_ambiguous_database_is_not_automatically_claimed(manager, surface, user):
    ...
    assert not list(manager.data_dir.glob("newsclaw.db.bak.desktop_owner.*"))
...
def test_backup_failure_keeps_owners_and_fallback_unchanged(manager, monkeypatch):
    ...
    monkeypatch.setenv("NEWSCLAW_DESKTOP_SESSION_TOKEN", "test")
    monkeypatch.setattr(
        "newsclaw.memory.owner_migration.sqlite3.connect",
...
def test_shared_managers_do_not_migrate_again_during_runtime(manager, monkeypatch):
    ...
    try:
        ...
        assert len(list(manager.data_dir.glob("newsclaw.db.bak.desktop_owner.*"))) == 2
    finally:

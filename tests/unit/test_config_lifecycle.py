...
from newsclaw.config_lifecycle import (
    ConfigApplyMode,
...
async def test_classify_config_api_reuses_runtime_lifecycle_policy(monkeypatch):
    from newsclaw.api.routes.config import ConfigClassifyRequest, classify_config
...
def test_desktop_component_reload_resets_cached_config(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "newsclaw.tools.desktop.config", desktop_config)
...
def test_desktop_component_reload_does_not_import_unloaded_platform_module(monkeypatch):
    monkeypatch.delitem(sys.modules, "newsclaw.tools.desktop.config", raising=False)
    plan = classify_config_changes(["DESKTOP_ENABLED"])

    assert reload_config_components(plan) == {"desktop": "reloaded"}
    assert "newsclaw.tools.desktop.config" not in sys.modules

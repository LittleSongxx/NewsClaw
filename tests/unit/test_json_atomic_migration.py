...
def _hold_transaction_lock(path: str, entered, release) -> None:
    from newsclaw.utils.atomic_io import path_transaction_lock
...
def test_atomic_json_write_can_fail_without_direct_overwrite(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("newsclaw.utils.atomic_io.time.sleep", lambda _seconds: None)
...
def test_orchestrator_persist_uses_atomic_json_write(tmp_path, monkeypatch):
    ...
    from newsclaw.utils import atomic_io as _io
    ...
    monkeypatch.setattr("newsclaw.agents.orchestrator.atomic_json_write", spy, raising=False)
    ...
    from newsclaw.agents import orchestrator as _o
...
def test_backup_settings_roundtrip(tmp_path):
    from newsclaw.workspace.backup import read_backup_settings, write_backup_settings
...
def test_identity_hashes_roundtrip(tmp_path):
    from newsclaw.agent.identity import _load_hashes, _save_hashes
...
def test_identity_hashes_recover_from_corruption(tmp_path):
    from newsclaw.agent.identity import _HASH_FILE, _load_hashes, _save_hashes

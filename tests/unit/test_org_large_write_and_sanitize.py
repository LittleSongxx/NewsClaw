...
from newsclaw.orgs._runtime_node_tools import (
...
def fs_handler(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("newsclaw.core.policy_v2.get_config_v2", lambda: _Config())
    agent = MagicMock()
...
async def test_execute_node_tool_appends_into_per_command_workspace(
    ...
    import newsclaw.orgs._runtime_node_artifacts as _artifacts
    from newsclaw.orgs._runtime_agent_host import NodeToolHost
...
async def test_execute_node_tool_fast_fails_on_truncated_write_args(
    ...
    import newsclaw.orgs._runtime_node_tools as nt
...
def test_is_content_moderation_error_detects_dashscope_rejection() -> None:
    from newsclaw.orgs._default_agent_builder import _is_content_moderation_error

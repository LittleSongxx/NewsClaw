...
from newsclaw.startup import cancel_startup_tasks, run_optional_startup
...
async def test_agent_ready_does_not_wait_for_warmup(monkeypatch):
    from newsclaw.agent import Agent
...
def test_package_imports_do_not_load_agent_runtime_or_model_sdk():
    code = """
import sys
import newsclaw.agent
from newsclaw.agent.errors import UserCancelledError
from newsclaw.agents.profile import AgentProfile
assert 'newsclaw.core._agent_runtime' not in sys.modules
assert 'anthropic' not in sys.modules
assert 'Agent' in dir(newsclaw.agent)
"""
...
def test_startup_dependency_check_does_not_import_or_install(monkeypatch, available):
    import newsclaw.runtime_channel_deps as deps
...
async def test_sticker_warmup_and_first_request_share_initialization(tmp_path, monkeypatch):
    from newsclaw.tools.sticker import StickerEngine
...
def test_serve_publishes_chat_ready_without_waiting_for_im(tmp_path, monkeypatch, outcome):
    ...
    import newsclaw.api.server as server
    import newsclaw.config as config
    import newsclaw.logging as logging_config

    monkeypatch.setattr(logging_config, "setup_logging", lambda **_: None)
    import newsclaw.main as main
    import newsclaw.runtime_env as runtime_env
    ...
    monkeypatch.setattr("newsclaw.api.host_resolution.resolve_api_host", lambda **_: "127.0.0.1")
    monkeypatch.setattr(server, "start_api_server", api_start)
    monkeypatch.setattr(server, "update_runtime_refs", update_refs)
    monkeypatch.setattr(
        "newsclaw.llm.client.get_default_client", lambda: SimpleNamespace(close=AsyncMock())
    )
...
async def test_mcp_connect_serializes_warmup_and_first_tool_request(monkeypatch):
    from newsclaw.tools.mcp import MCPClient, MCPConnectResult
...
async def test_mcp_cancelled_transport_cleans_up_and_propagates(
    ...
    import newsclaw.tools.mcp as mcp
    ...
    monkeypatch.setattr("newsclaw.runtime_manager.build_user_subprocess_environment", lambda _: {})
    monkeypatch.setattr("newsclaw.utils.path_helper.get_macos_enriched_env", lambda env: env)
...
def test_lazy_package_exports_preserve_public_symbols():
    import newsclaw.agent as agent
    import newsclaw.agents as agents

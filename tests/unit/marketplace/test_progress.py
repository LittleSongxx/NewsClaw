...
from newsclaw.integrations.marketplace.installer import MarketplaceInstallManager
...
async def test_download_tracks_real_bytes_and_percent(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "newsclaw.integrations.marketplace.installer.httpx.AsyncClient",
...
async def test_plugin_progress_reaches_market_job_and_relay_stops(tmp_path, monkeypatch, fails):
    from newsclaw.api.routes import plugins
...
async def test_plugin_failure_details_survive_polling_and_restart_without_external_reporting(
    ...
    from newsclaw.api.routes import plugins
    from newsclaw.integrations.marketplace.installer import MarketplaceInstallError
    from newsclaw.plugins.installer import PluginInstallError

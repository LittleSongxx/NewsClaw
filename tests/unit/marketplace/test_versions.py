...
from newsclaw.integrations.marketplace.installed import inspect_installation
...
def env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NEWSCLAW_ROOT", str(tmp_path / "custom-home"))
...
async def test_nested_skill_package_keeps_marketplace_manifest(env, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("newsclaw.api.routes.skills._resolve_agent", lambda request: None)
...
def test_named_workspace_does_not_use_default_workspace_installation(env, monkeypatch):
    ...
    put_skill(payload["version"])
    monkeypatch.setattr(
        settings, "project_root", settings.newsclaw_home / "workspaces" / "research"
    )

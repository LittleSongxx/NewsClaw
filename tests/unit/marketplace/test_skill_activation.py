...
from newsclaw.integrations.marketplace.installer import (
...
async def test_installed_skill_api_exposes_persisted_marketplace_identity(installation):
    from newsclaw.api.routes.skills import _build_skills_list_response
...
def installation(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "newsclaw.integrations.marketplace.installer.settings",
    ...
    monkeypatch.setattr("newsclaw.api.routes.skills._resolve_agent", lambda request: agent)
    ...
    monkeypatch.setattr("newsclaw.api.routes.skills._propagate", propagate)
...
async def test_default_disabled_skill_is_enabled_without_enabling_other_defaults(installation):
    ...
    # one ordinary skill plus a sibling that ships disabled by default
    for slug in ("ordinary", "chinese-writing"):
        target = skills / slug
        target.mkdir(parents=True)
        (target / "SKILL.md").write_bytes((source / "SKILL.md").read_bytes())
    enabled, restart = await MarketplaceInstallManager._install_skill(source, "docx", None)
    assert enabled and not restart
    # The installed default-disabled skill turns on; the sibling default-disabled
    # skill stays off and the ordinary skill is untouched.
    assert allowlist_io.read_allowlist()[1] == {"ordinary", "docx"}
...
async def test_missing_runtime_load_requires_restart(installation, monkeypatch):
    source, _ = installation
    monkeypatch.setattr("newsclaw.api.routes.skills._propagate", AsyncMock())
    enabled, restart = await MarketplaceInstallManager._install_skill(source, "demo", None)

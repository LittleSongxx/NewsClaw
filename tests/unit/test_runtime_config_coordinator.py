...
def test_pool_invalidation_runs_in_registered_engine_thread() -> None:
    from newsclaw.core import engine_bridge
    from newsclaw.runtime_config_coordinator import RuntimeConfigCoordinator
...
def test_refresh_identity_clears_compiled_cache_and_invalidates_pools(
    ...
    from newsclaw.prompt import builder, compiler
    from newsclaw.runtime_config_coordinator import RuntimeConfigCoordinator
...
async def test_im_apply_false_is_reported_as_failure(monkeypatch) -> None:
    ...
    main_stub = ModuleType("newsclaw.main")
    main_stub.apply_im_bot = _reject
    monkeypatch.setitem(sys.modules, "newsclaw.main", main_stub)

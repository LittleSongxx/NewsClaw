...
Mirrors how `web_fetch` emits `[NEWSCLAW_SOURCE]`: the chat route can
...
from newsclaw.tools.handlers.mcp import _build_mcp_envelope
...
def test_envelope_marks_success_with_server_and_tool():
    ...
    assert line.startswith("[NEWSCLAW_MCP] ")
    payload = json.loads(line[len("[NEWSCLAW_MCP] ") :])
    assert payload == {
        "status": "ok",
        "server": "github",
        "tool": "list_repos",

...
from newsclaw.api.routes import health, memory_repair
from newsclaw.api.routes.memory_repair import _file_repair_lock, _memory_dir, _move_triplet
from newsclaw.memory.exceptions import MemoryStorageUnavailable
from newsclaw.memory.manager import MemoryManager
from newsclaw.memory.noop_store import NoopUnifiedStore
...
def test_memory_manager_enters_degraded_mode_for_corrupt_db(tmp_path: Path):
    ...
    _write_corrupt_db(data_dir / "newsclaw.db")
...
def test_last_shutdown_marker_from_previous_clean_stop_is_clean(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("newsclaw.config.settings", Settings())
    ...
    monkeypatch.setenv("NEWSCLAW_SPAWN_STARTED_AT_MS", "2000")

"""Unit tests for :func:`newsclaw.plugins.reseed.warn_on_drift`.
...
from newsclaw.plugins.reseed import (
    EXCLUDED_DIR_NAMES,
    MTIME_TOLERANCE_SEC,
    STATUS_SOURCE_NEWER,
    compute_drift,
...
def test_warn_emitted_when_source_newer(drift_tree: tuple[Path, Path], caplog):
    ...
    assert "newsclaw plugins reseed --apply" in msg
...
def test_silenced_when_flag_disabled(drift_tree: tuple[Path, Path], monkeypatch, caplog):
    ...
    from newsclaw.plugins import reseed as _reseed

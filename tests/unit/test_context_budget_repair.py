...
from newsclaw.agent.loop_budget import LoopBudgetGuard
from newsclaw.agent.reasoning import ReasoningEngine
from newsclaw.core.microcompact import microcompact
from newsclaw.prompt.builder import _build_catalogs_section
...
def test_token_anomaly_compaction_uses_configured_summary_chars(monkeypatch):
    monkeypatch.setattr(
        "newsclaw.core._reasoning_runtime.settings.context_token_anomaly_threshold", 100
    )
    monkeypatch.setattr(
        "newsclaw.core._reasoning_runtime.settings.context_cached_summary_chars", 100
    )

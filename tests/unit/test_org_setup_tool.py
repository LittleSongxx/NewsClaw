...
def test_organization_watchdog_disabled_by_default():
    from newsclaw.orgs.org_models import Organization
...
class TestGetResources:
    ...
    def test_returns_templates(self, handler, tmp_data_dir):
        with patch("newsclaw.config.settings") as mock_settings:
    ...
    def test_builtin_presets_in_list(self, handler):
        """列出的内置 profile 必须来自当前预设表（trim 后为新闻/内容/办公）。"""
        from newsclaw.agents.presets import SYSTEM_PRESETS

        result = json.loads(handler._get_resources())
        agent_ids = {a["id"] for a in result["agents"]}
        assert {"default", "ai-news-editor"} <= agent_ids
        assert agent_ids <= {p.id for p in SYSTEM_PRESETS}, sorted(agent_ids)
...
class TestCreate:
    ...
    async def test_create_success(self, handler, tmp_data_dir):
        ...
        assert "[NEWSCLAW_ORG]" in result
        marker = result.split("[NEWSCLAW_ORG]", 1)[1].strip().splitlines()[0]
...
class TestToolRegistration:
    ...
    def test_exported_from_definitions(self):
        from newsclaw.tools.definitions import ORG_SETUP_TOOLS

        assert len(ORG_SETUP_TOOLS) > 0

    def test_not_in_base_tools(self):
        from newsclaw.tools.definitions import BASE_TOOLS, ORG_SETUP_TOOLS
    ...
    def test_handler_has_create_handler(self):
        from newsclaw.tools.handlers.org_setup import create_handler
...
class TestListOrgs:
    ...
    def test_list_empty(self, handler, tmp_data_dir):
        with patch("newsclaw.config.settings") as ms:
...
class TestDeleteOrg:
    ...
    async def test_delete_nonexistent(self, handler, tmp_data_dir):
        with (
            patch("newsclaw.config.settings") as ms,
            patch("newsclaw.orgs.runtime.get_runtime", return_value=None),
        ):
            ms.data_dir = tmp_data_dir

...
Walks ``src/newsclaw/tools/definitions/*.py`` plus the desktop handler's
...
def _definitions_dir() -> Path:
    return _project_root() / "src" / "newsclaw" / "tools" / "definitions"


def _desktop_handler_path() -> Path:
    return _project_root() / "src" / "newsclaw" / "tools" / "handlers" / "desktop.py"

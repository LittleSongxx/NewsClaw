...
def test_scheduler_add_task_rejects_invalid_name():
    ...
    from newsclaw.scheduler.scheduler import TaskScheduler
    from newsclaw.scheduler.task import ScheduledTask
    ...
    bad = ScheduledTask.create(
        name="../escape",
        task_type=__import__("newsclaw.scheduler.task", fromlist=["TaskType"]).TaskType.REMINDER,
        trigger_type=__import__(
            "newsclaw.scheduler.task", fromlist=["TriggerType"]
        ).TriggerType.ONCE,

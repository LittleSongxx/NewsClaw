...
from newsclaw.api.routes import bug_report
from newsclaw.api.routes import config as config_routes
from newsclaw.llm.capabilities import infer_capabilities, is_image_generation_model
...
async def test_security_write_reports_saved_runtime_refresh_failure(monkeypatch):
    ...
    monkeypatch.setattr(
        "newsclaw.core.policy_v2.global_engine.reset_policy_v2_layer",
        _fail_reset,
    )
...
async def test_security_write_uses_request_scoped_runtime_coordinator(monkeypatch):
    from newsclaw.runtime_config_coordinator import RuntimeApplyResult
...
def test_apply_llm_runtime_config_refreshes_all_runtime_components(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "newsclaw.llm.config.load_endpoints_config",
        lambda path=None: ([], [], ["stt"], {}),
    )
    monkeypatch.setattr(
        "newsclaw.llm.client.get_default_client_if_initialized",
...
def test_collect_endpoint_summary_redacts_keys_and_keeps_diagnostic_fields(tmp_path, monkeypatch):
    ...
    import newsclaw.llm.config as llm_config

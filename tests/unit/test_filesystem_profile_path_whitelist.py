"""Profile-aware path whitelist in :class:`FilesystemHandler`.

``trust`` 仍跳过白名单；``off`` / ``enabled=False`` 不再清空路径名单。
"""
...
class TestAllowedRootsShortCircuits:
    ...
    def test_profile_off_keeps_workspace_whitelist(self, tmp_path, monkeypatch):
        """``off`` 不再当「无白名单」：仍返回 workspace.paths。"""
    ...
    def test_security_globally_disabled_keeps_whitelist(self, tmp_path, monkeypatch):
        """``cfg.enabled = False`` 不再清白名单。"""
        cfg = _make_cfg(
            enabled=False,

...
async def test_resume_writes_replay_auth_and_reschedules_task(
    ...
    from newsclaw.api.routes.pending_approvals import (
        REPLAY_TTL_SECONDS,
        _resume_task,
    )
...
async def test_resume_prunes_expired_replay_auths_before_append(
    ...
    per tool call."""
    from newsclaw.api.routes.pending_approvals import _resume_task
    from newsclaw.scheduler.task import (
        ScheduledTask,
        TaskSource,
        TaskStatus,
...
def test_engine_step7_promotes_unattended_confirm_to_allow_with_replay_auth(
    ...
    from newsclaw.core.policy_v2.context import PolicyContext, ReplayAuthorization

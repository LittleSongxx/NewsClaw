...
from newsclaw.memory.lifecycle import LifecycleManager
from newsclaw.memory.storage import _SCHEMA_VERSION, MemoryStorage
...
def test_workspace_resolver_default_behavior(monkeypatch):
    ...
    from newsclaw.memory.workspace_resolver import (
        LEGACY_DEFAULT_WORKSPACE_ID,
        resolve_memory_workspace_id,
    )

    monkeypatch.delenv("NEWSCLAW_DESKTOP_PROJECT_WORKSPACE", raising=False)
...
def test_workspace_resolver_opt_in_project_mode(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("NEWSCLAW_DESKTOP_PROJECT_WORKSPACE", "1")
...
def test_isolated_memory_md_seed_phase_2b3():
    ...
    from newsclaw.agents.factory import AgentFactory
    from newsclaw.agents.profile import AgentProfile, AgentType
    ...
    # 不应该硬编码全局 NewsClaw 身份标记
    assert "NewsClaw 的全局记忆" not in seed
...
def test_apply_memory_isolation_seeds_md_when_missing(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("newsclaw.agents.profile.get_profile_store", lambda: fake_store)
    ...
    monkeypatch.setattr(
        "newsclaw.memory.manager.MemoryManager",
...
def test_iter_owned_session_ids_phase_2b5_audit(tmp_path: Path):
    ...
    storage = MemoryStorage(tmp_path / "newsclaw.db")

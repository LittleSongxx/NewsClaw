...
from newsclaw.agent.identity import (
    Identity,
    _file_hash,
    _save_hashes,
)
...
def identity_dir(tmp_path):
    ...
    (d / "SOUL.md").write_text(
        "# Soul\n\n你是 NewsClaw，一只忠诚的秋田犬AI助手。", encoding="utf-8"
    )
...
class TestIdentityLoading:
    def test_load_all_documents(self, identity_dir):
        ...
        identity.load()
        assert "NewsClaw" in identity.soul or "秋田犬" in identity.soul
    ...
    def test_get_system_prompt_does_not_inject_newsclaw_self_identity(self, tmp_path):
        ...
        assert "你是 NewsClaw，一个全能自进化AI助手。" not in prompt

...
def test_extract_source_used_from_openakita_marker():
    event = {
        ...
        "id": "abc",
        "result": (
            '[NEWSCLAW_SOURCE] {"requested_url":"https://example.com/a",'
...
def test_extract_mcp_call_returns_structured_payload():
    event = {
        ...
        "result": (
            ...
            '[NEWSCLAW_MCP] {"status":"ok","server":"github","tool":"list_repos",'
...
def test_extract_mcp_call_ignores_other_tools():
    assert (
        _extract_mcp_call(
            {"type": "tool_call_end", "tool": "web_fetch", "result": "[NEWSCLAW_MCP] {}"}
        )
        is None
    )
    assert (
    ...
    assert (
        _extract_mcp_call(
            {"type": "tool_call_end", "tool": "call_mcp_tool", "result": "[NEWSCLAW_MCP] not-json"}
        )
        is None
    )

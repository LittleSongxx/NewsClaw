...
from newsclaw.api.routes.agents import ProfileCreateRequest, create_agent_profile
from newsclaw.experience import ToolExperienceTracker
from newsclaw.runtime_envs import (
    apply_execution_environment,
    resolve_agent_env,
    resolve_scratch_env,
    resolve_skill_env,
)
from newsclaw.skills.parser import parse_skill
from newsclaw.skills.runtime_registry import (
    mark_skill_loaded,
...
def test_execution_env_resolvers_are_scoped(monkeypatch, tmp_path):
    monkeypatch.setattr("newsclaw.runtime_envs.get_runtime_root", lambda: tmp_path / "runtime")
...
def test_apply_execution_environment_prefers_managed_python(monkeypatch, tmp_path):
    monkeypatch.setattr("newsclaw.runtime_envs.get_runtime_root", lambda: tmp_path / "runtime")
    monkeypatch.setattr(
        "newsclaw.runtime_envs.resolve_pip_index",
    ...
    assert env["NEWSCLAW_EXECUTION_ENV_SCOPE"] == "agent"
    assert env["NEWSCLAW_AGENT_PYTHON"] == str(spec.python_path)
    assert env["NEWSCLAW_EXECUTION_DEPS_HASH"] == spec.deps_hash
...
def test_skill_runtime_registry_tracks_loaded_and_pending_update(monkeypatch, tmp_path):
    monkeypatch.setattr("newsclaw.skills.runtime_registry._get_newsclaw_root", lambda: tmp_path)

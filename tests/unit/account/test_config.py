...
def test_account_feature_defaults_to_disabled_in_newsclaw_fork() -> None:
    # NewsClaw fork：账户登录默认禁用（本地自足运行），不指向上游云。
...
def test_disabled_mode_ignores_provider_configuration() -> None:
    config = AccountFeatureConfig.from_env(
        {
            "NEWSCLAW_ACCOUNT_MODE": "disabled",
            "NEWSCLAW_ACCOUNT_BASE_URL": "not-a-url",
        }
    )
...
def test_custom_mode_requires_an_explicit_origin_and_client() -> None:
    with pytest.raises(ValueError, match="BASE_URL is required"):
        AccountFeatureConfig.from_env({"NEWSCLAW_ACCOUNT_MODE": "custom"})

    with pytest.raises(ValueError, match="CLIENT_ID is required"):
        AccountFeatureConfig.from_env(
            {
                "NEWSCLAW_ACCOUNT_MODE": "custom",
                "NEWSCLAW_ACCOUNT_BASE_URL": "https://accounts.vendor.example",
            }
        )
    ...
    config = AccountFeatureConfig.from_env(
        {
            "NEWSCLAW_ACCOUNT_MODE": "custom",
            "NEWSCLAW_ACCOUNT_BASE_URL": "https://accounts.vendor.example/",
            "NEWSCLAW_ACCOUNT_CLIENT_ID": "vendor-desktop",
            "NEWSCLAW_ACCOUNT_PROVIDER": "vendor-id",
            "NEWSCLAW_ACCOUNT_DISPLAY_NAME": "Vendor Account",
        }
    )
...
def test_invalid_mode_and_provider_url_fail_closed() -> None:
    with pytest.raises(ValueError, match="NEWSCLAW_ACCOUNT_MODE"):
        AccountFeatureConfig.from_env({"NEWSCLAW_ACCOUNT_MODE": "off"})

    with pytest.raises(ValueError, match="absolute HTTP"):
        AccountFeatureConfig.from_env(
            {
                "NEWSCLAW_ACCOUNT_MODE": "custom",
                "NEWSCLAW_ACCOUNT_BASE_URL": "file:///tmp/account",
                "NEWSCLAW_ACCOUNT_CLIENT_ID": "vendor-desktop",
            }
        )
    ...
    usernames = disabled_credential_usernames(
        {
            "NEWSCLAW_ACCOUNT_BASE_URL": "https://accounts.vendor.example",
            "NEWSCLAW_ACCOUNT_CLIENT_ID": "vendor-desktop",
            "NEWSCLAW_ACCOUNT_CREDENTIAL_NAMESPACE": "vendor-release",
        }
    )

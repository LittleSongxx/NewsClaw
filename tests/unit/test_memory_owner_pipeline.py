...
from newsclaw.memory.session_identity import memory_session_id, memory_session_user
from newsclaw.memory.types import ConversationTurn, Episode, MemoryType, SemanticMemory
...
def test_new_desktop_fallback_does_not_depend_on_historical_migration(
    ...
    if reason == "ambiguous_owners":
        ...
        monkeypatch.setattr("newsclaw.memory.owner_migration.align_desktop_owner", fail_backup)
    monkeypatch.setenv("NEWSCLAW_DESKTOP_SESSION_TOKEN", "test-launch")
...
async def test_existing_session_is_realigned_when_only_owner_changed(make_manager, monkeypatch):
    ...
    monkeypatch.setattr("newsclaw.core.im_context.ensure_im_context", stop_after_memory)

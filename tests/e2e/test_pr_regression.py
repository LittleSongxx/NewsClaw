...
async def test_memory_delete_by_query_reads_turn_scoped_authorization():
    ...
    from newsclaw.agent import ToolExecutor
    from newsclaw.core.risk_intent import TurnRiskAuthorization
    from newsclaw.core.tool_execution_context import ToolExecutionContext
    from newsclaw.memory.types import MemoryType
    ...
    memory = SimpleNamespace(
        ...
        source="manual",
        content="NEWSCLAW_RISKGATE_689_REPRO_TEST — 用于 RiskGate 复现的测试记忆标识。",
    )
    ...
    turn_auth = TurnRiskAuthorization(
        original_message="请删除长期记忆中所有包含 NEWSCLAW_RISKGATE_689_REPRO_TEST 的记忆。",
        ...
        authorized_intent={
            ...
            "scope": {
                ...
                "raw": "请删除长期记忆中所有包含 NEWSCLAW_RISKGATE_689_REPRO_TEST 的记忆。",
            },
        },
    )
...
async def test_dashscope_recovered_response_no_cooldown():
    ...
    from newsclaw.llm.types import LLMResponse, StopReason, TextBlock, Usage
...
async def test_plugin_failure_jsonl_appends(tmp_path: Path):
    ...
    from newsclaw.plugins.manager import PluginManager

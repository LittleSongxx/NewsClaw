...
from newsclaw import config as config_mod
from newsclaw.agent import Agent
from newsclaw.api.routes.conversation_lifecycle import (
    BusyInfo,
    ConversationLifecycleManager,
    StartResult,
)
from newsclaw.api.routes.double_texting import DoubleTextingPolicy, resolve_policy
from newsclaw.api.routes.turn_registry import TurnRegistry
from newsclaw.core import conversation_metrics as metrics
from newsclaw.core.agent_state import AgentState, TaskState, TaskStatus
from newsclaw.sessions.session import SessionContext
...
class TestConversationLifecycleRefresh:
    ...
    async def test_refresh_extends_busy_lease_without_changing_since(self, monkeypatch) -> None:
        from newsclaw.api.routes import conversation_lifecycle as lifecycle_mod
...
class TestAppendMarkerPersistence:
    ...
    async def test_append_marker_calls_persistence(self, monkeypatch) -> None:
        from newsclaw.sessions.session import Session, SessionConfig
...
class TestSSEDeltaCoalescer:
    ...
    def test_size_threshold_flushes_immediately(self) -> None:
        from newsclaw.core.sse_throttle import DeltaCoalescer
    ...
    def test_chain_text_is_coalesced(self) -> None:
        from newsclaw.core.sse_throttle import COALESCED_TYPES, DeltaCoalescer
...
class TestShellSoftKill:
    ...
    async def test_cancelled_error_carries_partial_payload_shape(
        ...
        from newsclaw.tools.shell import ShellTool

...
def isolated_account_credentials(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("newsclaw.account.oidc.KeyringTokenStore.clear", AsyncMock())


def test_disabled_account_mode_only_exposes_capability(monkeypatch) -> None:
    monkeypatch.setenv("NEWSCLAW_ACCOUNT_MODE", "disabled")
    app = create_app()
    ...
    assert "/api/internal/newsclaw/users/status" not in paths
...
def test_custom_account_mode_mounts_provider_routes(monkeypatch) -> None:
    monkeypatch.setenv("NEWSCLAW_ACCOUNT_MODE", "custom")
    monkeypatch.setenv("NEWSCLAW_ACCOUNT_BASE_URL", "https://accounts.vendor.example")
    monkeypatch.setenv("NEWSCLAW_ACCOUNT_CLIENT_ID", "vendor-desktop")
    monkeypatch.setenv("NEWSCLAW_ACCOUNT_DISPLAY_NAME", "Vendor Account")
    app = create_app()
    paths = app.openapi()["paths"]

    assert "/api/account/status" in paths
    assert "/api/account/login/start" in paths
    assert "/api/internal/newsclaw/users/status" in paths

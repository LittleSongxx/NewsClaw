...
from newsclaw.api.server import (
...
def test_threading_watchdog_registers_named_daemon_timer(
    ...
    """
    monkeypatch.setattr(
        "newsclaw.config.settings.shutdown_force_exit_grace_s", 30, raising=False
    )
...
def test_threading_watchdog_fires_os_exit_after_grace(
    ...
    """
    monkeypatch.setattr(
        "newsclaw.config.settings.shutdown_force_exit_grace_s", 1, raising=False
    )
    ...
    app = _fresh_fake_app()
    try:
        with patch("newsclaw.api.server.os._exit", side_effect=_fake_exit):
            _arm_force_exit_watchdog_sync(app)
            timer = app.state._force_exit_task  # type: ignore[attr-defined]
            assert isinstance(timer, threading.Timer)
...
def test_threading_watchdog_disabled_when_grace_zero(
    ...
    """
    monkeypatch.setattr(
        "newsclaw.config.settings.shutdown_force_exit_grace_s", 0, raising=False
    )
...
def test_dispatcher_routes_to_threading_by_default(
    ...
    monkeypatch.setattr(
        "newsclaw.config.settings.shutdown_force_exit_use_threading",
        True,
        raising=False,
    )

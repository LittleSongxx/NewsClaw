...
from newsclaw.api.routes.account_oidc import router
from newsclaw.api.routes.marketplace import router as marketplace_router


def client(monkeypatch, mode="newsclaw"):
...
def test_standalone_backend_accepts_same_user_native_credential(monkeypatch):
    web, account = client(monkeypatch)
    monkeypatch.delenv("NEWSCLAW_DESKTOP_SESSION_TOKEN")

...
from newsclaw.evolution.log_analyzer import ErrorPattern, LogEntry
from newsclaw.evolution.self_check import SelfChecker, SelfCheckPromptUnavailableError
...
class TestSelfCheckResilience:
    ...
    def test_bundled_prompt_is_used_without_workspace_override(
        ...
        monkeypatch.setattr(
            "newsclaw.evolution.self_check.resources.files",
            ...
            "selfcheck_timeout": ErrorPattern(
                ...
                samples=[
                    LogEntry(
                        timestamp=now,
                        level="ERROR",
                        logger_name="newsclaw.scheduler.executor",
    ...
    async def test_run_daily_check_saves_partial_report_on_time_budget(
        ...
        ticks = iter([0.0, 2.0])
        monkeypatch.setattr(
            "newsclaw.evolution.self_check.time.monotonic",

...
class TestCompileAll:
    def test_compile_all_with_identity_dir(self, tmp_path):
        from newsclaw.prompt.compiler import compile_all
...
class TestBuildSystemPrompt:
    ...
    def test_build_includes_identity(self, tmp_path):
        ...
        (identity_dir / "SOUL.md").write_text(
            "# Soul\nI am NewsClaw, the loyal dog.", encoding="utf-8"
        )

        prompt = build_system_prompt(identity_dir=identity_dir, tools_enabled=False)
        assert "NewsClaw" in prompt or "loyal" in prompt or len(prompt) > 50
    ...
    def test_agent_voice_empty_falls_back_to_newsclaw(self, tmp_path):
        ...
        prompt = build_system_prompt(identity_dir=identity_dir, tools_enabled=False)
        # default fallback
        assert "NewsClaw 充满好奇" in prompt
        assert "{{agent_name}}" not in prompt
    ...
    def test_agent_voice_replaces_in_none_mode(self, tmp_path):
        ...
        from newsclaw.prompt.builder import PromptMode, build_system_prompt
        ...
        # "你是 NewsClaw" wording (other unrelated rules sections may still mention
        # the NewsClaw project by name; we only guard the identity self-reference).
        assert "你是 码哥，一个 AI 助手。" in prompt
        assert "你是 NewsClaw，一个 AI 助手。" not in prompt

    def test_agent_voice_replaces_per_model_base_prompt(self, tmp_path):
        """Per-model 基础提示词不能把自定义 Agent 重新钉死为 NewsClaw。"""
        from newsclaw.prompt.builder import PromptMode, PromptProfile, build_system_prompt
        ...
        assert "你是 NewsClaw，一个帮助用户完成各类任务的 AI 助手。" not in prompt
        assert "# Agent Identity" in prompt
        assert "# NewsClaw System" not in prompt
        assert "NewsClaw 仅指运行平台或上游开源项目，不是当前 Agent 的自称" in prompt
...
class TestAgentResolveVoice:
    ...
    def test_resolve_voice_falls_back_to_settings_when_all_empty(self):
        ...
        agent._agent_profile = None
        agent.name = ""
        # settings.agent_name defaults to "NewsClaw"

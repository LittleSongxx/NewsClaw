...
def test_discover_builtins_returns_zero_for_missing_package() -> None:
    from newsclaw.runtime.templates.registry import discover_builtins

    n = discover_builtins(package="newsclaw.runtime.templates.builtin_does_not_exist")
    assert n == 0
...
def test_template_decorator_attaches_survivable_factory_mark(
    ...
    monkeypatch.setattr(
        "newsclaw.runtime.templates.registry._PENDING", [], raising=False
    )
...
def test_collect_builtin_factories_returns_empty_for_missing_package() -> None:
    factories = collect_builtin_factories(
        package="newsclaw.runtime.templates.builtin_does_not_exist"
    )
    assert factories == []

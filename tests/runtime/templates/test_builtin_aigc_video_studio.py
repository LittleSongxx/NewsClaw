...
def test_registry_bootstrap_picks_up_template_via_discover_builtins(
    ...
    """
    from newsclaw.runtime.templates import registry as registry_mod

    monkeypatch.setattr(registry_mod, "_PENDING", [], raising=False)
    for mod in list(sys.modules):
        if mod.startswith("newsclaw.runtime.templates.builtin"):
            sys.modules.pop(mod)
...
def test_module_reexports_factory_for_external_use() -> None:
    ...
    mod = importlib.import_module("newsclaw.runtime.templates.builtin.aigc_video_studio")

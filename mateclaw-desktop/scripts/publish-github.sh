#!/usr/bin/env bash
#
# Publish NewsClaw Desktop release artifacts to GitHub Releases
#
# Prerequisites:
...
echo "╔════════════════════════════════════════════════════════╗"
echo "║       NewsClaw Desktop - Publish to GitHub            ║"
echo "╚════════════════════════════════════════════════════════╝"
echo ""
...
# macOS artifacts
for pattern in "NewsClaw*.dmg" "NewsClaw*.zip"; do
...
while IFS= read -r -d '' f; do
  ARTIFACTS+=("$f")
done < <(find "$RELEASE_DIR" -maxdepth 1 -name "NewsClaw*Setup*.exe" \
  ! -name "NewsClaw_${VERSION}_Setup.exe" -print0 2>/dev/null)

# Linux artifacts
while IFS= read -r -d '' f; do
  ARTIFACTS+=("$f")
done < <(find "$RELEASE_DIR" -maxdepth 1 -name "NewsClaw*.AppImage" -print0 2>/dev/null)
...
while IFS= read -r -d '' f; do
  ...
  ! -name "NewsClaw_${VERSION}_Setup.exe.blockmap" -print0 2>/dev/null)
...
# (NewsClaw_X.Y.Z_Setup.exe, ~600 MB) alongside the per-arch installers
# (NewsClaw_X.Y.Z_x64_Setup.exe, NewsClaw_X.Y.Z_arm64_Setup.exe). The
...
MERGED_EXE_NAME="NewsClaw_${VERSION}_Setup.exe"
X64_EXE_NAME="NewsClaw_${VERSION}_x64_Setup.exe"
ARM64_EXE_NAME="NewsClaw_${VERSION}_arm64_Setup.exe"

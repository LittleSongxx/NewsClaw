...
class XhsPackageTest {
    ...
    void setUp(@TempDir Path tempDir) {
        ...
        contentItemService = mock(ContentItemService.class);
        tool = new XhsPackageTool(cache, contentItemService);
    }
    ...
    @Test
    @DisplayName("high-risk copy is rejected before image parsing or content-ledger writes")
    void highRiskCopyRejectedBeforePackaging() {
        String out = tool.xhs_package("全国第一模型", "正文", "AI", "", null, null);

        assertTrue(out.contains("小红书合规门禁阻断"), out);
        assertFalse(out.contains("至少需要 3 张"), "compliance must run before image resolution");
        assertFalse(out.contains("在线预览"), "blocked copy must not mint a preview");
        verifyNoInteractions(contentItemService);
    }
}

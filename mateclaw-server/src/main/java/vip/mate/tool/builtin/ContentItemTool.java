...
public class ContentItemTool {
    ...
    public String content_item(
        ...
        return switch (act) {
            case "check_recent" -> checkRecent(platform, topic, days, ctx);
            case "record" -> record(platform, topic, title, status, previewUrl, externalRef, ctx);
            case "mark_published" -> markPublished(id, externalRef, ctx);
            default -> "Error: unknown action '" + act + "'. Use check_recent | record | mark_published.";
        };
    }

    private String checkRecent(String platform, String topic, Integer days, @Nullable ToolContext ctx) {
        ...
        // Keep the legacy no-context path intact for embedded callers and old
        // integrations.  Agent turns carry a ChatOrigin, so those calls use
        // the workspace-aware query and cannot leak another workspace's rows.
        List<ContentItemEntity> recent = ctx == null
                ? contentItemService.findRecent(platform, topic, window)
                : contentItemService.findRecent(workspaceFromContext(ctx), platform, topic, window);
        if (recent.isEmpty()) {
    ...
    private String markPublished(Long id, String externalRef, @Nullable ToolContext ctx) {
        if (id == null) {
            return "Error: id is required for mark_published.";
        }
        boolean marked = ctx == null
                ? contentItemService.markPublished(id, externalRef)

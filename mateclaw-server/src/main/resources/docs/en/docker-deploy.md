...
> and service containers retain the `mateclaw` technical prefix for verified
...
| `MATECLAW_ENCRYPT_KEY` | Skill Secret / datasource-password encryption key; Compose requires it and it must be backed up |
...
| `MATECLAW_SETTING_KEY` | Encryption key for sensitive `mate_system_setting` values; set and back it up |
| `MATECLAW_CORS_ALLOWED_ORIGINS` | Production allowlist, e.g. `https://newsclaw.example.com` |

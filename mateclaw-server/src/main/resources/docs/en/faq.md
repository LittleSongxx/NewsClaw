...
MateClaw can start without a cloud API key (for example with Ollama, OAuth, or no Provider configured). Docker deployments may also pre-populate non-empty Provider keys in `.env`; those values take precedence over the database at runtime and are not persisted. Without environment values, configure Providers after startup in `Settings → Models`.

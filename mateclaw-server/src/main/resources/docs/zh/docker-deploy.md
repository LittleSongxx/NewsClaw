...
> 服务容器名保留 `mateclaw` 技术前缀，以维持已验证的本地卷、网络和运维命令兼容；
> 对外产品和仓库名称为 NewsClaw。
...
| `MATECLAW_ENCRYPT_KEY` | Skill Secret / 数据源密码加密密钥，Compose 强制要求并需长期备份 |
...
| `MATECLAW_SETTING_KEY` | `mate_system_setting` 敏感值加密密钥，建议显式设置并备份 |
| `MATECLAW_CORS_ALLOWED_ORIGINS` | 生产白名单，如 `https://newsclaw.example.com` |

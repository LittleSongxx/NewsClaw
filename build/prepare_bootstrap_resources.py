...
def build_wheel() -> Path:
    ...
    wheels = sorted(out_dir.glob("newsclaw-*.whl"), key=lambda item: item.stat().st_mtime)
    if not wheels:
        raise RuntimeError("python -m build --wheel completed but no newsclaw wheel was found")
    return wheels[-1]
...
def copy_wheel(wheel: Path, wheels_dir: Path) -> Path:
    ...
    for old in wheels_dir.glob("newsclaw-*.whl"):
        old.unlink()
...
def _has_bootstrap_manifest(path: Path) -> bool:
    ...
    return (
        ...
        and manifest.get("app_name") == "newsclaw"
...
def main() -> int:
    ...
    parser.add_argument("--uv-url", default=os.environ.get("NEWSCLAW_UV_URL"))
    ...
    wheel = sorted(DIST_DIR.glob("newsclaw-*.whl"), key=lambda item: item.stat().st_mtime)[-1] if args.skip_wheel_build else build_wheel()
